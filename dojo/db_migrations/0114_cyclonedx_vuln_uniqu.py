...
class Migration(migrations.Migration):
    def rename_cyclonedx_parser_vuln_uniq(apps, schema_editor):
        ...
        findings = finding_model.objects.filter(
            test__test_type=cyclonedx_testtype, unique_id_from_tool__isnull=False
        )
    ...
    def reverse_cyclonedx_parser_vuln_uniq(apps, schema_editor):
        ...
        cyclonedx_testtype = test_type_model.objects.filter(
            name="CycloneDX Scan"
        ).first()
        if cyclonedx_testtype:
        ...
        findings = finding_model.objects.filter(
            test__test_type=cyclonedx_testtype, vuln_id_from_tool__isnull=False
        )
    ...
    operations = [
        migrations.RunPython(
            rename_cyclonedx_parser_vuln_uniq, reverse_cyclonedx_parser_vuln_uniq
        )
    ]
