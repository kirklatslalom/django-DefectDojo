...
class Migration(migrations.Migration):
    ...
    def delete_empty_jira_project_configs(apps, schema_editor):
        logger.info(
            "removing JIRA_Projects with empty instance and empty project key created due to bugs in 1.10.0"
        )
        logger.info(
            "details in https://github.com/DefectDojo/django-DefectDojo/issues/3354"
        )
        ...
        for jira_project in JIRA_Project.objects.all():
            if not jira_project.jira_instance and not jira_project.project_key:
                ...
                logger.info(
                    "removing empty JIRA_Project %i for product %i:%s engagement %i:%s",
                    jira_project.id,
                    product_id,
                    product,
                    engagement_id,
    ...
    operations = [
        migrations.RunPython(
            delete_empty_jira_project_configs, migrations.RunPython.noop
        ),
    ]
