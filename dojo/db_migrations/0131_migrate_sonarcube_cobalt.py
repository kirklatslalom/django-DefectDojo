...
def migrate_sonarqube(apps, schema_editor):
    ...
    Product_API_Scan_Configuration_model = apps.get_model(
        "dojo", "Product_API_Scan_Configuration"
    )
    Test_model = apps.get_model("dojo", "Test")

    sq_products = Sonarqube_Product_model.objects.filter(
        sonarqube_tool_config__isnull=False
    )
...
def migrate_cobalt_io(apps, schema_editor):
    ...
    cobalt_products = Cobaltio_Product_model.objects.filter(
        cobaltio_tool_config__isnull=False
    )
