...
class Migration(migrations.Migration):
    def copy_existing_tags_to_tags_from_django_tagging_field(apps, schema_editor):
        ...
        logger.info(
            "Migrating tags from django-tagging to django-tagulous step1. Enable DEBUG logging to find out more."
        )
                ...
                if obj.tags:
                    ...
                    try:
                        if hasattr(obj, "prod_type_id") and obj.prod_type_id == 0:
                            logger.warning(
                                'product found without product type (prod_type==0), changing to: "_tag migration lost and found" product type'
                            )
                            Product_Type_Model = apps.get_model("dojo", "Product_Type")
                            (
                                prod_type_lost_and_found,
                                created,
                            ) = Product_Type_Model.objects.get_or_create(
                                name="_tag migration lost and found"
                            )
                            obj.prod_type = prod_type_lost_and_found
                            obj.save()
                            logger.warning(
                                "product type successfully changed to %i",
                                prod_type_lost_and_found.id,
                            )

                        obj.save()
                    except Exception as e:
                        logger.error(
                            "Error saving old existing django-tagging tags to new string field"
                        )
    ...
    def copy_tags_from_django_tagging_field_to_new_tagulous_tags_field(
        apps, schema_editor
    ):
        # We can't import the models directly as it may be a newer
        # version than this migration expects. We use the historical version.
        logger.info(
            "Migrating tags from django-tagging to django-tagulous step2. Enable DEBUG logging to find out more."
        )
        # for model_name in ['Product']:
        for model_name in [
            "Product",
    ...
    operations = [
        ...
        migrations.RunPython(
            copy_existing_tags_to_tags_from_django_tagging_field,
        ...
        migrations.RunPython(
            copy_tags_from_django_tagging_field_to_new_tagulous_tags_field,
            migrations.RunPython.noop,
        ),
    ]
