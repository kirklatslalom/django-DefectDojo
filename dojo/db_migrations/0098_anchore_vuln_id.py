...
class Migration(migrations.Migration):
    def move_to_vuln_id(apps, schema_editor):
        ...
        anchore_scan, _ = test_type_model.objects.get_or_create(
        ...
        findings = finding_model.objects.filter(
            test__test_type=anchore_scan, unique_id_from_tool__isnull=False
        )
    ...
    def reverse_move_to_vuln_id(apps, schema_editor):
        ...
        findings = finding_model.objects.filter(
            test__test_type=anchore_scan, vuln_id_from_tool__isnull=False
        )
