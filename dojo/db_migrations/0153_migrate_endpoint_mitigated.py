...
class Migration(migrations.Migration):
    ...
    def migrate_endpoint_mitigated(apps, schema_editor):
        ...
        if all_ep.count() == 0:
            ...
            logger.warning(
                "We identified %s endpoints marked as Mitigated and their status will be updated",
                all_ep.count(),
            )

            for ep in all_ep:
                epss = Endpoint_Status.objects.select_related("finding").filter(
                    endpoint=ep, mitigated=False
                )
                for eps in epss:
                    ...
                    eps.save()
                    logger.debug(
                        'Status for finding "%s" on endpoint "%s" marked as mitigated at "%s" by "%s"',
                        str(eps.finding),
                        str(ep),
                        eps.date,
                        eps.mitigated_by,
