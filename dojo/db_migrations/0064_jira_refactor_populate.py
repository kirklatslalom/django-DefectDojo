...
class Migration(migrations.Migration):
    ...
    def populate_jira_project(apps, schema_editor):
        ...
        for jira_issue in (
            ...
            if not jira_issue.jira_project and jira_issue.finding:
                ...
                jira_projects = (
                    jira_issue.finding.test.engagement.product.jira_project_set.all()
                )
    ...
    def populate_jira_instance_name_if_empty(apps, schema_editor):
        ...
        logger.info(
            "done with data migration, now removing some fields which may take a while depending on the amount of findings"
        )

    def show_info(apps, schema_editor):
        logger.info(
            "this migration should have run successfully. if not, there is a Django Management command to manually run the data conversion"
        )
        logger.info(
            "for docker-compose execute: docker-compose exec uwsgi ./manage.py jira_refactor_data_migration"
        )
