...
class Migration(migrations.Migration):
    ...
    def dedupe_endpoint_status(apps, schema_editor):
        ...
        to_process = (
            Endpoint_Status.objects.exclude(Q(endpoint=None) | Q(finding=None))
            .values("finding", "endpoint")
        ...
        if to_process.count() == 0:
            ...
            logger.warning(
                "We identified %s group(s) of endpoint status which needs to be deduplicated",
                to_process.count(),
            )
            ...
            for eps_group in to_process:
                ...
                logger.debug(
                    'Redundant endpoint statuses on finding: "%s" & endpoint "%s" will be removed. We are '
                ...
                Endpoint_Status.objects.filter(
                    finding=eps_group.get("finding"), endpoint=eps_group.get("endpoint")
                ).exclude(id=last_id).delete()
