...
def migrate_notify_sla_from_settings_file(apps, schema_editor):
    ...
    if hasattr(settings, "SLA_NOTIFY_ACTIVE_VERIFIED_ONLY"):
        system_settings_model = apps.get_model("dojo", "System_Settings")
        logger.info(
            "Migrating value from SLA_NOTIFY_ACTIVE_VERIFIED_ONLY into system settings model"
        )
        try:
            system_setting = system_settings_model.objects.get()
            system_setting.enable_notify_sla_active_verified = (
    ...
    if hasattr(settings, "SLA_NOTIFY_WITH_JIRA_ONLY"):
        system_settings_model = apps.get_model("dojo", "System_Settings")
        logger.info(
            "Migrating value from SLA_NOTIFY_WITH_JIRA_ONLY into system settings model"
        )
        try:
            system_setting = system_settings_model.objects.get()
            system_setting.enable_notify_sla_jira_only = (
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.BooleanField(
                default=False,
                help_text="Enables Notify when time to remediate according to Finding SLA's is breached for active Findings.",
                verbose_name="Enable Notifiy SLA's Breach for active Findings",
            ),
        ),
        migrations.AddField(
            model_name="system_settings",
            ...
            field=models.BooleanField(
                default=False,
                help_text="Enables Notify when time to remediate according to Finding SLA's is breached for active, verified Findings.",
                verbose_name="Enable Notifiy SLA's Breach for active, verified Findings",
            ),
        ),
        migrations.AddField(
            model_name="system_settings",
            ...
            field=models.BooleanField(
                default=False,
                help_text="Enables Notify when time to remediate according to Finding SLA's is breached for Findings that are linked to JIRA issues.",
                verbose_name="Enable Notifiy SLA's Breach for Findings linked to JIRA",
            ),
        ),
