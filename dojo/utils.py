...
def are_urls_equal(url1, url2, fields):
    ...
    deduplicationLogger.debug(
        "Check if url %s and url %s are equal in terms of %s.", url1, url2, fields
    )
    for field in fields:
        if field == "scheme":
            ...
            logger.warning(
                "Field "
                + field
                + " is not supported by the endpoint dedupe algorithm, ignoring it."
            )
    return True
...
def do_dedupe_finding(new_finding, *args, **kwargs):
    ...
    if enabled:
        deduplicationLogger.debug(
            "dedupe for: " + str(new_finding.id) + ":" + str(new_finding.title)
        )
        ...
        if deduplicationAlgorithm == settings.DEDUPE_ALGO_UNIQUE_ID_FROM_TOOL:
            ...
            == settings.DEDUPE_ALGO_UNIQUE_ID_FROM_TOOL_OR_HASH_CODE
        ):
            deduplicate_uid_or_hash_code(new_finding)
        else:
            deduplicationLogger.debug(
                "no configuration per parser found; using legacy algorithm"
            )
            deduplicate_legacy(new_finding)
    else:
        deduplicationLogger.debug(
            "dedupe: skipping dedupe because it's disabled in system settings get()"
        )
...
def deduplicate_legacy(new_finding):
    ...
    if new_finding.test.engagement.deduplication_on_engagement:
        eng_findings_cwe = (
            Finding.objects.filter(
                test__engagement=new_finding.test.engagement, cwe=new_finding.cwe
            )
            .exclude(id=new_finding.id)
            .exclude(cwe=0)
        ...
        eng_findings_title = (
            Finding.objects.filter(
                test__engagement=new_finding.test.engagement, title=new_finding.title
            )
            .exclude(id=new_finding.id)
            .exclude(duplicate=True)
            .values("id")
    ...
    total_findings = Finding.objects.filter(
        Q(id__in=eng_findings_cwe) | Q(id__in=eng_findings_title)
    ).prefetch_related(
        "endpoints",
        "test",
        "test__engagement",
    ...
    for find in total_findings.order_by("id"):
        ...
        if find.endpoints.count() != 0 and new_finding.endpoints.count() != 0:
            ...
            if all(x in list1 for x in list2):
                deduplicationLogger.debug(
                    "%s: existing endpoints are present in new finding", find.id
                )
                flag_endpoints = True
        elif (
            new_finding.static_finding
            and new_finding.file_path
            and len(new_finding.file_path) > 0
        ):
            if (
                str(find.line) == str(new_finding.line)
            ...
            deduplicationLogger.debug(
                "find.static/dynamic: %s/%s", find.static_finding, find.dynamic_finding
            )
            deduplicationLogger.debug(
                "new_finding.static/dynamic: %s/%s",
                new_finding.static_finding,
                new_finding.dynamic_finding,
            )
            deduplicationLogger.debug("find.file_path: %s", find.file_path)
            deduplicationLogger.debug(
                "new_finding.file_path: %s", new_finding.file_path
            )
...
def deduplicate_unique_id_from_tool(new_finding):
    ...
    deduplicationLogger.debug(
        "Found "
        + str(len(existing_findings))
        + " findings with same unique_id_from_tool"
    )
...
def deduplicate_hash_code(new_finding):
    ...
    deduplicationLogger.debug(
        "Found " + str(len(existing_findings)) + " findings with same hash_code"
    )
    for find in existing_findings:
        if is_deduplication_on_engagement_mismatch(new_finding, find):
            deduplicationLogger.debug(
...
def deduplicate_uid_or_hash_code(new_finding):
    ...
    deduplicationLogger.debug(
        "Found "
        + str(len(existing_findings))
        + " findings with either the same unique_id_from_tool or hash_code"
    )
    for find in existing_findings:
        if is_deduplication_on_engagement_mismatch(new_finding, find):
            deduplicationLogger.debug(
...
def set_duplicate(new_finding, existing_finding):
    if existing_finding.duplicate:
        logger.debug(
            ...
            existing_finding.duplicate_finding.id
            if existing_finding.duplicate_finding
            else "None",
        )
    ...
    deduplicationLogger.debug(
        "Setting new finding "
        + str(new_finding.id)
        + " as a duplicate of existing finding "
        + str(existing_finding.id)
    )
                ...
                if getattr(new_finding, rule.match_field) == rule.match_text:
                    if rule.application == "Append":
                        set_attribute_rule(
                            new_finding,
                            rule,
                            (getattr(new_finding, rule.applied_field) + rule.text),
                        )
                    else:
...
def add_breadcrumb(
    parent=None, title=None, top_level=True, url=None, request=None, clear=False
):
    title_done = False
    if clear:
    ...
    if top_level or crumbs is None:
        ...
        if parent is not None and getattr(parent, "get_breadcrumbs", None):
            ...
            title_done = True
            crumbs += [
                {"title": title, "url": request.get_full_path() if url is None else url}
            ]
    else:
        ...
        for crumb in crumbs:
            crumb_to_resolve = (
                crumb["url"]
                if "?" not in crumb["url"]
                else crumb["url"][: crumb["url"].index("?")]
            )
...
def get_punchcard_data(objs, start_date, weeks, view="Finding"):
    ...
    try:
            ...
            if created < start_of_week:
                ...
                if created >= start_of_week and created < start_of_next_week:
                    ...
                    while created >= start_of_next_week:
                        week_data, label = get_week_data(
                            start_of_week, tick, day_counts
                        )
        ...
        logger.exception(
            "Not showing punchcard graph due to exception gathering data", e
        )
        return None, None
...
def get_period_counts_legacy(
    ...
    for x in range(-1, period_interval):
        ...
        closed_in_range_count = findings_closed.filter(
            mitigated__date__range=[new_date, end_date]
        ).count()
        ...
        for finding in findings:
            if (
                new_date
                <= datetime.combine(finding.date, datetime.min.time()).replace(
                    tzinfo=timezone.get_current_timezone()
                )
                <= end_date
            ):
...
def get_period_counts(
    ...
    for x in range(-1, period_interval):
        ...
        try:
            ...
            closed_in_range_count = findings_closed.filter(
                mitigated_time__range=[new_date, end_date]
            ).count()
        ...
        if accepted_findings:
            ...
            try:
                risks_a = accepted_findings.filter(
                    risk_acceptance__created__date__range=date_range
                )
            except:
        ...
        for finding in findings:
            ...
            try:
                f_time = datetime.combine(finding.date, datetime.min.time()).replace(
                    tzinfo=tz
                )
            except:
                f_time = finding.date
...
def opened_in_period(start_date, end_date, pt):
    ...
    total_opened_in_period = Finding.objects.filter(
        ...
        total=Sum(
            Case(
                When(severity__in=("Critical", "High", "Medium", "Low"), then=Value(1)),
...
def get_page_items(request, items, page_size, prefix=""):
    return get_page_items_and_count(
        request, items, page_size, prefix=prefix, do_count=False
    )
...
def handle_uploaded_threat(f, eng):
    ...
    with open(
        settings.MEDIA_ROOT + "/threat/%s%s" % (eng.id, extension), "wb+"
    ) as destination:
...
def handle_uploaded_selenium(f, cred):
    name, extension = os.path.splitext(f.name)
    with open(
        settings.MEDIA_ROOT + "/selenium/%s%s" % (cred.id, extension), "wb+"
    ) as destination:
        for chunk in f.chunks():
            destination.write(chunk)
...
def dojo_crypto_encrypt(plaintext):
    ...
    if plaintext:
        ...
        iv = os.urandom(16)
        data = prepare_for_save(iv, encrypt(key, iv, plaintext.encode("utf-8")))
...
class Product_Tab:
    def __init__(self, product, title=None, tab=None):
        ...
        self.benchmark_type = Benchmark_Type.objects.filter(enabled=True).order_by(
            "name"
        )
...
def add_language(product, language, files=1, code=1):
    ...
    if not prod_language:
        try:
            ...
            if language_type:
                lang = Languages(
                    language=language_type, product=product, files=files, code=code
                )
                lang.save()
...
def user_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        try:
            ...
            logger.info(
                "creating default set (from template) of notifications for: "
                + str(instance)
            )
        ...
        if system_settings.default_group and system_settings.default_group_role:
            if (
                ...
                and re.fullmatch(
                    system_settings.default_group_email_pattern, instance.email
                )
...
def engagement_post_Save(sender, instance, created, **kwargs):
    if created:
        engagement = instance
        title = (
            "Engagement created for "
            + str(engagement.product)
            + ": "
            + str(engagement.name)
        )
        create_notification(
...
def sla_compute_and_notify(*args, **kwargs):
    ...
    system_settings = System_Settings.objects.get()
    if (
        not system_settings.enable_notify_sla_active
        and not system_settings.enable_notify_sla_active_verified
    ):
    ...
    try:
        if system_settings.enable_finding_sla:
            ...
            if system_settings.enable_notify_sla_active_verified:
                query = Q(
                    active=True, verified=True, is_mitigated=False, duplicate=False
                )
            ...
            for finding in findings:
                ...
                if jira_issue:
                    ...
                    if jira_instance is not None:
                        logger.debug(
                            "JIRA config for finding is {}".format(jira_instance)
                        )
                        ...
                        jiraconfig_sla_notification_enabled = (
                            jira_instance.global_jira_sla_notification
                        )

                        if (
                            jiraconfig_sla_notification_enabled
                            or product_jira_sla_comment_enabled
                        ):
                            logger.debug(
                ...
                logger.debug(
                    "Finding {} has {} days left to breach SLA.".format(
                        finding.id, sla_age
                    )
                )
                if sla_age < 0:
                    post_breach_count += 1
                    logger.info(
                        "Finding {} has breached by {} days.".format(
                            finding.id, abs(sla_age)
                        )
                    )
                    _notify(
                        finding,
                        "Finding {} - SLA breached by {} day(s)! Overdue notice".format(
                            finding.id, abs(sla_age)
                        ),
                    )
                # The finding is within the pre-breach period
                    ...
                    _notify(
                        finding,
                        "Finding {} - SLA pre-breach warning - {} day(s) left".format(
                            finding.id, sla_age
                        ),
                    )
                # The finding breaches the SLA today
                    ...
                    at_breach_count += 1
                    logger.info(
                        "Security SLA breach warning. Finding ID {} breaching today ({})".format(
                            finding.id, sla_age
                        )
                    )
                    _notify(
                        finding,
                        "Finding {} - SLA is breaching today".format(finding.id),
                    )
...
def get_words_for_field(model, fieldname):
    ...
    models = None
    if model == Finding:
        models = get_authorized_findings(
            Permissions.Finding_View, user=get_current_user()
        )
...
def add_success_message_to_response(message):
    if get_current_request():
        messages.add_message(
            get_current_request(), messages.SUCCESS, message, extra_tags="alert-success"
        )
...
def add_error_message_to_response(message):
    if get_current_request():
        messages.add_message(
            get_current_request(), messages.ERROR, message, extra_tags="alert-danger"
        )
...
def mass_model_updater(
    model_type, models, function, fields, page_size=1000, order="asc", log_prefix=""
):
    ...
    for p in range(1, total_pages):
        ...
        for model in page:
            ...
            if i > 0 and i % page_size == 0:
                ...
                batch = []
                logger.debug(
                    "%s%s out of %s models processed ...", log_prefix, i, total_count
                )
...
class async_delete:
    ...
    def delete_chunk(self, objects, **kwargs):
        for object in objects:
            try:
                ...
                logger.debug(
                    "ASYNC_DELETE: object has already been deleted elsewhere. Skipping"
                )
                # The id must be None
    ...
    def crawl(self, object, model_list, **kwargs):
        ...
        logger.debug(
            "ASYNC_DELETE: Successfully deleted "
            + self.get_object_name(object)
            + ": "
            + str(object)
        )
...
def log_user_login(sender, request, user, **kwargs):
    ...
    logger.info(
        "login user: {user} via ip: {ip}".format(
            user=user.username, ip=request.META.get("REMOTE_ADDR")
        )
    )
...
def log_user_logout(sender, request, user, **kwargs):
    ...
    logger.info(
        "logout user: {user} via ip: {ip}".format(
            user=user.username, ip=request.META.get("REMOTE_ADDR")
        )
    )
...
def log_user_login_failed(sender, credentials, request, **kwargs):
    ...
    if "username" in credentials:
        ...
        logger.error(
            "login failed because of missing username via ip: {ip}".format(
...
def get_password_requirements_string():
    ...
    if s.count(", ") == 1:
        password_requirements_string = (
            s.rsplit(", ", 1)[0] + " and " + s.rsplit(", ", 1)[1]
        )
    elif s.count(", ") > 1:
        password_requirements_string = (
            s.rsplit(", ", 1)[0] + ", and " + s.rsplit(", ", 1)[1]
