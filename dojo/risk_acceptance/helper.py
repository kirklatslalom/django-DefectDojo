...
def expire_now(risk_acceptance):
    logger.info(
        "Expiring risk acceptance %i:%s with %i findings",
        risk_acceptance.id,
        risk_acceptance,
    ...
    if risk_acceptance.reactivate_expired:
        for finding in risk_acceptance.accepted_findings.all():
            if not finding.active:
                logger.debug(
                    "%i:%s: unaccepting a.k.a reactivating finding.",
                    finding.id,
                    finding,
                )
                finding.active = True
                ...
                logger.debug(
                    "%i:%s already active, no changes made.", finding.id, finding
                )
    ...
    title = (
        ...
        + str(len(accepted_findings))
        + " accepted findings has expired for "
        + str(risk_acceptance.engagement.product)
        + ": "
        + str(risk_acceptance.engagement.name)
    )
...
def reinstate(risk_acceptance, old_expiration_date):
    if risk_acceptance.expiration_date_handled:
        logger.info(
            "Reinstating risk acceptance %i:%s with %i findings",
            risk_acceptance.id,
            risk_acceptance,
            len(risk_acceptance.accepted_findings.all()),
        )

        expiration_delta_days = get_system_setting(
            "risk_acceptance_form_default_days", 90
        )
        risk_acceptance.expiration_date = timezone.now() + relativedelta(
        ...
        for finding in risk_acceptance.accepted_findings.all():
            if finding.active:
                logger.debug(
                    "%i:%s: accepting a.k.a. deactivating finding", finding.id, finding
                )
                ...
                logger.debug(
                    "%i:%s: already inactive, not making any changes",
                    finding.id,
                    finding,
                )
...
def remove_finding_from_risk_acceptance(risk_acceptance, finding):
    logger.debug(
        "removing finding %i from risk acceptance %i", finding.id, risk_acceptance.id
    )
...
def expiration_handler(*args, **kwargs):
    ...
    logger.info(
        "expiring %i risk acceptances that are past expiration date",
    ...
    if heads_up_days > 0:
        ...
        for risk_acceptance in risk_acceptances:
            ...
            notification_title = (
                "Risk acceptance with "
                + str(len(risk_acceptance.accepted_findings.all()))
                + " accepted findings will expire on "
                + timezone.localtime(risk_acceptance.expiration_date).strftime(
                    "%b %d, %Y"
                )
                + " for "
                + str(risk_acceptance.engagement.product)
            ...
            post_jira_comments(
                risk_acceptance, expiration_warning_message_creator, heads_up_days
            )
...
def expiration_message_creator(risk_acceptance, heads_up_days=0):
    return "Risk acceptance [(%s)|%s] with %i findings has expired" % (
        ...
        get_full_url(
            reverse(
                "view_risk_acceptance",
                args=(risk_acceptance.engagement.id, risk_acceptance.id),
            )
        ),
        len(risk_acceptance.accepted_findings.all()),
    )
...
def reinstation_message_creator(risk_acceptance, heads_up_days=0):
    return (
        "Risk acceptance [(%s)|%s] with %i findings has been reinstated (expires on %s)"
        % (
            escape_for_jira(risk_acceptance.name),
            get_full_url(
                reverse(
...
def accepted_message_creator(risk_acceptance, heads_up_days=0):
    if risk_acceptance:
        return (
            "Finding has been added to risk acceptance [(%s)|%s] with %i findings (expires on %s)"
            % (
                escape_for_jira(risk_acceptance.name),
                get_full_url(
                    reverse(
        ...
        if jira_instance:
            ...
            for finding in findings:
                if finding.has_jira_issue:
                    logger.debug(
                        "Creating JIRA comment for something risk acceptance related"
                    )
                    jira_helper.add_simple_jira_comment(
                        jira_instance, finding.jira_issue, jira_comment
                    )
...
def risk_unaccept(finding, perform_save=True):
    logger.debug(
        "unaccepting finding %i:%s if it is currently risk accepted",
        finding.id,
        finding,
    )
