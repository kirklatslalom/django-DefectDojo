...
def new_sla_config(request):
    ...
    return render(request, "dojo/new_sla_config.html", {"form": tform})
...
def sla_config(request):
    ...
    add_breadcrumb(
        title="SLA Configurations", top_level=not len(request.GET), request=request
    )
    return render(
        request, "dojo/sla_config.html", {"confs": confs, "settings": settings}
    )
