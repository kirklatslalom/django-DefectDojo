...
def dashboard(request: HttpRequest) -> HttpResponse:
    ...
    finding_count = findings.filter(created__date__range=date_range).count()
    mitigated_count = findings.filter(mitigated__date__range=date_range).count()
    accepted_count = findings.filter(
...
def get_severities_all(findings) -> Dict[str, int]:
    severities_all = (
        findings.values("severity").annotate(count=Count("severity")).order_by()
    )
...
def get_severities_by_month(findings, today):
    severities_by_month = (
        findings.filter(created__date__gte=(today - relativedelta(months=6)))
    ...
    for ms in severities_by_month:
        ...
        month_stats = results.setdefault(
            key, {"y": key, "a": 0, "b": 0, "c": 0, "d": 0, "e": 0, None: 0}
        )
