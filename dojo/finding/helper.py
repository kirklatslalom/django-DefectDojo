...
INACTIVE_FINDINGS_QUERY = Q(
    active=False, duplicate=False, is_mitigated=False, false_p=False, out_of_scope=False
)
ACCEPTED_FINDINGS_QUERY = Q(risk_accepted=True)
NOT_ACCEPTED_FINDINGS_QUERY = Q(risk_accepted=False)
WAS_ACCEPTED_FINDINGS_QUERY = Q(risk_acceptance__isnull=False) & Q(
...
def pre_save_finding_status_change(sender, instance, changed_fields=None, **kwargs):
    ...
    logger.debug(
        "%i: changed status fields pre_save: %s", instance.id or 0, changed_fields
    )

    for field, (old, new) in changed_fields.items():
        logger.debug(
            "%i: %s changed from %s to %s" % (instance.id or 0, field, old, new)
        )
        user = None
...
def update_finding_status(new_state_finding, user, changed_fields=None):
    ...
    is_new_finding = not changed_fields or (
        changed_fields and len(changed_fields) == 1 and "id" in changed_fields
    )
    ...
    if is_new_finding or "is_mitigated" in changed_fields:
        ...
        if new_state_finding.is_mitigated:
            ...
            logger.debug(
                "finding being mitigated, set mitigated and mitigated_by fields"
            )
...
def create_finding_group(finds, finding_group_name):
    ...
    finding_group_name_dummy = "bulk group " + strftime(
    ...
    if finding_group is not None:
        if finding_group != finding.finding_group:
            if finding.finding_group:
                logger.debug(
                    "removing finding %d from finding_group %s",
                    finding.id,
                    finding.finding_group,
                )
                finding.finding_group.findings.remove(finding)
            logger.debug(
                "adding finding %d to finding_group %s", finding.id, finding_group
            )
...
def get_group_by_group_name(finding, finding_group_by_option):
    ...
    if finding_group_by_option == "component_name":
        ...
        if finding.component_name or finding.component_version:
            group_name = "%s:%s" % (
                (finding.component_name if finding.component_name else "None"),
...
def add_finding_to_auto_group(finding, group_by, **kwargs):
    ...
    if name is not None:
        ...
        finding_group, created = Finding_Group.objects.get_or_create(
            test=test, creator=creator, name=name
        )
        if created:
            logger.debug(
                "Created Finding Group %d:%s for test %d:%s",
                finding_group.id,
                finding_group,
                test.id,
                test,
...
def finding_delete(instance, **kwargs):
    ...
    if duplicate_cluster:
        ...
        logger.debug(
            "no duplicate cluster found for finding: %d, so no need to reconfigure",
            instance.id,
        )
...
def finding_post_delete(sender, instance, **kwargs):
    logger.debug(
        "finding post_delete, sender: %s instance: %s",
        to_str_typed(sender),
...
def reconfigure_duplicate_cluster(original, cluster_outside):
    ...
    if settings.DUPLICATE_CLUSTER_CASCADE_DELETE:
        ...
        logger.debug(
            "reconfigure_duplicate_cluster: cluster_outside: %s", cluster_outside
        )
        # set new original to first finding in cluster (ordered by id)
        new_original = cluster_outside.order_by("id").first()
        if new_original:
            logger.debug(
                "changing original of duplicate cluster %d to: %s:%s",
                original.id,
                new_original.id,
                new_original.title,
            )
...
def prepare_duplicates_for_delete(test=None, engagement=None):
    logger.debug(
        "prepare duplicates for delete, test: %s, engagement: %s",
    ...
    if len(originals) == 0:
        logger.debug(
            "no originals found, so no duplicates to prepare for deletion of original"
        )
        return
    ...
    for original in originals:
        i += 1
        logger.debug(
            "%d/%d: preparing duplicate cluster for deletion of original: %d",
            i,
            total,
            original.id,
        )
        ...
        logger.debug(
            "done preparing duplicate cluster for deletion of original: %d", original.id
        )
...
def test_pre_delete(sender, instance, **kwargs):
    logger.debug(
        "test pre_delete, sender: %s instance: %s",
        to_str_typed(sender),
        to_str_typed(instance),
    )
...
def test_post_delete(sender, instance, **kwargs):
    logger.debug(
        "test post_delete, sender: %s instance: %s",
        to_str_typed(sender),
        to_str_typed(instance),
    )
...
def engagement_pre_delete(sender, instance, **kwargs):
    logger.debug(
        "engagement pre_delete, sender: %s instance: %s",
        to_str_typed(sender),
        to_str_typed(instance),
    )
...
def engagement_post_delete(sender, instance, **kwargs):
    logger.debug(
        "engagement post_delete, sender: %s instance: %s",
        to_str_typed(sender),
        to_str_typed(instance),
    )
...
def fix_loop_duplicates():
    ...
    if loop_count > 0:
        ...
        new_originals = Finding.objects.filter(
            duplicate_finding__isnull=True, duplicate=True
        )
        ...
        loop_count = Finding.objects.filter(
            duplicate_finding__isnull=False, original_finding__isnull=False
        ).count()
        deduplicationLogger.info(
            "%d Finding found which still has Loops, please run fix loop duplicates again"
            % loop_count
        )
    return loop_count
...
def removeLoop(finding_id, counter):
    ...
    if (real_original.id > finding_id) and (
        real_original.duplicate_finding is not None
    ):
...
def add_endpoints(new_finding, form):
    added_endpoints = save_endpoints_to_add(
        form.endpoints_to_add_list, new_finding.test.engagement.product
    )
    endpoint_ids = []
    for endpoint in added_endpoints:
        endpoint_ids.append(endpoint.id)

    new_finding.endpoints.set(
        form.cleaned_data["endpoints"] | Endpoint.objects.filter(id__in=endpoint_ids)
    )
