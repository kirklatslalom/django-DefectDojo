...
class EditRiskAcceptanceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["path"].help_text = (
            "Existing proof uploaded: %s" % self.instance.filename()
...
class EngForm(forms.ModelForm):
    ...
    product = forms.ModelChoiceField(
        label="Product", queryset=Product.objects.none(), required=True
    )
    ...
    lead = forms.ModelChoiceField(queryset=None, required=True, label="Testing Lead")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if product:
            ...
            self.fields[
                "lead"
            ].queryset = get_authorized_users_for_product_and_product_type(
...
class TestForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if obj:
            ...
            self.fields[
                "api_scan_configuration"
            ].queryset = Product_API_Scan_Configuration.objects.filter(product=product)
        else:
            self.fields["lead"].queryset = get_authorized_users(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self.instance, "test") or (
            ...
            if self.instance.risk_accepted:
                self.fields[
                    "risk_accepted"
                ].help_text = "Uncheck to unaccept the risk. Use full risk acceptance from the dropdown menu if you need advanced settings such as an expiry date."
            elif self.instance.test.engagement.product.enable_simple_risk_acceptance:
                self.fields[
                    "risk_accepted"
                ].help_text = "Check to accept the risk. Use full risk acceptance from the dropdown menu if you need advanced settings such as an expiry date."
        ...
        if self.instance.duplicate:
            self.fields[
                "duplicate"
            ].help_text = "Original finding that is being duplicated here (readonly). Use view finding page to manage duplicate relationships. Unchecking duplicate here will reset this findings duplicate status, but will trigger deduplication logic."
        else:
            self.fields[
                "duplicate"
            ].help_text = (
...
class EditPlannedRemediationDateFindingForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if finding is not None:
            self.fields[
                "planned_remediation_date"
            ].initial = finding.planned_remediation_date
...
class ReviewFindingForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["reviewers"].choices = self._get_choices(
            get_authorized_users(Permissions.Finding_View).filter(is_active=True)
        )
...
RuleFormSet = modelformset_factory(
    Child_Rule, extra=2, max_num=10, exclude=[""], can_delete=True
)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.target == "engagement":
            product_name = (
                self.product.name
                if self.product
                else self.engagement.product.name
                if self.engagement.product
                else ""
            )
            ...
            self.fields["project_key"].help_text = (
                "JIRA settings are inherited from product "
                "%s"
            ...
            if self.instance.id:
                ...
                self.fields[
                    "inherit_from_product"
                ].help_text = "Once an engagement has a JIRA Project stored, you cannot switch back to inheritance to avoid breaking existing JIRA issues"
...
class GITHUBFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            ...
            "Checking this will overwrite content of your Github issue, or create one."
        )
...
class JIRAFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if is_finding_groups_enabled():
            self.fields[
                "push_to_jira"
            ].help_text = "Checking this will overwrite content of your JIRA issue, or create one. If this finding is part of a Finding Group, the group will pushed instead of the finding."
        else:
            self.fields[
                "push_to_jira"
            ].help_text = "Checking this will overwrite content of your JIRA issue, or create one."
        ...
        if (
            ...
            self.fields[
                "jira_issue"
            ].help_text = "Changing the linked JIRA issue for finding groups is not (yet) supported."
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            if self.instance.has_jira_issue:
                ...
                self.fields[
                    "push_to_jira"
                ].help_text = "Checking this will update the existing EPIC in JIRA."
...
class GoogleSheetFieldsForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in self.all_fields:
            ...
            if (
                i.name == "id"
                or i.editable is False
                or i.many_to_one
                or i.name in protect
            ):
                self.fields["Protect " + i.name] = forms.BooleanField(
