...
from dojo.filters import ReportFindingFilter, EndpointReportFilter, EndpointFilter
...
def report_builder(request):
    ...
    return render(
        ...
        {"available_widgets": available_widgets, "in_use_widgets": in_use_widgets},
    )
...
def report_findings(request):
    ...
    paged_findings = get_page_items(
        request, findings.qs.distinct().order_by("numerical_severity"), 25
    )
...
def product_endpoint_report(request, pid):
    ...
    try:
        start_date = (
            Finding.objects.filter(endpoints__in=endpoints.qs)
            .order_by("date")[:1][0]
            .date
        )
    except:
...
def generate_report(request, obj, host_view=False):
    ...
    if type(obj).__name__ == "Product_Type":
        ...
        type(obj).__name__ == "QuerySet"
        or type(obj).__name__ == "CastTaggedQuerySet"
        or type(obj).__name__ == "TagulousCastTaggedQuerySet"
    ):
        ...
        if obj is None:
            ...
            raise Exception(
                f"Report cannot be generated for object of type {type(obj).__name__}"
            )
    ...
    if type(obj).__name__ == "Product_Type":
        ...
        context = {
            ...
            "endpoint_opened_per_month": endpoint_monthly_counts["opened_per_period"]
            if endpoint_monthly_counts is not None
            else [],
            "endpoint_active_findings": findings.qs.distinct().order_by(
                "numerical_severity"
            ),
        ...
        if host_view:
            ...
            endpoints = Endpoint.objects.filter(
                host=endpoint.host, product=endpoint.product
            ).distinct()
    ...
    if engagement:
        product_tab = Product_Tab(
            engagement.product, title="Engagement Report", tab="engagements"
        )
        product_tab.setEngagement(engagement)
    elif test:
        product_tab = Product_Tab(
            test.engagement.product, title="Test Report", tab="engagements"
        )
        ...
        if host_view:
            product_tab = Product_Tab(
                endpoint.product, title="Endpoint Host Report", tab="endpoints"
            )
        else:
            product_tab = Product_Tab(
                endpoint.product, title="Endpoint Report", tab="endpoints"
            )
...
def prefetch_related_findings_for_report(findings):
    return findings.prefetch_related(
        "test",
...
def prefetch_related_endpoints_for_report(endpoints):
    return endpoints.prefetch_related("product", "tags")
        ...
        if first_row:
            ...
            for key in dir(finding):
                if (
                    ...
                    and not callable(getattr(finding, key))
...
def excel_export(request):
    ...
    for finding in findings:
        if row_num == 1:
            ...
            col_num += 1
            cell = worksheet.cell(
                row=row_num, column=col_num, value="vulnerability_ids"
            )
            cell.font = font_bold
        ...
        if row_num > 1:
            ...
            for key in dir(finding):
                if (
                    ...
                    col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=finding.test.test_type.name
            )
            col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=finding.test.engagement.id
            )
            col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=finding.test.engagement.name
            )
            col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=finding.test.engagement.product.id
            )
            col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=finding.test.engagement.product.name
            )
            col_num += 1
