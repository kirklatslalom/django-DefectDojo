...
def personal_notifications(request):
    ...
    add_breadcrumb(
        title=_("Personal notification settings"), top_level=False, request=request
    )
...
def system_notifications(request):
    try:
        notifications_obj = Notifications.objects.get(
            user=None, product__isnull=True, template=False
        )
    except:
    ...
    add_breadcrumb(
        title=_("System notification settings"), top_level=False, request=request
    )
...
def template_notifications(request):
    ...
    add_breadcrumb(
        title=_("Template notification settings"), top_level=False, request=request
    )
