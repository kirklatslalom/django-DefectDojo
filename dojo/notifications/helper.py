...
def create_notification(event=None, **kwargs):
    ...
    if "recipients" in kwargs:
        ...
        for recipient_notifications in Notifications.objects.filter(
            user__username__in=kwargs["recipients"], user__is_active=True, product=None
        ):
        ...
        if not ("no_users" in kwargs and kwargs["no_users"] is True):
            ...
            users = (
                ...
                .filter(
                    (Q(applicable_notifications_count__gt=0) | Q(is_superuser=True))
                )
            )
    ...
    if "description" not in kwargs.keys():
        if event == "product_added":
            kwargs["description"] = _(
                "Product %(title)s has been created successfully."
                % {"title": kwargs["title"]}
            )
        elif event == "product_type_added":
            kwargs["description"] = _(
                "Product Type %(title)s has been created successfully."
                % {"title": kwargs["title"]}
            )
        else:
            kwargs["description"] = _(
                "Event %(event)s  has occurred." % {"event": str(event)}
            )
...
def create_notification_message(event, user, notification_type, *args, **kwargs):
    ...
    try:
        ...
        logger.error(
            "error during rendering of template %s exception is %s", template, e
        )
    finally:
...
def process_notifications(event, notifications=None, **kwargs):
    ...
    logger.debug(
        ...
        + ("asynchronously" if we_want_async() else "synchronously")
    )
    ...
    def _post_slack_message(channel):
        res = requests.request(
            ...
            data={
                ...
                "text": create_notification_message(
    ...
    try:
        ...
        if user is not None:
            ...
            if get_system_setting("slack_channel") is not None:
                ...
                logger.info(
                    "Sending system notification to system channel {}.".format(channel)
                )
                _post_slack_message(channel)
            else:
                logger.debug(
        ...
        if user is None:
            if get_system_setting("msteams_url") is not None:
                ...
                res = requests.request(
                    ...
                    data=create_notification_message(
                        event, None, "msteams", *args, **kwargs
                    ),
                )
        ...
        if "error" in user:
            ...
            if "email" in user["user"]["profile"]:
                if user_email == user["user"]["profile"]["email"]:
                    ...
                    logger.warning(
                        "A user with email {} could not be found in this Slack workspace.".format(
                            user_email
                        )
                    )
...
def log_alert(e, notification_type=None, *args, **kwargs):
    ...
    for user in users:
        alert = Alerts(
            ...
            source=notification_type[:100]
            if notification_type
            else kwargs.get("source", "unknown")[:100],
        )
        # relative urls will fail validation
        alert.clean_fields(exclude=["url"])
        alert.save()
...
def notify_test_created(test):
    title = (
        "Test created for "
        + str(test.engagement.product)
        + ": "
        + str(test.engagement.name)
        + ": "
        + str(test)
    )
    create_notification(
...
def notify_scan_added(
    ...
    findings_mitigated = sorted(
        list(findings_mitigated), key=lambda x: x.numerical_severity
    )
    findings_reactivated = sorted(
        list(findings_reactivated), key=lambda x: x.numerical_severity
    )
    findings_untouched = sorted(
        list(findings_untouched), key=lambda x: x.numerical_severity
    )
