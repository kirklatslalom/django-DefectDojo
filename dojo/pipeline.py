...
def social_uid(backend, details, response, *args, **kwargs):
    if settings.AZUREAD_TENANT_OAUTH2_ENABLED and isinstance(
...
def update_azure_groups(backend, uid, user=None, social=None, *args, **kwargs):
    if (
        ...
        if "groups" not in kwargs["response"] or kwargs["response"]["groups"] == "":
            logger.warn(
                "No groups in response. Stopping to update groups of user based on azureAD"
            )
            return
            ...
            for group_from_response in group_IDs:
                ...
                if is_group_id(group_from_response):
                    ...
                    logger.debug(
                        "detected "
                        + group_from_response
                        + " as group name and will not call microsoft graph"
                    )
...
def assign_user_to_groups(user, group_names, social_provider):
    for group_name in group_names:
        group, created_group = Dojo_Group.objects.get_or_create(
            name=group_name, social_provider=social_provider
        )
        if created_group:
            logger.debug(
                "Group %s for social provider %s was created",
                str(group),
                social_provider,
            )
        ...
        if is_member_created:
            logger.debug(
                "User %s become member of group %s (social provider: %s)",
                user,
                str(group),
                social_provider,
            )
...
def cleanup_old_groups_for_user(user, group_names):
    for group_member in Dojo_Group_Member.objects.select_related("group").filter(
        user=user
    ):
        group = group_member.group
        if str(group) not in group_names:
            logger.debug(
                "Deleting membership of user %s from %s group %s",
                user,
                group.social_provider,
                str(group),
            )
            group_member.delete()
        ...
        for project in projects:
            if project.path_with_namespace not in user_product_names:
                try:
                    ...
                    product = Product(
                        name=project.path_with_namespace, prod_type=product_type
                    )
                    product.save()
                product_member, created = Product_Member.objects.get_or_create(
                    product=product,
                    user=user,
                    defaults={"role": Role.objects.get(id=Roles.Owner)},
                )
                ...
                if (
                    settings.GITLAB_PROJECT_IMPORT_TAGS
                    or settings.GITLAB_PROJECT_IMPORT_URL
                ):
                    product.save()
...
def create_user(strategy, details, backend, user=None, *args, **kwargs):
    if not settings.SOCIAL_AUTH_CREATE_USER:
        ...
        return social_core.pipeline.user.create_user(
