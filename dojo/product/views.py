...
def product(request):
    ...
    add_breadcrumb(
        title=_("Product List"), top_level=not len(request.GET), request=request
    )
...
def prefetch_for_product(prods):
    ...
    if isinstance(
        ...
        prefetched_prods = prefetched_prods.annotate(
            active_engagement_count=Count(
                "engagement__id", filter=Q(engagement__active=True)
            )
        )
        prefetched_prods = prefetched_prods.annotate(
            closed_engagement_count=Count(
                "engagement__id", filter=Q(engagement__active=False)
            )
        )
        prefetched_prods = prefetched_prods.annotate(
            last_engagement_date=Max("engagement__target_start")
        )
        prefetched_prods = prefetched_prods.annotate(
        ...
        prefetched_prods = prefetched_prods.annotate(
            active_verified_finding_count=Count(
                "engagement__test__finding__id",
                filter=Q(
        ...
        active_endpoint_query = Endpoint.objects.filter(
            finding__active=True, finding__mitigated__isnull=True
        ).distinct()
        prefetched_prods = prefetched_prods.prefetch_related(
            Prefetch(
                "endpoint_set",
                queryset=active_endpoint_query,
        ...
        if get_system_setting("enable_github"):
            prefetched_prods = prefetched_prods.prefetch_related(
                Prefetch(
                    "github_pkey_set",
                    queryset=GITHUB_PKey.objects.all().select_related("git_conf"),
...
def view_product(request, pid):
    prod_query = (
        Product.objects.all()
        .select_related(
            "product_manager", "technical_contact", "team_manager", "sla_configuration"
        )
    ...
    product_type_members = get_authorized_members_for_product_type(
        prod.prod_type, Permissions.Product_Type_View
    )
    product_groups = get_authorized_groups_for_product(prod, Permissions.Product_View)
    product_type_groups = get_authorized_groups_for_product_type(
        prod.prod_type, Permissions.Product_Type_View
    )
    ...
    benchmarks = Benchmark_Product_Summary.objects.filter(
        product=prod, publish=True, benchmark_type__enabled=True
    ).order_by("benchmark_type__name")
...
def view_product_components(request, pid):
    ...
    if connection.vendor == "postgresql":
        ...
        component_query = Finding.objects.filter(
    ...
    component_words = component_query.exclude(component_name__isnull=True).values_list(
        "component_name", flat=True
    )

    return render(
        request,
        "dojo/product_components.html",
        {
            "prod": prod,
            "filter": comp_filter,
            "product_tab": product_tab,
...
def finding_querys(request, prod):
    ...
    filters["accepted"] = findings_qs.filter(ACCEPTED_FINDINGS_QUERY).filter(
        date__range=[start_date, end_date]
    )
    filters["verified"] = findings_qs.filter(
        date__range=[start_date, end_date],
        false_p=False,
        active=True,
        verified=True,
    ...
    filters["new_verified"] = findings_qs.filter(
        date__range=[week, end_date],
        false_p=False,
        verified=True,
        active=True,
        duplicate=False,
    ...
    filters["open"] = findings_qs.filter(
        date__range=[start_date, end_date],
        false_p=False,
        duplicate=False,
        out_of_scope=False,
    ...
    filters["inactive"] = findings_qs.filter(
        date__range=[start_date, end_date],
        duplicate=False,
        out_of_scope=False,
        active=False,
    ...
    filters["closed"] = findings_qs.filter(
        date__range=[start_date, end_date],
        false_p=False,
        duplicate=False,
        out_of_scope=False,
    ...
    filters["false_positive"] = findings_qs.filter(
        date__range=[start_date, end_date],
        false_p=True,
        duplicate=False,
        out_of_scope=False,
    )
    filters["out_of_scope"] = findings_qs.filter(
        date__range=[start_date, end_date],
        false_p=False,
        duplicate=False,
        out_of_scope=True,
...
def endpoint_querys(request, prod):
    ...
    endpoints_query = (
        Endpoint_Status.objects.filter(
            finding__test__engagement__product=prod,
        ...
        .annotate(severity=F("finding__severity"))
    )
    ...
    filters["accepted"] = endpoints_qs.filter(
        date__range=[start_date, end_date], risk_accepted=True
    ).order_by("date")
    filters["verified"] = endpoints_qs.filter(
        date__range=[start_date, end_date],
        false_positive=False,
        mitigated=True,
        out_of_scope=False,
    ).order_by("date")
    filters["new_verified"] = endpoints_qs.filter(
        date__range=[week, end_date],
        false_positive=False,
        mitigated=True,
        out_of_scope=False,
    ).order_by("date")
    filters["open"] = endpoints_qs.filter(
        date__range=[start_date, end_date], mitigated=False, finding__active=True
    )
    filters["inactive"] = endpoints_qs.filter(
        date__range=[start_date, end_date], mitigated=True
    )
    filters["closed"] = endpoints_qs.filter(
        date__range=[start_date, end_date], mitigated=True
    )
    filters["false_positive"] = endpoints_qs.filter(
        date__range=[start_date, end_date], false_positive=True
    )
    filters["out_of_scope"] = endpoints_qs.filter(
        date__range=[start_date, end_date], out_of_scope=True
    )
    ...
    filters["open_vulns"] = (
        ...
        .order_by("finding__cwe")
        .values("finding__cwe")
        .annotate(count=Count("finding__cwe"))
    )
...
def view_product_metrics(request, pid):
    ...
    result = EngagementFilter(
        request.GET,
        queryset=Engagement.objects.filter(product=prod, active=False).order_by(
            "-target_end"
        ),
    )
    ...
    tests = Test.objects.filter(engagement__product=prod).prefetch_related(
        "finding_set", "test_type"
    )
    tests = tests.annotate(
        verified_finding_count=Count("finding__id", filter=Q(finding__verified=True))
    )
    ...
    punchcard, ticks = get_punchcard_data(
        filters.get("open", None), start_date, weeks_between, view
    )
    ...
    return render(
        ...
        "dojo/product_metrics.html",
        {
            ...
            "reset_link": reverse("view_product_metrics", args=(prod.id,))
            + "?type="
            + view,
...
def view_engagements(request, pid):
    ...
    engs = Engagement.objects.filter(
        product=prod, active=True, status="In Progress"
    ).order_by("-updated")
    active_engs_filter = ProductEngagementFilter(
        request.GET, queryset=engs, prefix="active"
    )
    result_active_engs = get_page_items(
        request, active_engs_filter.qs, default_page_num, prefix="engs"
    )
    ...
    result_active_engs.object_list = prefetch_for_view_engagements(
        result_active_engs.object_list, recent_test_day_count
    )

    # Engagements that are queued because they haven't started or paused
    engs = Engagement.objects.filter(
        ~Q(status="In Progress"), product=prod, active=True
    ).order_by("-updated")
    queued_engs_filter = ProductEngagementFilter(
        request.GET, queryset=engs, prefix="queued"
    )
    result_queued_engs = get_page_items(
        request, queued_engs_filter.qs, default_page_num, prefix="queued_engs"
    )
    result_queued_engs.object_list = prefetch_for_view_engagements(
        result_queued_engs.object_list, recent_test_day_count
    )

    # Cancelled or Completed Engagements
    engs = Engagement.objects.filter(product=prod, active=False).order_by("-target_end")
    inactive_engs_filter = ProductEngagementFilter(
        request.GET, queryset=engs, prefix="closed"
    )
    result_inactive_engs = get_page_items(
        request, inactive_engs_filter.qs, default_page_num, prefix="inactive_engs"
    )
    result_inactive_engs.object_list = prefetch_for_view_engagements(
        result_inactive_engs.object_list, recent_test_day_count
    )
...
def prefetch_for_view_engagements(engagements, recent_test_day_count):
    engagements = (
        engagements.select_related("lead")
        .prefetch_related(
            Prefetch(
                "test_set",
        ...
        .annotate(
            ...
            count_findings_open=Count(
                "test__finding__id", filter=Q(test__finding__active=True)
            ),
            count_findings_open_verified=Count(
                "test__finding__id",
                filter=Q(test__finding__active=True) & Q(test__finding__verified=True),
            ),
            count_findings_close=Count(
                "test__finding__id", filter=Q(test__finding__is_mitigated=True)
            ),
            count_findings_duplicate=Count(
                "test__finding__id", filter=Q(test__finding__duplicate=True)
            ),
            count_findings_accepted=Count(
                "test__finding__id", filter=Q(test__finding__risk_accepted=True)
            ),
        )
    )
...
def new_product(request, ptid=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user_has_permission_or_403(
                request.user, product_type, Permissions.Product_Type_Add_Product
            )
                ...
                if gform.is_valid():
                    ...
                    if github_pkey.git_conf is not None and github_pkey.git_project:
                        ...
                        try:
                            ...
                            repo.create_label(
                                name="security", color="FF0000", description=description
                            )
                            repo.create_label(
                                name="security / info",
                                color="00FEFC",
                            ...
                            logger.info(
                                "Labels cannot be created - they may already exists"
                            )
    ...
    return render(
        request,
        "dojo/new_product.html",
        {"form": form, "jform": jira_project_form, "gform": gform},
    )
...
def edit_product(request, pid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            success, jform = jira_helper.process_jira_project_form(
                request, instance=jira_project, product=product
            )
            error = not success
...
def delete_product(request, pid):
        ...
        if "id" in request.POST and str(product.id) == request.POST["id"]:
            ...
            if form.is_valid():
                ...
                if get_setting("ASYNC_OBJECT_DELETE"):
                    ...
                    message = _(
                        "Product and relationships will be removed in the background."
                    )
                else:
                    message = _("Product and relationships removed.")
                    product.delete()
                messages.add_message(
                    request, messages.SUCCESS, message, extra_tags="alert-success"
                )
                create_notification(
                    event="other",
    ...
    return render(
        ...
        {"product": product, "form": form, "product_tab": product_tab, "rels": rels},
    )
...
def new_eng_for_app(request, pid, cicd=False):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            success, jira_project_form = jira_helper.process_jira_project_form(
                request, instance=None, engagement=engagement
            )
            error = not success
...
def new_tech_for_prod(request, pid):
    ...
    product_tab = Product_Tab(
        get_object_or_404(Product, id=pid), title=_("Add Technology"), tab="settings"
    )
    return render(
        request,
        "dojo/new_tech.html",
        {"form": form, "product_tab": product_tab, "pid": pid},
    )
...
def edit_technology(request, tid):
    ...
    product_tab = Product_Tab(
        technology.product, title=_("Edit Technology"), tab="settings"
    )
    return render(
        request,
        "dojo/edit_technology.html",
        {"form": form, "product_tab": product_tab, "technology": technology},
    )
...
def delete_technology(request, tid):
    ...
    product_tab = Product_Tab(
        technology.product, title=_("Delete Technology"), tab="settings"
    )
    return render(
        request,
        "dojo/delete_technology.html",
        {
...
def ad_hoc_finding(request, pid):
    ...
    jform = None
    gform = None
    form = AdHocFindingForm(
        initial={"date": timezone.now().date()}, req_resp=None, product=prod
    )
    use_jira = jira_helper.get_jira_project(test) is not None

    if request.method == "POST":
        form = AdHocFindingForm(request.POST, req_resp=None, product=prod)
        if (form["active"].value() is False or form["false_p"].value()) and form[
            "duplicate"
        ].value() is False:
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            if "githubform-push_to_github" in request.POST:
                gform = GITHUBFindingForm(
                    request.POST, prefix="jiragithub", enabled=push_all_jira_issues
                )
                if gform.is_valid():
        ...
        if get_system_setting("enable_github"):
            if GITHUB_PKey.objects.filter(product=test.engagement.product).count() != 0:
                gform = GITHUBFindingForm(
                    enabled=push_all_jira_issues, prefix="githubform"
                )
        else:
            gform = None
...
def engagement_presets(request, pid):
    ...
    return render(
        ...
        {"product_tab": product_tab, "presets": presets, "prod": prod},
    )
...
def edit_engagement_presets(request, pid, eid):
    ...
    return render(
        ...
        {"product_tab": product_tab, "tform": tform, "prod": prod},
    )
...
def add_engagement_presets(request, pid):
    ...
    return render(
        ...
        {"tform": tform, "pid": pid, "product_tab": product_tab},
    )
...
def edit_notifications(request, pid):
    ...
    if request.method == "POST":
        product_notifications = (
            Notifications.objects.filter(user=request.user).filter(product=prod).first()
        )
...
def add_product_member(request, pid):
    ...
    if request.method == "POST":
        memberform = Add_Product_MemberForm(
            request.POST, initial={"product": product.id}
        )
        if memberform.is_valid():
            if memberform.cleaned_data["role"].is_owner and not user_has_permission(
                ...
                if (
                    "users" in memberform.cleaned_data
                    and len(memberform.cleaned_data["users"]) > 0
                ):
...
def edit_product_member(request, memberid):
    ...
    product_tab = Product_Tab(
        member.product, title=_("Edit Product Member"), tab="settings"
    )
    return render(
        request,
        "dojo/edit_product_member.html",
        {
            "memberid": memberid,
            "form": memberform,
            "product_tab": product_tab,
...
def delete_product_member(request, memberid):
    ...
    product_tab = Product_Tab(
        member.product, title=_("Delete Product Member"), tab="settings"
    )
    return render(
        request,
        "dojo/delete_product_member.html",
        {
            "memberid": memberid,
            "form": memberform,
            "product_tab": product_tab,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                logger.exception(e)
                messages.add_message(
                    request, messages.ERROR, str(e), extra_tags="alert-danger"
                )
    else:
        form = Product_API_Scan_ConfigurationForm()

    product_tab = Product_Tab(
        product, title=_("Add API Scan Configuration"), tab="settings"
    )
...
def edit_api_scan_configuration(request, pid, pascid):
    product_api_scan_configuration = get_object_or_404(
        Product_API_Scan_Configuration, id=pascid
    )

    if product_api_scan_configuration.product.pk != int(
        pid
    ):  # user is trying to edit Tool Configuration from another product (trying to by-pass auth)
        raise Http404()

    if request.method == "POST":
        form = Product_API_Scan_ConfigurationForm(
            request.POST, instance=product_api_scan_configuration
        )
        if form.is_valid():
            try:
        ...
        form = Product_API_Scan_ConfigurationForm(
            instance=product_api_scan_configuration
        )

    product_tab = Product_Tab(
        get_object_or_404(Product, id=pid),
        title=_("Edit API Scan Configuration"),
        tab="settings",
    )
    return render(
        request,
        "dojo/edit_product_api_scan_configuration.html",
        {"form": form, "product_tab": product_tab},
    )
...
def delete_api_scan_configuration(request, pid, pascid):
    ...
    if product_api_scan_configuration.product.pk != int(
        pid
    ):  # user is trying to delete Tool Configuration from another product (trying to by-pass auth)
        raise Http404()
    ...
    if request.method == "POST":
        ...
        form = DeleteProduct_API_Scan_ConfigurationForm(
            instance=product_api_scan_configuration
        )

    product_tab = Product_Tab(
        get_object_or_404(Product, id=pid),
        title=_("Delete Tool Configuration"),
        tab="settings",
    )
    return render(
        request,
...
def edit_product_group(request, groupid):
    ...
    if request.method == "POST":
        ...
        if groupform.is_valid():
            if group.role.is_owner and not user_has_permission(
                request.user, group.product, Permissions.Product_Group_Add_Owner
            ):
                messages.add_message(
                    request,
                    messages.WARNING,
    ...
    product_tab = Product_Tab(
        group.product, title=_("Edit Product Group"), tab="settings"
    )
    return render(
        request,
        "dojo/edit_product_group.html",
        {
            "groupid": groupid,
            "form": groupform,
            "product_tab": product_tab,
...
def delete_product_group(request, groupid):
    ...
    product_tab = Product_Tab(
        group.product, title=_("Delete Product Group"), tab="settings"
    )
    return render(
        request,
        "dojo/delete_product_group.html",
        {
            "groupid": groupid,
            "form": groupform,
            "product_tab": product_tab,
...
def add_product_group(request, pid):
    ...
    if request.method == "POST":
        group_form = Add_Product_GroupForm(
            request.POST, initial={"product": product.id}
        )
        if group_form.is_valid():
            if group_form.cleaned_data["role"].is_owner and not user_has_permission(
                ...
                if (
                    "groups" in group_form.cleaned_data
                    and len(group_form.cleaned_data["groups"]) > 0
                ):
                    for group in group_form.cleaned_data["groups"]:
                        groups = Product_Group.objects.filter(
