...
def get_authorized_products(permission, user=None):
    ...
    authorized_product_type_groups = Product_Type_Group.objects.filter(
        product_type=OuterRef("prod_type_id"), group__users=user, role__in=roles
    )
    authorized_product_groups = Product_Group.objects.filter(
        product=OuterRef("pk"), group__users=user, role__in=roles
    )
...
def get_authorized_product_members_for_user(user, permission):
    ...
    if request_user.is_superuser:
        return Product_Member.objects.filter(user=user).select_related(
            "role", "product"
        )

    if (
        hasattr(request_user, "global_role")
        and request_user.global_role.role is not None
        and role_has_permission(request_user.global_role.role.id, permission)
    ):
        return Product_Member.objects.filter(user=user).select_related(
            "role", "product"
        )
...
def get_authorized_app_analysis(permission):
    ...
    roles = get_roles_for_permission(permission)
    authorized_product_type_roles = Product_Type_Member.objects.filter(
        product_type=OuterRef("product__prod_type_id"), user=user, role__in=roles
    )
    authorized_product_roles = Product_Member.objects.filter(
...
def get_authorized_dojo_meta(permission):
    ...
    endpoint_authorized_product_groups = Product_Group.objects.filter(
        product=OuterRef("endpoint__product_id"), group__users=user, role__in=roles
    )
