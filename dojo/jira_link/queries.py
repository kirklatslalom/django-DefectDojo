...
def get_authorized_jira_projects(permission, user=None):
    ...
    engagement_authorized_product_groups = Product_Group.objects.filter(
        product=OuterRef("engagement__product_id"), group__users=user, role__in=roles
    )
...
def get_authorized_jira_issues(permission):
    ...
    jira_issues = jira_issues.annotate(
        ...
        finding_group__test__engagement__product__prod_type__authorized_group=Exists(
            finding_group_authorized_product_type_groups
        ),
