    ...
    if request.method == "POST":
        try:
            ...
            if parsed.get("webhookEvent") == "jira:issue_updated":
                ...
                if jissue.finding:
                    logging.info(
                        f"Received issue update for {jissue.jira_key} for finding {jissue.finding.id}"
                    )
                    findings = [jissue.finding]
                elif jissue.finding_group:
                    logging.info(
                        f"Received issue update for {jissue.jira_key} for finding group {jissue.finding_group}"
                    )
                    ...
                    logging.info(
                        f"Received issue update for {jissue.jira_key} for unknown object"
                    )
                    raise Http404(
                        f"No finding, finding_group or engagement found for JIRA issue {jissue.jira_key}"
                    )
            ...
            if parsed.get("webhookEvent") == "comment_created":
                ...
                if jissue.finding:
                    ...
                    create_notification(
                        event="other",
                        title=f"JIRA incoming comment - {jissue.finding}",
                        finding=jissue.finding,
                        url=reverse("view_finding", args=(jissue.finding.id,)),
                        icon="check",
                    )
                    ...
                    create_notification(
                        ...
                        finding=jissue.finding,
                        url=reverse(
                            "view_finding_group", args=(jissue.finding_group.id,)
                        ),
                        icon="check",
                    )
                    ...
                    raise Http404(
                        f"No finding or engagement found for JIRA issue {jissue.jira_key}"
                    )
                ...
                for finding in findings:
                    ...
                    new_note = Notes()
                    new_note.entry = (
                        f"({commentor_display_name} ({commentor})): {comment_text}"
                    )
            ...
            if parsed.get("webhookEvent") not in [
                ...
                logger.info(
                    f"Unrecognized JIRA webhook event received: {parsed.get('webhookEvent')}"
                )
...
def express_new_jira(request):
    if request.method == "POST":
        ...
        add_breadcrumb(
            title="New Jira Configuration (Express)", top_level=False, request=request
        )
    return render(request, "dojo/express_new_jira.html", {"jform": jform})
...
def edit_jira(request, jid):
    ...
    if request.method == "POST":
        ...
        add_breadcrumb(
            title="Edit JIRA Configuration", top_level=False, request=request
        )
