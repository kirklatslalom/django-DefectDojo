...
DEFAULT_STATS = {
    sev.lower(): {stat_field: 0 for stat_field in STATS_FIELDS} for sev in SEVERITIES
}
...
def _get_annotations_for_statistics():
    annotations = {
        stats_field.lower(): Count(Case(When(**{stats_field: True}, then=1)))
        for stats_field in STATS_FIELDS
        if stats_field != "total"
    }
    # add total
...
class Regulation(models.Model):
    ...
    name = models.CharField(
        max_length=128, unique=True, help_text=_("The name of the regulation.")
    )
    ...
    jurisdiction = models.CharField(
        max_length=64, help_text=_("The territory over which the regulation applies.")
    )
...
class Dojo_User(User):
    ...
    def force_password_reset(user):
        return (
            hasattr(user, "usercontactinfo")
            and user.usercontactinfo.force_password_reset
        )
...
class UserContactInfo(models.Model):
    ...
    slack_username = models.CharField(
        ...
        null=True,
        max_length=150,
        help_text=_("Email address associated with your slack account"),
        verbose_name=_("Slack Email Address"),
    )
    ...
    force_password_reset = models.BooleanField(
        default=False,
        help_text=_("Forces this user to reset their password on next login."),
    )


class Dojo_Group(models.Model):
    AZURE = "AzureAD"
    SOCIAL_CHOICES = ((AZURE, _("AzureAD")),)
    name = models.CharField(max_length=255, unique=True)
    description = models.CharField(max_length=4000, null=True, blank=True)
    users = models.ManyToManyField(
        Dojo_User, through="Dojo_Group_Member", related_name="users", blank=True
    )
...
class System_Settings(models.Model):
    enable_auditlog = models.BooleanField(
        ...
        help_text=_(
            "With this setting turned on, Dojo maintains an audit log "
    ...
    enable_deduplication = models.BooleanField(
        ...
        help_text=_(
            "With this setting turned on, Dojo deduplicates findings by "
    ...
    delete_duplicates = models.BooleanField(
        default=False,
        blank=False,
        help_text=_("Requires next setting: maximum number of duplicates to retain."),
    )
    ...
    email_from = models.CharField(
        max_length=200, default="no-reply@example.com", blank=True
    )

    enable_jira = models.BooleanField(
        default=False, verbose_name=_("Enable JIRA integration"), blank=False
    )
    ...
    disable_jira_webhook_secret = models.BooleanField(
        default=False,
    ...
    jira_webhook_secret = models.CharField(
        max_length=64,
        blank=False,
        null=True,
        verbose_name=_("JIRA Webhook URL"),
    ...
    jira_minimum_severity = models.CharField(
        max_length=20, blank=True, null=True, choices=jira_choices, default="Low"
    )
    ...
    add_vulnerability_id_to_jira_label = models.BooleanField(
        default=False,
        verbose_name=_("Add vulnerability Id as a JIRA label"),
        blank=False,
    )

    enable_github = models.BooleanField(
        default=False, verbose_name=_("Enable GITHUB integration"), blank=False
    )

    enable_slack_notifications = models.BooleanField(
        default=False, verbose_name=_("Enable Slack notifications"), blank=False
    )
    ...
    slack_token = models.CharField(
        ...
        blank=True,
        help_text=_(
            "Token required for interacting "
    ...
    enable_msteams_notifications = models.BooleanField(
        default=False,
        verbose_name=_("Enable Microsoft Teams notifications"),
        blank=False,
    )
    msteams_url = models.CharField(
        max_length=400,
        default="",
        blank=True,
    ...
    enable_product_grade = models.BooleanField(
        default=False,
        verbose_name=_("Enable Product Grading"),
        help_text=_(
    ...
    product_grade_a = models.IntegerField(
        ...
        help_text=_("Percentage score for an " "'A' >="),
    )
    product_grade_b = models.IntegerField(
        default=80,
        verbose_name=_("Grade B"),
        help_text=_("Percentage score for a " "'B' >="),
    )
    product_grade_c = models.IntegerField(
        default=70,
        verbose_name=_("Grade C"),
        help_text=_("Percentage score for a " "'C' >="),
    )
    product_grade_d = models.IntegerField(
        default=60,
        verbose_name=_("Grade D"),
        help_text=_("Percentage score for a " "'D' >="),
    )
    product_grade_f = models.IntegerField(
        default=59,
        verbose_name=_("Grade F"),
        help_text=_("Percentage score for an " "'F' <="),
    )
    enable_benchmark = models.BooleanField(
        ...
        help_text=_(
            "Enables Benchmarks such as the OWASP ASVS "
    ...
    risk_acceptance_form_default_days = models.IntegerField(
        null=True,
        blank=True,
        default=180,
        help_text=_("Default expiry period for risk acceptance form."),
    )
    risk_acceptance_notify_before_expiration = models.IntegerField(
        null=True,
        blank=True,
        default=10,
        verbose_name=_("Risk acceptance expiration heads up days"),
        help_text=_(
...
class Global_Role(models.Model):
    user = models.OneToOneField(
        Dojo_User, null=True, blank=True, on_delete=models.CASCADE
    )
    group = models.OneToOneField(
        Dojo_Group, null=True, blank=True, on_delete=models.CASCADE
    )
    role = models.ForeignKey(
        Role,
        on_delete=models.CASCADE,
...
class NoteHistory(models.Model):
    note_type = models.ForeignKey(
        Note_Type, null=True, blank=True, on_delete=models.CASCADE
    )
    data = models.TextField()
    time = models.DateTimeField(null=True, editable=False, default=get_current_datetime)
    current_editor = models.ForeignKey(
        Dojo_User, editable=False, null=True, on_delete=models.CASCADE
    )
...
class Notes(models.Model):
    ...
    date = models.DateTimeField(
        null=False, editable=False, default=get_current_datetime
    )
    ...
    edit_time = models.DateTimeField(
        null=True, editable=False, default=get_current_datetime
    )
...
class FileUpload(models.Model):
    ...
    def copy(self):
        ...
        new_file = ContentFile(
            self.file.read(), name="{}.{}".format(uuid4(), extension)
        )
        copy.file = new_file
        copy.save()
    ...
    def get_accessible_url(self, obj, obj_id):
        ...
        return "access_url/{file_id}/{obj_id}/{obj_type}".format(
            file_id=self.id, obj_id=obj_id, obj_type=obj_type
        )
...
class Product_Type(models.Model):
    ...
    members = models.ManyToManyField(
        Dojo_User,
        through="Product_Type_Member",
        related_name="prod_type_members",
        blank=True,
    )
    authorization_groups = models.ManyToManyField(
        Dojo_Group,
        through="Product_Type_Group",
        related_name="product_type_groups",
        blank=True,
    )
    ...
    def get_breadcrumbs(self):
        bc = [
            {"title": str(self), "url": reverse("edit_product_type", args=(self.id,))}
        ]
        return bc
...
class DojoMeta(models.Model):
    ...
    def clean(self):
        ...
        ids = [self.product_id, self.endpoint_id, self.finding_id]
        ids_count = 0
        ...
        if ids_count == 0:
            raise ValidationError(
                "Metadata entries need either a product, an endpoint or a finding"
            )
        if ids_count > 1:
            raise ValidationError(
                "Metadata entries may not have more than one relation, either a product, an endpoint either or a finding"
            )
...
class Product(models.Model):
    ...
    updated = models.DateTimeField(auto_now=True, null=True)
    sla_configuration = models.ForeignKey(
        SLA_Configuration,
    ...
    members = models.ManyToManyField(
        Dojo_User, through="Product_Member", related_name="product_members", blank=True
    )
    authorization_groups = models.ManyToManyField(
        Dojo_Group, through="Product_Group", related_name="product_groups", blank=True
    )
    prod_numeric_grade = models.IntegerField(null=True, blank=True)

    # Metadata
    business_criticality = models.CharField(
        max_length=9, choices=BUSINESS_CRITICALITY_CHOICES, blank=True, null=True
    )
    platform = models.CharField(
        max_length=11, choices=PLATFORM_CHOICES, blank=True, null=True
    )
    lifecycle = models.CharField(
        max_length=12, choices=LIFECYCLE_CHOICES, blank=True, null=True
    )
    origin = models.CharField(
        max_length=19, choices=ORIGIN_CHOICES, blank=True, null=True
    )
    user_records = models.PositiveIntegerField(
        blank=True,
        null=True,
        help_text=_("Estimate the number of user records within the application."),
    )
    ...
    tags = TagField(
        ...
        help_text=_(
            "Add tags that help describe this product. Choose from the list or add new tags. Press Enter key to add."
        ),
    )

    enable_simple_risk_acceptance = models.BooleanField(
        default=False,
        help_text=_("Allows simple risk acceptance by checking/unchecking a checkbox."),
    )
    enable_full_risk_acceptance = models.BooleanField(
        default=True,
        help_text=_(
    ...
    def findings_count(self):
        try:
            ...
            self.active_finding_count = Finding.objects.filter(
                active=True, test__engagement__product=self
            ).count()
            return self.active_finding_count
    ...
    def findings_active_verified_count(self):
        try:
            ...
            # ideally it's always prefetched and we can remove this code in the future
            self.active_verified_finding_count = Finding.objects.filter(
                active=True, verified=True, test__engagement__product=self
            ).count()
            return self.active_verified_finding_count
    ...
    def get_breadcrumbs(self):
        bc = [{"title": str(self), "url": reverse("view_product", args=(self.id,))}]
        return bc
...
class Tool_Configuration(models.Model):
    ...
    tool_type = models.ForeignKey(
        Tool_Type, related_name="tool_type", on_delete=models.CASCADE
    )
    authentication_type = models.CharField(
        max_length=15,
        choices=(("API", "API Key"), ("Password", "Username/Password"), ("SSH", "SSH")),
        null=True,
        blank=True,
    )
    extras = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Additional definitions that will be " "consumed by scanner"),
    )
    username = models.CharField(max_length=200, null=True, blank=True)
    password = models.CharField(max_length=600, null=True, blank=True)
    auth_title = models.CharField(
        max_length=200, null=True, blank=True, verbose_name=_("Title for SSH/API Key")
    )
    ssh = models.CharField(max_length=6000, null=True, blank=True)
    api_key = models.CharField(
        max_length=600, null=True, blank=True, verbose_name=_("API Key")
    )
...
class Product_API_Scan_Configuration(models.Model):
    ...
    tool_configuration = models.ForeignKey(
        Tool_Configuration, null=False, blank=False, on_delete=models.CASCADE
    )
...
class ToolConfigForm_Admin(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            not cleaned_data["password"]
            and not cleaned_data["ssh"]
            and not cleaned_data["api_key"]
        ):
...
class Engagement_Presets(models.Model):
    title = models.CharField(
        max_length=500, default=None, help_text=_("Brief description of preset.")
    )
    test_type = models.ManyToManyField(Test_Type, default=None, blank=True)
    network_locations = models.ManyToManyField(
        Network_Locations, default=None, blank=True
    )
    ...
    scope = models.CharField(
        max_length=800,
        help_text=_("Scope of Engagement testing, IP's/Resources/URL's)"),
        default=None,
        blank=True,
    )
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
...
class Engagement(models.Model):
    ...
    version = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Version of the product the engagement tested."),
    )
    ...
    lead = models.ForeignKey(
        Dojo_User, editable=True, null=True, blank=True, on_delete=models.RESTRICT
    )
    ...
    preset = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text=_("Settings and notes for performing this engagement."),
        on_delete=models.CASCADE,
    )
    reason = models.CharField(max_length=2000, null=True, blank=True)
    report_type = models.ForeignKey(
        Report_Type, null=True, blank=True, on_delete=models.CASCADE
    )
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    updated = models.DateTimeField(auto_now=True, null=True)
    created = models.DateTimeField(auto_now_add=True, null=True)
    ...
    tracker = models.URLField(
        max_length=200,
        help_text=_("Link to epic or ticket system with changes to version."),
        editable=True,
        blank=True,
        null=True,
    )
    ...
    progress = models.CharField(max_length=100, default="threat_model", editable=False)
    tmodel_path = models.CharField(
        max_length=1000, default="none", editable=False, blank=True, null=True
    )
    ...
    build_id = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Build ID of the product the engagement tested."),
    ...
    commit_hash = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Commit hash from repo"),
    ...
    branch_tag = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Tag or branch of the product the engagement tested."),
        verbose_name=_("Branch/Tag"),
    )
    build_server = models.ForeignKey(
        Tool_Configuration,
        verbose_name=_("Build Server"),
        help_text=_("Build server responsible for CI/CD test"),
        null=True,
        blank=True,
    ...
    source_code_management_server = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_("SCM Server"),
        help_text=_("Source code server for CI/CD test"),
    ...
    orchestration_engine = models.ForeignKey(
        Tool_Configuration,
        verbose_name=_("Orchestration Engine"),
        help_text=_("Orchestration service responsible for CI/CD test"),
        null=True,
        blank=True,
        related_name="orchestration",
        on_delete=models.CASCADE,
    )
    deduplication_on_engagement = models.BooleanField(
        default=False,
        verbose_name=_("Deduplication within this engagement only"),
        help_text=_(
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this engagement. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
    ...
    def get_breadcrumbs(self):
        ...
        bc += [{"title": str(self), "url": reverse("view_engagement", args=(self.id,))}]
        return bc
...
class Endpoint_Params(models.Model):
    ...
    method = models.CharField(
        max_length=20, blank=False, null=True, choices=method_type
    )
...
class Endpoint_Status(models.Model):
    ...
    mitigated_by = models.ForeignKey(
        Dojo_User, editable=True, null=True, on_delete=models.RESTRICT
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["finding", "endpoint"], name="endpoint-finding relation"
            )
        ]
...
class Endpoint(models.Model):
    ...
    host = models.CharField(
        ...
        max_length=500,
        help_text=_(
            "The host name or IP address. It must not include the port number. "
    ...
    query = models.CharField(
        ...
        max_length=1000,
        help_text=_(
            "The query string, the question mark should be omitted."
    ...
    endpoint_params = models.ManyToManyField(
        Endpoint_Params, blank=True, editable=False
    )
    findings = models.ManyToManyField(
        "Finding", blank=True, verbose_name=_("Findings"), through=Endpoint_Status
    )
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this endpoint. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
    ...
    def clean(self):
        ...
        if self.protocol or self.protocol == "":
            if not re.match(
                r"^[A-Za-z][A-Za-z0-9\.\-\+]+$", self.protocol
            ):  # https://tools.ietf.org/html/rfc3986#section-3.1
                errors.append(
                    ValidationError(
                        'Protocol "{}" has invalid format'.format(self.protocol)
                    )
                )
            if self.protocol == "":
                self.protocol = None
        ...
        if self.userinfo or self.userinfo == "":
            if not re.match(
                r"^[A-Za-z0-9\.\-_~%\!\$&\'\(\)\*\+,;=:]+$", self.userinfo
            ):  # https://tools.ietf.org/html/rfc3986#section-3.2.1
                errors.append(
                    ValidationError(
                        'Userinfo "{}" has invalid format'.format(self.userinfo)
                    )
                )
            if self.userinfo == "":
                self.userinfo = None
        ...
        if self.host:
            if not re.match(r"^[A-Za-z0-9_\-\+][A-Za-z0-9_\.\-\+]+$", self.host):
                try:
                    ...
                    errors.append(
                        ValidationError(
                            'Host "{}" has invalid format'.format(self.host)
                        )
                    )
        else:
        ...
        if self.port or self.port == 0:
            try:
                ...
                if not (0 <= int_port < 65536):
                    errors.append(
                        ValidationError(
                            'Port "{}" has invalid format - out of range'.format(
                                self.port
                            )
                        )
                    )
                ...
                errors.append(
                    ValidationError(
                        'Port "{}" has invalid format - it is not a number'.format(
                            self.port
                        )
                    )
                )
        ...
        if self.path or self.path == "":
            while (
                len(self.path) > 0 and self.path[0] == "/"
            ):  # Endpoint store "root-less" path
            ...
            if any([null_char in self.path for null_char in null_char_list]):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Path "{}" has invalid format - It contains the NULL character. The following action was taken: {}'.format(
                                old_value, action_string
                            )
                        )
                    )
            if self.path == "":
        ...
        if self.query or self.query == "":
            ...
            if any([null_char in self.query for null_char in null_char_list]):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Query "{}" has invalid format - It contains the NULL character. The following action was taken: {}'.format(
                                old_value, action_string
                            )
                        )
                    )
        ...
        if self.fragment or self.fragment == "":
            ...
            if any([null_char in self.fragment for null_char in null_char_list]):
                ...
                if "postgres" in db_type:
                    ...
                    errors.append(
                        ValidationError(
                            'Fragment "{}" has invalid format - It contains the NULL character. The following action was taken: {}'.format(
                                old_value, action_string
                            )
                        )
                    )
        ...
        try:
            if self.host:
                ...
                url = hyperlink.EncodedURL(
                    ...
                    query=tuple(
                        (qe.split("=", 1) if "=" in qe else (qe, None))
                        for qe in self.query.split("&")
                    )
                    if self.query
                    else (),  # inspired by https://github.com/python-hyper/hyperlink/blob/b8c9152cd826bbe8e6cc125648f3738235019705/src/hyperlink/_url.py#L1427
    ...
    def host_endpoints(self):
        return Endpoint.objects.filter(host=self.host, product=self.product).distinct()
    ...
    def host_mitigated_endpoints(self):
        meps = Endpoint_Status.objects.filter(
            endpoint__in=self.host_endpoints()
        ).filter(
            Q(mitigated=True)
    ...
    def get_breadcrumbs(self):
        bc = self.product.get_breadcrumbs()
        bc += [{"title": self.host, "url": reverse("view_endpoint", args=(self.id,))}]
        return bc
    ...
    def from_uri(uri):
        ...
        query_parts = (
            []
        )  # inspired by https://github.com/python-hyper/hyperlink/blob/b8c9152cd826bbe8e6cc125648f3738235019705/src/hyperlink/_url.py#L1768
        for k, v in url.query:
            if v is None:
                query_parts.append(k)
            else:
                query_parts.append("=".join([k, v]))
...
class Development_Environment(models.Model):
    ...
    def get_breadcrumbs(self):
        return [{"title": str(self), "url": reverse("edit_dev_env", args=(self.id,))}]


class Sonarqube_Issue(models.Model):
    key = models.CharField(
        max_length=30, unique=True, help_text=_("SonarQube issue key")
    )
...
class Sonarqube_Issue_Transition(models.Model):
    sonarqube_issue = models.ForeignKey(
        Sonarqube_Issue, on_delete=models.CASCADE, db_index=True
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
...
class Test(models.Model):
    ...
    percent_complete = models.IntegerField(null=True, blank=True, editable=True)
    notes = models.ManyToManyField(Notes, blank=True, editable=False)
    files = models.ManyToManyField(FileUpload, blank=True, editable=False)
    environment = models.ForeignKey(
        Development_Environment, null=True, blank=False, on_delete=models.RESTRICT
    )
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this test. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
    ...
    commit_hash = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("Commit hash tested, a reimport may update this field."),
        verbose_name=_("Commit Hash"),
    )
    branch_tag = models.CharField(
        editable=True,
        ...
        null=True,
        blank=True,
        help_text=_("Tag or branch that was tested, a reimport may update this field."),
        verbose_name=_("Branch/Tag"),
    )
    api_scan_configuration = models.ForeignKey(
        Product_API_Scan_Configuration,
        null=True,
        editable=True,
        blank=True,
        on_delete=models.CASCADE,
        verbose_name=_("API Scan Configuration"),
    )
    ...
    def get_breadcrumbs(self):
        ...
        bc += [{"title": str(self), "url": reverse("view_test", args=(self.id,))}]
        return bc
    ...
    def unaccepted_open_findings(self):
        return Finding.objects.filter(
            risk_accepted=False, active=True, verified=True, duplicate=False, test=self
        )
        ...
        if hasattr(settings, "DEDUPLICATION_ALGORITHM_PER_PARSER"):
            if self.test_type.name in settings.DEDUPLICATION_ALGORITHM_PER_PARSER:
                deduplicationLogger.debug(
                    f"using DEDUPLICATION_ALGORITHM_PER_PARSER for test_type.name: {self.test_type.name}"
                )
                ...
                deduplicationLogger.debug(
                    f"using DEDUPLICATION_ALGORITHM_PER_PARSER for scan_type: {self.scan_type}"
                )
                deduplicationAlgorithm = settings.DEDUPLICATION_ALGORITHM_PER_PARSER[
                    self.scan_type
                ]
            ...
            deduplicationLogger.debug(
                "Section DEDUPLICATION_ALGORITHM_PER_PARSER not found in settings.dist.py"
            )

        deduplicationLogger.debug(
            f"DEDUPLICATION_ALGORITHM_PER_PARSER is: {deduplicationAlgorithm}"
        )
    ...
    def hash_code_fields(self):
        ...
        if hasattr(settings, "HASHCODE_FIELDS_PER_SCANNER"):
            if self.test_type.name in settings.HASHCODE_FIELDS_PER_SCANNER:
                deduplicationLogger.debug(
                    f"using HASHCODE_FIELDS_PER_SCANNER for test_type.name: {self.test_type.name}"
                )
                hashCodeFields = settings.HASHCODE_FIELDS_PER_SCANNER[
                    self.test_type.name
                ]
            elif self.scan_type in settings.HASHCODE_FIELDS_PER_SCANNER:
                deduplicationLogger.debug(
                    f"using HASHCODE_FIELDS_PER_SCANNER for scan_type: {self.scan_type}"
                )
                hashCodeFields = settings.HASHCODE_FIELDS_PER_SCANNER[self.scan_type]
        else:
            deduplicationLogger.debug(
                "Section HASHCODE_FIELDS_PER_SCANNER not found in settings.dist.py"
            )
    ...
    def hash_code_allows_null_cwe(self):
        ...
        if hasattr(settings, "HASHCODE_ALLOWS_NULL_CWE"):
            if self.test_type.name in settings.HASHCODE_ALLOWS_NULL_CWE:
                deduplicationLogger.debug(
                    f"using HASHCODE_ALLOWS_NULL_CWE for test_type.name: {self.test_type.name}"
                )
                hashCodeAllowsNullCwe = settings.HASHCODE_ALLOWS_NULL_CWE[
                    self.test_type.name
                ]
            elif self.scan_type in settings.HASHCODE_ALLOWS_NULL_CWE:
                deduplicationLogger.debug(
                    f"using HASHCODE_ALLOWS_NULL_CWE for scan_type: {self.scan_type}"
                )
                hashCodeAllowsNullCwe = settings.HASHCODE_ALLOWS_NULL_CWE[
                    self.scan_type
                ]
            ...
            deduplicationLogger.debug(
                "Section HASHCODE_ALLOWS_NULL_CWE not found in settings.dist.py"
            )

        deduplicationLogger.debug(
            f"HASHCODE_ALLOWS_NULL_CWE is: {hashCodeAllowsNullCwe}"
        )
    ...
    def statistics(self):
        ...
        return _get_statistics_for_queryset(
            Finding.objects.filter(test=self), _get_annotations_for_statistics
        )
...
class Test_Import(TimeStampedModel):
    ...
    test = models.ForeignKey(
        Test, editable=False, null=False, blank=False, on_delete=models.CASCADE
    )
    findings_affected = models.ManyToManyField(
        "Finding", through="Test_Import_Finding_Action"
    )
    ...
    def get_queryset(self):
        ...
        super_query = super_query.annotate(
            created_findings_count=Count(
                "findings",
                filter=Q(test_import_finding_action__action=IMPORT_CREATED_FINDING),
            )
        )
        super_query = super_query.annotate(
            closed_findings_count=Count(
                "findings",
                filter=Q(test_import_finding_action__action=IMPORT_CLOSED_FINDING),
            )
        )
        super_query = super_query.annotate(
            reactivated_findings_count=Count(
                "findings",
                filter=Q(test_import_finding_action__action=IMPORT_REACTIVATED_FINDING),
            )
        )
        super_query = super_query.annotate(
            untouched_findings_count=Count(
                "findings",
                filter=Q(test_import_finding_action__action=IMPORT_UNTOUCHED_FINDING),
            )
        )
        return super_query
    ...
    def statistics(self):
        ...
        for action in IMPORT_ACTIONS:
            stats[action[1].lower()] = _get_statistics_for_queryset(
                Finding.objects.filter(
                    test_import_finding_action__test_import=self,
                    test_import_finding_action__action=action[0],
                ),
...
class Test_Import_Finding_Action(TimeStampedModel):
    test_import = models.ForeignKey(
        Test_Import, editable=False, null=False, blank=False, on_delete=models.CASCADE
    )
    finding = models.ForeignKey(
        "Finding", editable=False, null=False, blank=False, on_delete=models.CASCADE
    )
    action = models.CharField(
        max_length=100, null=True, blank=True, choices=IMPORT_ACTIONS
    )
...
class Finding(models.Model):
    ...
    cve = models.CharField(
        ...
        help_text=_(
            "An id of a vulnerability in a security advisory associated with this finding. Can be a Common Vulnerabilities and Exposures (CVE) or from other sources."
        ),
    )
    ...
    )  # not displayed and pretty much the same as references. To remove?
    ...
    severity_justification = models.TextField(
        null=True,
        blank=True,
    ...
    under_defect_review = models.BooleanField(
        default=False,
    ...
    is_mitigated = models.BooleanField(
        default=False,
        verbose_name=_("Is Mitigated"),
        help_text=_("Denotes if this flaw has been fixed."),
    )
    thread_id = models.IntegerField(
        default=0, editable=False, verbose_name=_("Thread ID")
    )
    mitigated = models.DateTimeField(
        editable=False,
        null=True,
        blank=True,
    ...
    dynamic_finding = models.BooleanField(
        default=True,
    ...
    vuln_id_from_tool = models.CharField(
        null=True,
        blank=True,
        max_length=500,
    ...
    sast_source_object = models.CharField(
        null=True,
        blank=True,
        max_length=500,
    ...
    sast_sink_object = models.CharField(
        null=True,
        blank=True,
        max_length=500,
    ...
    sast_source_line = models.IntegerField(
        null=True,
        blank=True,
        verbose_name=_("SAST Source Line number"),
        help_text=_("Source line number of the attack vector."),
    )
    sast_source_file_path = models.CharField(
        null=True,
        blank=True,
        max_length=4000,
    ...
    nb_occurences = models.IntegerField(
        null=True,
        blank=True,
    ...
    planned_remediation_date = models.DateField(
        null=True,
        editable=True,
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this finding. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
    ...
    def copy(self, test=None):
        ...
        for endpoint_status in old_status_findings:
            endpoint_status.copy(
                finding=copy
            )  # adding or setting is not necessary, link is created by Endpoint_Status.copy()
    ...
    def unaccepted_open_findings(cls):
        return cls.objects.filter(
            active=True, verified=True, duplicate=False, risk_accepted=False
        )
    ...
    def compute_hash_code(self):
        ...
        if (
            not hasattr(settings, "HASHCODE_FIELDS_PER_SCANNER")
            or not hasattr(settings, "HASHCODE_ALLOWS_NULL_CWE")
            or not hasattr(settings, "HASHCODE_ALLOWED_FIELDS")
        ):
            deduplicationLogger.debug(
                "no or incomplete configuration per hash_code found; using legacy algorithm"
            )
        ...
        if not hash_code_fields:
            deduplicationLogger.debug(
                "No configuration for hash_code computation found; using default fields for "
                + ("dynamic" if self.dynamic_finding else "static")
                + " scanners"
            )
            return self.compute_hash_code_legacy()
        ...
        if self.cwe == 0 and not self.test.hash_code_allows_null_cwe:
            deduplicationLogger.warn(
                "Cannot compute hash_code based on configured fields because cwe is 0 for finding of title '"
                + self.title
                + "' found in file '"
                + str(self.file_path)
        ...
        deduplicationLogger.debug(
            "computing hash_code for finding id "
            + str(self.id)
            + " based on: "
            + ", ".join(hash_code_fields)
        )
        ...
        for hashcodeField in hash_code_fields:
            if hashcodeField == "endpoints":
                ...
                deduplicationLogger.debug(
                    hashcodeField + " : " + str(getattr(self, hashcodeField))
                )
        deduplicationLogger.debug(
            "compute_hash_code - fields_to_hash = " + fields_to_hash
        )
    ...
    def compute_hash_code_legacy(self):
        ...
        deduplicationLogger.debug(
            "compute_hash_code_legacy - fields_to_hash = " + fields_to_hash
        )
        return self.hash_fields(fields_to_hash)
    ...
    def get_vulnerability_ids(self):
        ...
        if self.id is None:
            if self.unsaved_vulnerability_ids:
                deduplicationLogger.debug(
                    "get_vulnerability_ids before the finding was saved"
                )
                ...
                vulnerability_id_str = "".join(
                    sorted(list(dict.fromkeys(vulnerability_id_str_list)))
                )
            else:
                deduplicationLogger.debug(
                    "finding has no unsaved vulnerability references"
                )
        else:
            vulnerability_ids = Vulnerability_Id.objects.filter(finding=self)
            deduplicationLogger.debug(
                "get_vulnerability_ids after the finding was saved. Vulnerability references count: "
                + str(vulnerability_ids.count())
            )
        ...
        if self.id is None:
            if len(self.unsaved_endpoints) > 0:
                ...
                endpoint_str_list = list(
                    map(lambda endpoint: str(endpoint), self.unsaved_endpoints)
                )
                # deduplicate (usually done upon saving finding) and sort endpoints
                endpoint_str = "".join(sorted(list(dict.fromkeys(endpoint_str_list))))
            else:
                ...
                deduplicationLogger.debug(
                    "trying to get endpoints on a finding before it was saved but no endpoints found (static parser wrongly identified as dynamic?"
                )
        else:
            deduplicationLogger.debug(
                "get_endpoints: after the finding was saved. Endpoints count: "
                + str(self.endpoints.count())
            )
            # convert list of endpoints to the list of their canonical representation
            endpoint_str_list = list(
                map(lambda endpoint: str(endpoint), self.endpoints.all())
            )
            # sort endpoints strings
            endpoint_str = "".join(sorted(endpoint_str_list))
        return endpoint_str
    ...
    def hash_fields(self, fields_to_hash):
        ...
        return hashlib.sha256(
            fields_to_hash.casefold().encode("utf-8").strip()
        ).hexdigest()
    ...
    def duplicate_finding_set(self):
        if self.duplicate:
            if self.duplicate_finding is not None:
                originals = (
                    Finding.objects.get(id=self.duplicate_finding.id)
                    .original_finding.all()
                    .order_by("title")
                )
    ...
    def get_sla_periods(self):
        sla_configuration = SLA_Configuration.objects.filter(
            id=self.test.engagement.product.sla_configuration_id
        ).first()
    ...
    def github_conf(self):
        try:
            github_product_key = GITHUB_PKey.objects.get(
        ...
        **kwargs,
        ...
        if self.cvssv3:
            try:
                ...
                logger.error(
                    "Can't compute cvssv3 score for finding id %i. Invalid cvssv3 vector found: '%s'. Exception: %s",
                    self.id,
                    self.cvssv3,
                    ex,
                )
        ...
        if self.pk is None:
            ...
            finding_helper.update_finding_status(
                self, user, changed_fields={"id": (None, None)}
            )
        ...
        logger.debug(
            "Saving finding of id "
            + str(self.id)
            + " dedupe_option:"
            + str(dedupe_option)
            + " (self.pk is %s)",
            "None" if self.pk is None else "not None",
        )
        ...
        if (
            dedupe_option
            or false_history
            or issue_updater_option
            or product_grading_option
            or push_to_jira
        ):
            finding_helper.post_process_finding_save(
                self,
                dedupe_option=dedupe_option,
                false_history=false_history,
                rules_option=rules_option,
                product_grading_option=product_grading_option,
                issue_updater_option=issue_updater_option,
    ...
    def get_breadcrumbs(self):
        ...
        bc += [{"title": str(self), "url": reverse("view_finding", args=(self.id,))}]
        return bc
    ...
    def get_sast_source_file_path_with_link(self):
        ...
        link = (
            self.test.engagement.source_code_management_uri
            + "/"
    ...
    def get_references_with_links(self):
        ...
        for match in matches:
            ...
            if (
                not (match[0].startswith("[") or match[0].startswith("("))
                and not match[0] in processed_matches
            ):
                self.references = self.references.replace(
                    match[0], create_bleached_link(match[0], match[0]), 1
                )
...
class Stub_Finding(models.Model):
    ...
    reporter = models.ForeignKey(
        Dojo_User, editable=False, default=1, on_delete=models.RESTRICT
    )
    ...
    def get_breadcrumbs(self):
        ...
        bc += [
            {
                "title": "Potential Finding: " + str(self),
...
class Finding_Group(TimeStampedModel):
    ...
    def severity(self):
        if not self.findings.all():
            return None
        max_number_severity = max(
            [Finding.get_number_severity(find.severity) for find in self.findings.all()]
        )
    ...
    def components(self):
        ...
        for finding in self.findings.all():
            if finding.component_name is not None:
                components.setdefault(finding.component_name, set()).add(
                    finding.component_version
                )
        return "; ".join(
            f"""{name}: {", ".join(map(str, versions))}"""
    ...
    def sla_days_remaining_internal(self):
        ...
        return min(
            [
                find.sla_days_remaining()
                for find in self.findings.all()
                if find.sla_days_remaining()
            ],
            default=None,
        )
...
class Finding_Template(models.Model):
    ...
    numerical_severity = models.CharField(
        max_length=4, null=True, blank=True, editable=False
    )
    template_match = models.BooleanField(
        default=False,
        verbose_name=_("Template Match Enabled"),
        help_text=_(
    ...
    template_match_title = models.BooleanField(
        default=False,
        verbose_name=_("Match Template by Title and CWE"),
        help_text=_("Matches by title text (contains search) and CWE."),
    )
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this finding template. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
    ...
    def get_breadcrumbs(self):
        bc = [{"title": str(self), "url": reverse("view_template", args=(self.id,))}]
        return bc
...
class Vulnerability_Id_Template(models.Model):
    finding_template = models.ForeignKey(
        Finding_Template, editable=False, on_delete=models.CASCADE
    )
    vulnerability_id = models.TextField(max_length=50, blank=False, null=False)
...
class Check_List(models.Model):
    ...
    session_issues = models.ManyToManyField(
        Finding, related_name="session_issues", blank=True
    )
    encryption_crypto = models.CharField(max_length=50, default="none")
    crypto_issues = models.ManyToManyField(
        Finding, related_name="crypto_issues", blank=True
    )
    configuration_management = models.CharField(max_length=50, default="")
    config_issues = models.ManyToManyField(
        Finding, related_name="config_issues", blank=True
    )
    authentication = models.CharField(max_length=50, default="none")
    auth_issues = models.ManyToManyField(
        Finding, related_name="auth_issues", blank=True
    )
    authorization_and_access_control = models.CharField(max_length=50, default="none")
    author_issues = models.ManyToManyField(
        Finding, related_name="author_issues", blank=True
    )
    data_input_sanitization_validation = models.CharField(max_length=50, default="none")
    data_issues = models.ManyToManyField(
        Finding, related_name="data_issues", blank=True
    )
    sensitive_data = models.CharField(max_length=50, default="none")
    sensitive_issues = models.ManyToManyField(
        Finding, related_name="sensitive_issues", blank=True
    )
    other = models.CharField(max_length=50, default="none")
    other_issues = models.ManyToManyField(
        Finding, related_name="other_issues", blank=True
    )
...
class BurpRawRequestResponse(models.Model):
    finding = models.ForeignKey(
        Finding, blank=True, null=True, on_delete=models.CASCADE
    )
...
class Risk_Acceptance(models.Model):
    ...
    recommendation = models.CharField(
        ...
        max_length=2,
        null=False,
        default=TREATMENT_FIX,
        help_text=_("Recommendation from the security team."),
        verbose_name=_("Security Recommendation"),
    )

    recommendation_details = models.TextField(
        null=True,
        blank=True,
        help_text=_("Explanation of security recommendation"),
        verbose_name=_("Security Recommendation Details"),
    )
    ...
    decision = models.CharField(
        ...
        max_length=2,
        null=False,
        default=TREATMENT_ACCEPT,
        help_text=_("Risk treatment decision by risk owner"),
    )
    ...
    expiration_date_warned = models.DateTimeField(
        default=None,
        null=True,
        blank=True,
        help_text=_(
    ...
    expiration_date_handled = models.DateTimeField(
        default=None,
        null=True,
        blank=True,
        help_text=_(
    ...
    reactivate_expired = models.BooleanField(
        null=False,
        blank=False,
        default=True,
        verbose_name=_("Reactivate findings on expiration"),
        help_text=_("Reactivate findings when risk acceptance expires?"),
    )
    restart_sla_expired = models.BooleanField(
        default=False,
        null=False,
        verbose_name=_("Restart SLA on expiration"),
        help_text=_(
    ...
    def name_and_expiration_info(self):
        return (
            str(self.name)
            + (" (expired " if self.is_expired else " (expires ")
            + (
                timezone.localtime(self.expiration_date).strftime("%b %d, %Y")
    ...
    def copy(self, engagement=None):
        ...
        # Save the necessary ManyToMany relationships
        old_notes = list(self.notes.all())
        old_accepted_findings_hash_codes = [
            finding.hash_code for finding in self.accepted_findings.all()
        ]
        # Wipe the IDs of the new object
        copy.pk = None
        copy.id = None
        ...
        if engagement:
            new_accepted_findings = Finding.objects.filter(
                test__engagement=engagement,
                hash_code__in=old_accepted_findings_hash_codes,
                risk_accepted=True,
            ).distinct()
...
class FileAccessToken(models.Model):
    ...
    user = models.ForeignKey(
        Dojo_User, null=False, blank=False, on_delete=models.CASCADE
    )
    file = models.ForeignKey(
        FileUpload, null=False, blank=False, on_delete=models.CASCADE
    )
...
class BannerConf(models.Model):
    ...
    banner_message = models.CharField(
        max_length=500,
        help_text=_(
            "This message will be displayed on the login page. It can contain basic html tags, for example <a href='https://www.fred.com' style='color: #337ab7;' target='_blank'>https://example.com</a>"
        ),
        default="",
    )
...
class GITHUB_Conf(models.Model):
    ...
    api_key = models.CharField(
        max_length=2000, help_text=_("Enter your Github API Key"), default=""
    )
...
class GITHUB_Issue(models.Model):
    ...
    finding = models.OneToOneField(
        Finding, null=True, blank=True, on_delete=models.CASCADE
    )
...
class GITHUB_PKey(models.Model):
    ...
    git_project = models.CharField(
        max_length=200,
        blank=True,
        verbose_name=_("Github project"),
        help_text=_("Specify your project location. (:user/:repo)"),
    )
    git_conf = models.ForeignKey(
        GITHUB_Conf,
    ...
    git_push_notes = models.BooleanField(
        default=False,
        blank=True,
        help_text=_(
            "Notes added to findings will be automatically added to the corresponding github issue"
        ),
    )
...
class JIRA_Instance(models.Model):
    ...
    issue_template_dir = models.CharField(
        max_length=255,
        null=True,
        blank=True,
        help_text=_(
    ...
    open_status_key = models.IntegerField(
        verbose_name=_("Reopen Transition ID"),
        help_text=_(
    ...
    close_status_key = models.IntegerField(
        verbose_name=_("Close Transition ID"),
        help_text=_(
    ...
    info_mapping_severity = models.CharField(
        max_length=200,
        help_text=_("Maps to the 'Priority' field in Jira. For example: Info"),
    )
    low_mapping_severity = models.CharField(
        max_length=200,
        help_text=_("Maps to the 'Priority' field in Jira. For example: Low"),
    )
    medium_mapping_severity = models.CharField(
        max_length=200,
        help_text=_("Maps to the 'Priority' field in Jira. For example: Medium"),
    )
    high_mapping_severity = models.CharField(
        max_length=200,
        help_text=_("Maps to the 'Priority' field in Jira. For example: High"),
    )
    critical_mapping_severity = models.CharField(
        max_length=200,
        help_text=_("Maps to the 'Priority' field in Jira. For example: Critical"),
    )
    ...
    global_jira_sla_notification = models.BooleanField(
        default=True,
        blank=False,
        verbose_name=_("Globally send SLA notifications as comment?"),
        help_text=_("This setting can be overidden at the Product level"),
    )
    ...
    def false_positive_resolutions(self):
        return [
            m.strip() for m in (self.false_positive_mapping_resolution or "").split(",")
        ]
...
class JIRA_Project(models.Model):
    ...
    engagement = models.OneToOneField(
        Engagement, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    push_all_issues = models.BooleanField(
        default=False,
        blank=True,
        help_text=_(
    ...
    product_jira_sla_notification = models.BooleanField(
        default=False, blank=True, verbose_name=_("Send SLA notifications as comment?")
    )
    risk_acceptance_expiration_notification = models.BooleanField(
        default=False,
        blank=True,
        verbose_name=_("Send Risk Acceptance expiration notifications as comment?"),
    )

    def clean(self):
        if not self.jira_instance:
            raise ValidationError(
                "Cannot save JIRA Project Configuration without JIRA Instance"
            )

    def __str__(self):
        return ("%s: " + self.project_key + "(%s)") % (
            str(self.id),
            str(self.jira_instance.url) if self.jira_instance else "None",
        )
...
class JIRA_Issue(models.Model):
    ...
    engagement = models.OneToOneField(
        Engagement, null=True, blank=True, on_delete=models.CASCADE
    )
    finding_group = models.OneToOneField(
        Finding_Group, null=True, blank=True, on_delete=models.CASCADE
    )

    jira_creation = models.DateTimeField(
        editable=True,
        null=True,
        verbose_name=_("Jira creation"),
        help_text=_("The date a Jira issue was created from this finding."),
    )
    jira_change = models.DateTimeField(
        editable=True,
        null=True,
    ...
    def set_obj(self, obj):
        if type(obj) == Finding:
            ...
            raise ValueError(
                "unknown objec type whiel creating JIRA_Issue: %s" % to_str_typed(obj)
            )
    ...
    def __str__(self):
        ...
        if self.finding:
            text = (
                self.finding.test.engagement.product.name
                + " | Finding: "
                + self.finding.title
                + ", ID: "
                + str(self.finding.id)
            ...
            text = (
                self.engagement.product.name
                + " | Engagement: "
                + self.engagement.name
                + ", ID: "
                + str(self.engagement.id)
            )
...
class Notifications(models.Model):
    product_type_added = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    product_added = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    engagement_added = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    test_added = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )

    scan_added = MultiSelectField(
    ...
    jira_update = MultiSelectField(
        choices=NOTIFICATION_CHOICES,
        default=DEFAULT_NOTIFICATION,
        blank=True,
        verbose_name=_("JIRA problems"),
        help_text=_(
            "JIRA sync happens in the background, errors will be shown as notifications/alerts so make sure to subscribe"
        ),
    )
    upcoming_engagement = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    stale_engagement = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    auto_close_engagement = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    close_engagement = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    user_mentioned = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    code_review = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    review_requested = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    other = MultiSelectField(
        choices=NOTIFICATION_CHOICES, default=DEFAULT_NOTIFICATION, blank=True
    )
    user = models.ForeignKey(
        Dojo_User, default=None, null=True, editable=False, on_delete=models.CASCADE
    )
    product = models.ForeignKey(
        Product, default=None, null=True, editable=False, on_delete=models.CASCADE
    )
    template = models.BooleanField(default=False)
    sla_breach = MultiSelectField(
        choices=NOTIFICATION_CHOICES,
        default=DEFAULT_NOTIFICATION,
        blank=True,
    ...
    risk_acceptance_expiration = MultiSelectField(
        choices=NOTIFICATION_CHOICES,
        default="alert",
        blank=True,
    ...
    def merge_notifications_list(cls, notifications_list):
        ...
        for notifications in notifications_list:
            if result is None:
                ...
                result.product_type_added = merge_sets_safe(
                    result.product_type_added, notifications.product_type_added
                )
                result.product_added = merge_sets_safe(
                    result.product_added, notifications.product_added
                )
                result.engagement_added = merge_sets_safe(
                    result.engagement_added, notifications.engagement_added
                )
                result.test_added = merge_sets_safe(
                    result.test_added, notifications.test_added
                )
                result.scan_added = merge_sets_safe(
                    result.scan_added, notifications.scan_added
                )
                result.jira_update = merge_sets_safe(
                    result.jira_update, notifications.jira_update
                )
                result.upcoming_engagement = merge_sets_safe(
                    result.upcoming_engagement, notifications.upcoming_engagement
                )
                result.stale_engagement = merge_sets_safe(
                    result.stale_engagement, notifications.stale_engagement
                )
                result.auto_close_engagement = merge_sets_safe(
                    result.auto_close_engagement, notifications.auto_close_engagement
                )
                result.close_engagement = merge_sets_safe(
                    result.close_engagement, notifications.close_engagement
                )
                result.user_mentioned = merge_sets_safe(
                    result.user_mentioned, notifications.user_mentioned
                )
                result.code_review = merge_sets_safe(
                    result.code_review, notifications.code_review
                )
                result.review_requested = merge_sets_safe(
                    result.review_requested, notifications.review_requested
                )
                result.other = merge_sets_safe(result.other, notifications.other)
                result.sla_breach = merge_sets_safe(
                    result.sla_breach, notifications.sla_breach
                )
                result.risk_acceptance_expiration = merge_sets_safe(
                    result.risk_acceptance_expiration,
                    notifications.risk_acceptance_expiration,
                )
...
class Tool_Product_Settings(models.Model):
    ...
    url = models.CharField(max_length=2000, null=True, blank=True)
    product = models.ForeignKey(
        Product, default=1, editable=False, on_delete=models.CASCADE
    )
    tool_configuration = models.ForeignKey(
        Tool_Configuration,
        null=False,
...
class Tool_Product_History(models.Model):
    product = models.ForeignKey(
        Tool_Product_Settings, editable=False, on_delete=models.CASCADE
    )
...
class Alerts(models.Model):
    ...
    user_id = models.ForeignKey(
        Dojo_User, null=True, editable=False, on_delete=models.CASCADE
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
...
class Cred_User(models.Model):
    ...
    environment = models.ForeignKey(
        Development_Environment, null=False, on_delete=models.RESTRICT
    )
...
class Cred_Mapping(models.Model):
    ...
    product = models.ForeignKey(
        Product, null=True, blank=True, related_name="product", on_delete=models.CASCADE
    )
    finding = models.ForeignKey(
        Finding, null=True, blank=True, related_name="finding", on_delete=models.CASCADE
    )
    engagement = models.ForeignKey(
        Engagement,
        null=True,
    ...
    test = models.ForeignKey(
        Test, null=True, blank=True, related_name="test", on_delete=models.CASCADE
    )
    is_authn_provider = models.BooleanField(
        default=False, verbose_name=_("Authentication Provider")
    )
    url = models.URLField(max_length=2000, null=True, blank=True)
...
class Language_Type(models.Model):
    ...
    color = models.CharField(
        max_length=7, null=True, blank=True, verbose_name=_("HTML color")
    )
...
class Languages(models.Model):
    ...
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    user = models.ForeignKey(
        Dojo_User, editable=True, blank=True, null=True, on_delete=models.RESTRICT
    )
    files = models.IntegerField(
        blank=True, null=True, verbose_name=_("Number of files")
    )
    blank = models.IntegerField(
        blank=True, null=True, verbose_name=_("Number of blank lines")
    )
    comment = models.IntegerField(
        blank=True, null=True, verbose_name=_("Number of comment lines")
    )
    code = models.IntegerField(
        blank=True, null=True, verbose_name=_("Number of code lines")
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
...
class App_Analysis(models.Model):
    ...
    confidence = models.IntegerField(
        blank=True, null=True, verbose_name=_("Confidence level")
    )
    version = models.CharField(
        max_length=200, null=True, blank=True, verbose_name=_("Version Number")
    )
...
class Objects_Product(models.Model):
    ...
    name = models.CharField(max_length=100, null=True, blank=True)
    path = models.CharField(
        max_length=600, verbose_name=_("Full file path"), null=True, blank=True
    )
    folder = models.CharField(
        max_length=400, verbose_name=_("Folder"), null=True, blank=True
    )
    artifact = models.CharField(
        max_length=400, verbose_name=_("Artifact"), null=True, blank=True
    )
    ...
    tags = TagField(
        ...
        force_lowercase=True,
        help_text=_(
            "Add tags that help describe this object. Choose from the list or add new tags. Press Enter key to add."
        ),
    )
...
class Testing_Guide(models.Model):
    testing_guide_category = models.ForeignKey(
        Testing_Guide_Category, on_delete=models.CASCADE
    )
    identifier = models.CharField(
        max_length=20, blank=True, null=True, help_text=_("Test Unique Identifier")
    )
    ...
    how_to_test = models.TextField(
        default=None, help_text=_("How to test the objective")
    )
    results_expected = models.CharField(
        max_length=800, help_text=_("What the results look like for a test")
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
    updated = models.DateTimeField(auto_now=True)
...
class Benchmark_Type(models.Model):
    ...
    benchmark_source = models.CharField(
        max_length=20, blank=False, null=True, choices=source, default="OWASP ASVS"
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
    updated = models.DateTimeField(auto_now=True)
...
class Benchmark_Category(models.Model):
    type = models.ForeignKey(
        Benchmark_Type, verbose_name=_("Benchmark Type"), on_delete=models.CASCADE
    )
    name = models.CharField(max_length=300)
...
class Benchmark_Product(models.Model):
    ...
    enabled = models.BooleanField(
        default=True, help_text=_("Applicable for this specific product.")
    )
    notes = models.ManyToManyField(Notes, blank=True, editable=False)
    created = models.DateTimeField(auto_now_add=True, null=False)
    updated = models.DateTimeField(auto_now=True)
...
class Benchmark_Product_Summary(models.Model):
    ...
    desired_level = models.CharField(
        max_length=15, null=False, choices=asvs_level, default="Level 1"
    )
    current_level = models.CharField(
        max_length=15, blank=True, null=True, choices=asvs_level, default="None"
    )
    asvs_level_1_benchmark = models.IntegerField(
        null=False,
        default=0,
        help_text=_("Total number of active benchmarks for this application."),
    )
    asvs_level_1_score = models.IntegerField(
        null=False, default=0, help_text=_("ASVS Level 1 Score")
    )
    ...
    asvs_level_2_score = models.IntegerField(
        null=False, default=0, help_text=_("ASVS Level 2 Score")
    )
    ...
    asvs_level_3_score = models.IntegerField(
        null=False, default=0, help_text=_("ASVS Level 3 Score")
    )
    publish = models.BooleanField(
        default=False, help_text=_("Publish score to Product.")
    )
    created = models.DateTimeField(auto_now_add=True, null=False)
    updated = models.DateTimeField(auto_now=True)
...
finding_opts = [
    f.name for f in Finding._meta.fields if f.name not in ["last_status_update"]
]
...
class Child_Rule(models.Model):
    ...
    # and_rules = models.ManyToManyField('self')
    parent_rule = models.ForeignKey(
        Rule, editable=False, null=True, on_delete=models.CASCADE
    )
...
class Question(PolymorphicModel, TimeStampedModel):
    ...
    order = models.PositiveIntegerField(default=1, help_text=_("The render order"))
...
class ChoiceQuestion(Question):
    ...
    multichoice = models.BooleanField(default=False, help_text=_("Select one or more"))
...
class Engagement_Survey(models.Model):
    name = models.CharField(
        max_length=200, null=False, blank=False, editable=True, default=""
    )
...
class Answer(PolymorphicModel, TimeStampedModel):
    ...
    answered_survey = models.ForeignKey(
        Answered_Survey, null=False, blank=False, on_delete=models.CASCADE
    )


class TextAnswer(Answer):
    answer = models.TextField(blank=False, help_text=_("The answer text"), default="")
...
class ChoiceAnswer(Answer):
    answer = models.ManyToManyField(
        Choice, help_text=_("The selected choices as the answer")
    )
