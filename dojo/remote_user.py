...
from django.contrib.auth.middleware import (
    RemoteUserMiddleware as OriginalRemoteUserMiddleware,
)
from django.contrib.auth.backends import RemoteUserBackend as OriginalRemoteUserBackend
from drf_spectacular.extensions import OpenApiAuthenticationExtension
from rest_framework.authentication import (
    RemoteUserAuthentication as OriginalRemoteUserAuthentication,
)
...
class RemoteUserAuthentication(OriginalRemoteUserAuthentication):
    def authenticate(self, request):
        ...
        if (
            ...
            in settings.AUTH_REMOTEUSER_TRUSTED_PROXY
        ):
            ...
            logger.debug(
                "Requested came from untrusted proxy %s; This is list of trusted proxies: %s",
                IPAddress(request.META["REMOTE_ADDR"]),
...
class RemoteUserBackend(OriginalRemoteUserBackend):
    ...
    def configure_user(self, request, user, created=True):
        ...
        if (
            settings.AUTH_REMOTEUSER_EMAIL_HEADER
            and settings.AUTH_REMOTEUSER_EMAIL_HEADER in request.META
            and user.email != request.META[settings.AUTH_REMOTEUSER_EMAIL_HEADER]
        ):
            user.email = request.META[settings.AUTH_REMOTEUSER_EMAIL_HEADER]
            logger.debug(
                "Updating email for user %s to value %s", user.username, user.email
            )
            changed = True

        if (
            settings.AUTH_REMOTEUSER_FIRSTNAME_HEADER
            and settings.AUTH_REMOTEUSER_FIRSTNAME_HEADER in request.META
            and user.first_name
            != request.META[settings.AUTH_REMOTEUSER_FIRSTNAME_HEADER]
        ):
            user.first_name = request.META[settings.AUTH_REMOTEUSER_FIRSTNAME_HEADER]
            logger.debug(
                "Updating first_name for user %s to value %s",
                user.username,
                user.first_name,
            )
            changed = True
        ...
        if (
            settings.AUTH_REMOTEUSER_LASTNAME_HEADER
            and settings.AUTH_REMOTEUSER_LASTNAME_HEADER in request.META
            and user.last_name != request.META[settings.AUTH_REMOTEUSER_LASTNAME_HEADER]
        ):
            user.last_name = request.META[settings.AUTH_REMOTEUSER_LASTNAME_HEADER]
            logger.debug(
                "Updating last_name for user %s to value %s",
                user.username,
                user.last_name,
            )
            changed = True
        ...
        if (
            settings.AUTH_REMOTEUSER_GROUPS_HEADER
            and settings.AUTH_REMOTEUSER_GROUPS_HEADER in request.META
        ):
        ...
        if (
            settings.AUTH_REMOTEUSER_GROUPS_CLEANUP
            and settings.AUTH_REMOTEUSER_GROUPS_HEADER
            and settings.AUTH_REMOTEUSER_GROUPS_HEADER in request.META
        ):
            cleanup_old_groups_for_user(
                user, request.META[settings.AUTH_REMOTEUSER_GROUPS_HEADER].split(",")
            )
