...
def endpoint_filter(**kwargs):
    ...
    if kwargs.get("port"):
        if (
            (kwargs.get("protocol"))
            and (kwargs["protocol"].lower() in SCHEME_PORT_MAP)
            and (SCHEME_PORT_MAP[kwargs["protocol"].lower()] == kwargs["port"])
        ):
            qs = qs.filter(
                Q(port__isnull=True)
...
def clean_hosts_run(apps, change):
    def err_log(message, html_log, endpoint_html_log, endpoint):
        ...
        logger.error(
            "Endpoint (id={}) {}. {}".format(endpoint.pk, message, error_suffix)
        )
    ...
    for endpoint in Endpoint_model.objects.order_by("id"):
        ...
        if endpoint.host:
            if not re.match(
                r"^[A-Za-z][A-Za-z0-9\.\-\+]+$", endpoint.host
            ):  # is old host valid FQDN?
                try:
                    ...
                    try:
                        ...
                        if parts.protocol:
                            if endpoint.protocol and (
                                endpoint.protocol != parts.protocol
                            ):
                                message = (
                                    "has defined protocol ({}) and it is not the same as protocol in host "
                        ...
                        if parts.port:
                            try:
                                if (endpoint.port is not None) and (
                                    int(endpoint.port) != parts.port
                                ):
                                    message = (
                                        "has defined port number ({}) and it is not the same as port number in "
                                        "host ({})".format(endpoint.port, parts.port)
                                    )
                                    err_log(
                                        message, html_log, endpoint_html_log, endpoint
                                    )
                                else:
                                    if change:
                        ...
                        if parts.path:
                            if endpoint.path and (endpoint.path != parts.path):
                                message = (
                                    "has defined path ({}) and it is not the same as path in host "
                        ...
                        if parts.query:
                            if endpoint.query and (endpoint.query != parts.query):
                                message = (
                                    "has defined query ({}) and it is not the same as query in host "
                        ...
                        if parts.fragment:
                            if endpoint.fragment and (
                                endpoint.fragment != parts.fragment
                            ):
                                message = (
                                    "has defined fragment ({}) and it is not the same as fragment in host "
                        ...
                        if change and (
                            endpoint.pk not in broken_endpoints
            ...
            if endpoint.id not in to_be_deleted:
                ...
                if ep.count() > 1:
                    ...
                    if change:
                        ...
                        logger.info(message)
                        Endpoint_Status_model.objects.filter(
                            endpoint__in=ep_ids[1:]
                        ).update(endpoint=ep_ids[0])
                        epss = (
                            Endpoint_Status_model.objects.filter(endpoint=ep_ids[0])
                            .values("finding")
                            .annotate(total=Count("id"))
                            .filter(total__gt=1)
                        )
                        for eps in epss:
                            esm = Endpoint_Status_model.objects.filter(
                                finding=eps["finding"]
                            ).order_by("-last_modified")
                            message = (
                                "Endpoint Statuses {} will be replaced by '{}'".format(
                                    [
                                        "last_modified: {} (id={})".format(
                                            x.last_modified, x.pk
                                        )
                                        for x in esm[1:]
                                    ],
                                    "last_modified: {} (id={})".format(
    ...
    for endpoint in endpoints:
        try:
            if "://" in endpoint:  # is it full uri?
                endpoint_ins = Endpoint.from_uri(
                    endpoint
                )  # from_uri validate URI format + split to components
            else:
...
def endpoint_meta_import(
    file, product, create_endpoints, create_tags, create_meta, origin="UI", request=None
):
    content = file.read()
