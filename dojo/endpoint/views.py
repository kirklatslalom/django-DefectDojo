...
def process_endpoints_view(request, host_view=False, vulnerable=False):
    ...
    endpoints = get_authorized_endpoints(
        Permissions.Endpoint_View, endpoints, request.user
    )
    ...
    if host_view:
        ...
        endpoints = EndpointFilter(
            request.GET, queryset=endpoints.filter(id__in=ids), user=request.user
        )
    else:
...
def process_endpoint_view(request, eid, host_view=False):
    ...
    product_tab = Product_Tab(
        endpoint.product, "Host" if host_view else "Endpoint", tab="endpoints"
    )
    return render(
        request,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user_has_permission_or_403(
                request.user, form.product, Permissions.Endpoint_Add
            )
            endpoints = form.save()
            tags = request.POST.get("tags")
            for e in endpoints:
...
def add_meta_data(request, eid):
    ...
    add_breadcrumb(
        parent=endpoint, title="Add Metadata", top_level=False, request=request
    )
...
def endpoint_bulk_update_all(request, pid=None):
    if request.method == "POST":
        ...
        endpoints = Endpoint.objects.filter(id__in=endpoints_to_update).order_by(
        ...
        if request.POST.get("delete_bulk_endpoints") and endpoints_to_update:
            ...
            if pid is not None:
                product = get_object_or_404(Product, id=pid)
                user_has_permission_or_403(
                    request.user, product, Permissions.Endpoint_Delete
                )

            endpoints = get_authorized_endpoints(
                Permissions.Endpoint_Delete, endpoints, request.user
            )
            ...
            product_calc = list(
                Product.objects.filter(endpoint__id__in=endpoints_to_update).distinct()
            )
            endpoints.delete()
            ...
            if skipped_endpoint_count > 0:
                add_error_message_to_response(
                    "Skipped deletion of {} endpoints because you are not authorized.".format(
            ...
            if endpoints_to_update:
                ...
                endpoints = get_authorized_endpoints(
                    Permissions.Endpoint_Edit, endpoints, request.user
                )
                ...
                if skipped_endpoint_count > 0:
                    add_error_message_to_response(
                        "Skipped mitigation of {} endpoints because you are not authorized.".format(
                            skipped_endpoint_count
                        )
                    )

                eps_count = Endpoint_Status.objects.filter(
        ...
        if endpoints_to_update and len(enable) > 0:
            ...
            for endpoint in endpoints:
                endpoint_status = Endpoint_Status.objects.get(
                    endpoint=endpoint, finding__id=fid
                )
...
def prefetch_for_endpoints(endpoints):
    if isinstance(endpoints, QuerySet):
        ...
        endpoints = endpoints.annotate(
            active_finding_count=Count("finding__id", filter=Q(finding__active=True))
        )
    else:
...
def migrate_endpoints_view(request):
    ...
    return render(
        request,
        "dojo/migrate_endpoints.html",
        {"product_tab": None, "name": view_name, "html_log": html_log},
    )
