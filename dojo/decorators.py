...
def we_want_async(*args, func=None, **kwargs):
    ...
    if sync:
        logger.debug(
            "dojo_async_task %s: running task in the foreground as sync=True has been found as kwarg",
            func,
        )
        return False
    ...
    if Dojo_User.wants_block_execution(user):
        logger.debug(
            "dojo_async_task %s: running task in the foreground as block_execution is set to True for %s",
            func,
            user,
        )
        return False
    ...
    logger.debug(
        "dojo_async_task %s: no current user, running task in the background", func
    )
    return True
