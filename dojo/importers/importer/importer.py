...
class DojoDefaultImporter(object):
    def create_test(
        ...
        if created:
            logger.info(
                "Created new Test_Type with name %s because a report is being imported",
                test_type.name,
            )
        ...
        for (group_name, findings) in group_names_to_findings_dict.items():
            ...
            if create_finding_groups_for_all_findings or len(findings) > 1:
                for finding in findings:
                    finding_helper.add_finding_to_auto_group(
                        finding, group_by, **kwargs
                    )
            if push_to_jira:
    ...
    def close_old_findings(
        ...
        if service:
            ...
            old_findings = old_findings.filter(
                Q(service__isnull=True) | Q(service__exact="")
            )
    ...
    def import_scan(
        ...
        if (
            ...
            and api_scan_configuration.product != engagement.product
        ):
