...
def match_new_finding_to_existing_finding(new_finding, test, deduplication_algorithm):
    ...
    deduplicationLogger.debug(
        "return findings bases on algorithm: %s", deduplication_algorithm
    )
    if deduplication_algorithm == "hash_code":
        return (
            Finding.objects.filter(test=test, hash_code=new_finding.hash_code)
        ...
        return (
            Finding.objects.filter(
                test=test, unique_id_from_tool=new_finding.unique_id_from_tool
            )
        ...
        logger.error(
            "Internal error: unexpected deduplication_algorithm: '%s' ",
...
def update_endpoint_status(existing_finding, new_finding, user):
    ...
    if new_finding.is_mitigated:
        ...
        endpoint_status_to_mitigate = list(
            filter(
                lambda existing_finding_endpoint_status: existing_finding_endpoint_status.endpoint
                not in new_finding_endpoints_list,
    ...
    if settings.ASYNC_FINDING_IMPORT:
        ...
        if len(chunk_list) < 2:
            mitigate_endpoint_status(
                endpoint_status_to_mitigate, user, kwuser=user, sync=True
            )
            return
        # First kick off all the workers
        for endpoint_status_list in chunk_list:
            mitigate_endpoint_status(
                endpoint_status_list, user, kwuser=user, sync=False
            )
    else:
        mitigate_endpoint_status(
            endpoint_status_to_mitigate, user, kwuser=user, sync=True
...
def mitigate_endpoint_status(endpoint_status_list, user, **kwargs):
    for endpoint_status in endpoint_status_list:
        logger.debug(
            "Re-import: mitigating endpoint %s that is no longer present",
            str(endpoint_status.endpoint),
        )
...
def reactivate_endpoint_status(endpoint_status_list, **kwargs):
    for endpoint_status in endpoint_status_list:
        logger.debug(
            "Re-import: reactivating endpoint %s that is present in this scan",
            str(endpoint_status.endpoint),
        )
...
def get_target_engagement_if_exists(
    engagement_id=None, engagement_name=None, product=None
):
    if engagement_id:
    ...
    engagement = get_last_object_or_none(
        Engagement, product=product, name=engagement_name
    )
    return engagement


def get_target_test_if_exists(
    test_id=None, test_title=None, scan_type=None, engagement=None
):
    """
    ...
    if test_title:
        return get_last_object_or_none(
            Test, engagement=engagement, title=test_title, scan_type=scan_type
        )
...
def get_or_create_product(
    product_name=None, product_type_name=None, auto_create_context=None
):
    ...
    if not auto_create_context:
        raise ValueError(
            "auto_create_context not True, unable to create non-existing product"
        )
    else:
...
def get_or_create_engagement(
    ...
    if not auto_create_context:
        raise ValueError(
            "auto_create_context not True, unable to create non-existing engagement"
        )
    else:
        product = get_or_create_product(
            product_name, product_type_name, auto_create_context
        )
