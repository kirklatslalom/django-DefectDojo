...
def update_timestamps(test, version, branch_tag, build_id, commit_hash, now, scan_date):
    ...
    if test.engagement.engagement_type == "CI/CD":
        test.engagement.target_end = max_safe(
            [scan_date.date(), test.engagement.target_end]
        )
...
def update_import_history(
    ...
    logger.debug(
        "new: %d closed: %d reactivated: %d untouched: %d",
        len(new_findings),
        len(closed_findings),
    ...
    for finding in closed_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for closed finding: %i", finding.id
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
                test_import=test_import, finding=finding, action=IMPORT_CLOSED_FINDING
            )
        )
    for finding in new_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for created finding: %i", finding.id
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
                test_import=test_import, finding=finding, action=IMPORT_CREATED_FINDING
            )
        )
    for finding in reactivated_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for reactivated finding: %i",
            finding.id,
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
    ...
    for finding in untouched_findings:
        logger.debug(
            "preparing Test_Import_Finding_Action for untouched finding: %i", finding.id
        )
        test_import_finding_action_list.append(
            Test_Import_Finding_Action(
                test_import=test_import,
...
def add_endpoints_to_unsaved_finding(finding, test, endpoints, **kwargs):
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            raise Exception(
                "Endpoints in your database are broken. Please access {} and migrate them to new format or "
...
def handle_vulnerability_ids(finding):
    ...
    if finding.unsaved_vulnerability_ids:
        ...
        finding.unsaved_vulnerability_ids = list(
            dict.fromkeys(finding.unsaved_vulnerability_ids)
        )
