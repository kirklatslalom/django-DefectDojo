...
class CheckmarxParser(object):
        ...
        for query in root.findall("Query"):
            ...
            if self.mode != "detailed":
                for key in list(dupes):
                    ...
                    dupes[key].vuln_id_from_tool = ",".join(vuln_ids_from_tool[key])[
                        :500
                    ]
    ...
    def _process_result_file_name_aggregated(
        self, test, dupes, vuln_ids_from_tool, findingdetail, query, result, find_date
    ):
        ...
        if not (aggregateKeys in dupes):
            ...
            find.description = "{}\n\n-----\n### {}. {}\n{}\n{}".format(
                find.description, find.nb_occurences, title, findingdetail, description
            )
