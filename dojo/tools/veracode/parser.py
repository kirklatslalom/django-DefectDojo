...
class VeracodeParser(object):
    ...
    def get_findings(self, filename, test):
        ...
        for component in root.findall(
            "x:software_composition_analysis/x:vulnerable_components" "/x:component",
            namespaces=XML_NAMESPACE,
        ):
            ...
            for vulnerability in component.findall(
                ...
                dupes[str(uuid.uuid4())] = self.__xml_sca_flaw_to_finding(
                    test, report_date, _vendor, _library, _version, vulnerability
                )
    ...
    def __xml_flaw_to_finding(cls, app_id, xml_node, mitigation_text, test):
        ...
        if "date_first_occurrence" in xml_node.attrib:
            _date_found = datetime.strptime(
                xml_node.attrib["date_first_occurrence"], "%Y-%m-%d %H:%M:%S %Z"
            )
