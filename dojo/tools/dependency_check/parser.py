...
class DependencyCheckParser(object):
    ...
    def add_finding(self, finding, dupes):
        key_str = "|".join(
            [str(finding.title), str(finding.cwe), str(finding.file_path).lower()]
        )
    ...
    def get_filename_and_path_from_dependency(
        ...
        if not related_dependency:
            return dependency.findtext(f"{namespace}fileName"), dependency.findtext(
                f"{namespace}filePath"
            )
    ...
    def get_finding_from_vulnerability(
        ...
        if not severity:
            if cvssv3_node is not None:
                ...
                severity = (
                    cvssv2_node.findtext(f"{namespace}severity").lower().capitalize()
                )
        ...
        if vulnerability.tag == "{}suppressedVulnerability".format(namespace):
            if notes is None:
                notes = (
                    "Document on why we are suppressing this vulnerability is missing!"
                )
                    ...
                    for vulnerability in vulnerabilities.findall(
                        ...
                        if vulnerability:
                            finding = self.get_finding_from_vulnerability(
                                dependency, None, vulnerability, test, namespace
                            )
                            if scan_date:
