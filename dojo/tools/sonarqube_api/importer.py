...
class SonarQubeApiImporter(object):
    ...
    def prepare_client(test):
        ...
        if test.api_scan_configuration:
            ...
            sqqs = product.product_api_scan_configuration_set.filter(
                product=product, tool_configuration__tool_type__name="SonarQube"
            )
            if (
                sqqs.count() == 1
        ...
        return (
            SonarQubeAPI(tool_config=config.tool_configuration if config else None),
            config,
        )
    ...
    def import_issues(self, test):
        ...
        try:
            ...
            organization = (
                config.service_key_2 if (config and config.service_key_2) else None
            )
            ...
            issues = client.find_issues(
                component["key"], organization=organization, branch=test.branch_tag
            )
        ...
        try:
            ...
            for hotspot in hotspots:
                ...
                description = self.clean_rule_description_html(
                    rule.get("vulnerabilityDescription", "No description provided.")
                )
                ...
                sonarqube_issue, _ = Sonarqube_Issue.objects.update_or_create(
                    key=hotspot["key"], defaults={"status": status, "type": issue_type}
                )
