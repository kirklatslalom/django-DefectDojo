...
class SonarQubeAPI:
    def __init__(self, tool_config=None):
        ...
        if (
            not tool_config
        ):  # https://github.com/DefectDojo/django-DefectDojo/pull/4676 cases no. 1-3
            try:
    ...
    def find_project(self, project_name, organization=None, branch=None):
        ...
        if not response.ok:
            raise Exception(
                f'Unable to find the project {project_name} due to {response.status_code} - {response.content.decode("utf-8")}'
            )
    ...
    def get_project(self, project_key, organization=None, branch=None):
        ...
        if not response.ok:
            raise Exception(
                f'Unable to find the project {project_key} due to {response.status_code} - {response.content.decode("utf-8")}'
            )

        return response.json().get("component")

    def find_issues(
        self, component_key, types="VULNERABILITY", organization=None, branch=None
    ):
        """
        Search for issues.
        ...
        while page <= max_page:
            request_filter = {"componentKeys": component_key, "types": types, "p": page}
            ...
            if not response.ok:
                raise Exception(
                    f'Unable to find the issues for component {component_key} due to {response.status_code} - {response.content.decode("utf-8")}'
                )
    ...
    def find_hotspots(self, project_key, organization=None, branch=None):
        ...
        while page <= max_page:
            ...
            if not response.ok:
                raise Exception(
                    f"Unable to find the hotspots for project {project_key} due to {response.status_code} - {response.content}"
                )
    ...
    def get_issue(self, issue_key):
        ...
        :return:
        """
        request_filter = {"issues": issue_key, "types": "BUG,VULNERABILITY,CODE_SMELL"}
        ...
        if not response.ok:
            raise Exception(
                f'Unable to get issue {issue_key} due to {response.status_code} - {response.content.decode("utf-8")}'
            )
        ...
        for issue in issues:
            if issue["key"] == issue_key:
                return issue
        raise Exception(
            f"""Expected Issue "{issue_key}", but it returned {[x.get('key') for x in response.json().get('issues')]}."""
        )
    ...
    def get_rule(self, rule_id):
        ...
        if not rule:
            ...
            if not response.ok:
                raise Exception(
                    f'Unable to get the rule {rule_id} due to {response.status_code} - {response.content.decode("utf-8")}'
                )
    ...
    def get_hotspot_rule(self, rule_id):
        ...
        if not rule:
            ...
            if not response.ok:
                raise Exception(
                    f"Unable to get the hotspot rule {rule_id} due to {response.status_code} - {response.content}"
                )
    ...
    def transition_issue(self, issue_key, transition):
        ...
        if not response.ok:
            raise Exception(
                f'Unable to transition {transition} the issue {issue_key} due to {response.status_code} - {response.content.decode("utf-8")}'
            )
    ...
    def add_comment(self, issue_key, text):
        ...
        if not response.ok:
            raise Exception(
                f'Unable to add a comment into issue {issue_key} due to {response.status_code} - {response.content.decode("utf-8")}'
            )
    ...
    def test_connection(self):
        ...
        if not response.ok:
            raise Exception(
                f'Unable to connect and search in SonarQube due to {response.status_code} - {response.content.decode("utf-8")}'
            )
