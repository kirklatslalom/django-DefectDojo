...
def jira_status_reconciliation(*args, **kwargs):
    ...
    logger.debug(
        "mode: %s product:%s engagement: %s dryrun: %s",
        mode,
        product,
        engagement,
        dryrun,
    ...
    if mode and mode not in (
        ...
        print(
            "mode must be one of reconcile, push_status_to_jira or import_status_from_jira"
        )
        return False
    ...
    findings = findings.order_by(
        "test__engagement__product__id", "test__engagement__id"
    )
    ...
    messages = [
        "jira_key;finding_url;resolution_or_status;find.jira_issue.jira_change;issue_from_jira.fields.updated;find.last_status_update;issue_from_jira.fields.updated;find.last_reviewed;issue_from_jira.fields.updated;flag1;flag2;flag3;action;change_made"
    ]
    for find in findings:
        ...
        if not issue_from_jira:
            message = (
                "%s;%s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;unable to retrieve JIRA Issue;%s"
                % (
                    find.jira_issue.jira_key,
                    settings.SITE_URL,
                    find.id,
                    find.status(),
                    None,
                    ...
                    None,
                    None,
                    find.jira_issue.jira_change,
                    None,
                    find.last_status_update,
                    None,
                    find.last_reviewed,
                    None,
                    "error",
        ...
        assignee = (
            issue_from_jira.fields.assignee
            if hasattr(issue_from_jira.fields, "assignee")
            else None
        )
        assignee_name = assignee.displayName if assignee else None
        resolution = (
            issue_from_jira.fields.resolution
            if issue_from_jira.fields.resolution
            and issue_from_jira.fields.resolution != "None"
            else None
        )
        ...
        if mode == "reconcile" and not find.last_status_update:
            message = (
                "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;skipping finding with no last_status_update;%s"
                % (
                    find.jira_issue.jira_key,
                    settings.SITE_URL,
                    find.id,
                    ...
                    None,
                    find.jira_issue.jira_change,
                    issue_from_jira.fields.updated,
                    find.last_status_update,
                    issue_from_jira.fields.updated,
                    find.last_reviewed,
                    issue_from_jira.fields.updated,
            ...
            message = (
                "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%sskipping risk accepted findings;%s"
                % (
                    find.jira_issue.jira_key,
                    settings.SITE_URL,
                    find.id,
            ...
            message = (
                "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;no action both sides are active/open;%s"
                % (
                    find.jira_issue.jira_key,
                    settings.SITE_URL,
                    find.id,
            ...
            not jira_helper.issue_from_jira_is_active(issue_from_jira)
            and not find.active
        ):
            message = (
                "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;no action both sides are inactive/closed;%s"
                % (
                    find.jira_issue.jira_key,
                    settings.SITE_URL,
                    find.id,
            ...
            if mode in ("push_status_to_jira", "import_status_from_jira"):
                ...
                flag1 = not find.jira_issue.jira_change or (
                    find.jira_issue.jira_change < issue_from_jira.fields.updated
                )
                flag2 = not find.last_status_update or (
                    find.last_status_update < issue_from_jira.fields.updated
                )
                flag3 = not find.last_reviewed or (
                    find.last_reviewed < issue_from_jira.fields.updated
                )
                ...
                if flag1 and flag2 and flag3:
                    ...
                    flag3 = (
                        find.is_mitigated
                        and find.mitigated
                        and find.jira_issue.jira_change
                        and find.mitigated > find.jira_issue.jira_change
                    )
            ...
            if action == "import_status_from_jira":
                ...
                if status_changed:
                    message = (
                        "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s finding in defectdojo;%s"
                        % (
                            find.jira_issue.jira_key,
                            settings.SITE_URL,
                            find.id,
                    ...
                    message = (
                        "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;no changes made from jira resolution;%s"
                        % (
                            find.jira_issue.jira_key,
                            settings.SITE_URL,
                            find.id,
                ...
                if not prev_jira_instance or (
                    jira_instance.id != prev_jira_instance.id
                ):
                ...
                status_changed = (
                    jira_helper.push_status_to_jira(
                        find, jira_instance, jira, issue_from_jira, save=True
                    )
                    if not dryrun
                    else "dryrun"
                )
                ...
                if status_changed:
                    message = (
                        "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s jira issue;%s;"
                        % (
                            find.jira_issue.jira_key,
                            settings.SITE_URL,
                            find.id,
                    ...
                    message = (
                        "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;no changes made while pushing status to jira;%s"
                        % (
                            find.jira_issue.jira_key,
                            settings.SITE_URL,
                            find.id,
                ...
                message = (
                    "%s; %s/finding/%d;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;unable to determine source of truth;%s"
                    % (
                        find.jira_issue.jira_key,
                        settings.SITE_URL,
                        find.id,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--product", help="Only process findings in this product (name)"
        )
        parser.add_argument(
            "--engagement", help="Only process findings in this product (name)"
        )
        parser.add_argument(
            "--daysback",
            type=int,
            help="Only process findings created in the last 'daysback' days",
        )
        parser.add_argument(
            "--dryrun",
            action="store_true",
            help="Only print actions to be performed, but make no modifications.",
        )
