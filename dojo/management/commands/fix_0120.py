...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not (executor.migration_plan([("dojo", "0119_default_group_is_staff")])):
            ...
            logger.warning(
                'This command will remove field "sonarqube_config" in model "Test" to be able to finish migration 0120_sonarqube_test_and_clean'
            )
            try:
            ...
            logger.error(
                "Only migrations stacked in front of 0120 can be fixed by this command"
            )
