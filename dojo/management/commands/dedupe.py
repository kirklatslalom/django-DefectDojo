...
def generate_hash_code(f):
    ...
    if f.hash_code != old_hash_code:
        logger.debug(
            "%d: hash_code changed from %s to %s", f.id, old_hash_code, f.hash_code
        )
    return f
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--hash_code_only", action="store_true", help="Only compute hash codes"
        )
        parser.add_argument(
            "--dedupe_only", action="store_true", help="Only run deduplication"
        )
        parser.add_argument(
            "--dedupe_sync",
            action="store_true",
            help="Run dedupe in the foreground, default false",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not hash_code_only:
            if get_system_setting("enable_deduplication"):
                ...
                logger.info(
                    "######## Done deduplicating (%s) ########",
                    ("foreground" if dedupe_sync else "tasks submitted to celery"),
                )
            else:
