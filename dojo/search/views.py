...
def simple_search(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            search_tags = (
                "tag" in operators
                or "test-tag" in operators
                or "engagement-tag" in operators
                or "product-tag" in operators
                or "tags" in operators
                or "test-tags" in operators
                or "engagement-tags" in operators
                or "product-tags" in operators
                or "not-tag" in operators
                or "not-test-tag" in operators
                or "not-engagement-tag" in operators
                or "not-product-tag" in operators
                or "not-tags" in operators
                or "not-test-tags" in operators
                or "not-engagement-tags" in operators
                or "not-product-tags" in operators
            )
            ...
            search_findings = (
                "finding" in operators
                or search_finding_id
                or search_tags
                or not operators
            )

            search_finding_templates = (
                "template" in operators
                or search_tags
                or not (operators or search_finding_id)
            )
            search_tests = (
                "test" in operators
                or search_tags
            ...
            search_engagements = (
                "engagement" in operators
                or search_tags
                or not (operators or search_finding_id)
            )

            search_products = (
                "product" in operators
                or search_tags
                or not (operators or search_finding_id)
            )
            search_endpoints = (
                "endpoint" in operators
                or search_tags
                or not (operators or search_finding_id)
            )
            search_languages = (
                "language" in operators
                or search_tags
                or not (operators or search_finding_id)
            )
            search_technologies = (
                "technology" in operators
                or search_tags
                or not (operators or search_finding_id)
            )
            ...
            authorized_app_analysis = get_authorized_app_analysis(
                Permissions.Product_View
            )
            authorized_vulnerability_ids = get_authorized_vulnerability_ids(
            ...
            if search_tags and tag or tags or not_tag or not_tags:
                ...
                tagged_findings = (
                    authorized_findings.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                    .prefetch_related("tags")
                )
                tagged_finding_templates = (
                    authorized_finding_templates.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                )
                tagged_tests = (
                    authorized_tests.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                    .prefetch_related("tags")
                )
                tagged_engagements = (
                    authorized_engagements.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                    .prefetch_related("tags")
                )
                tagged_products = (
                    authorized_products.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                    .prefetch_related("tags")
                )
                tagged_endpoints = (
                    authorized_endpoints.filter(Q1 | Q2)
                    .exclude(Q3 | Q4)
                    .distinct()[:max_results]
                    .prefetch_related("tags")
                )
            ...
            tagged_results = (
                tagged_findings
                or tagged_finding_templates
                or tagged_tests
                or tagged_engagements
                or tagged_products
                or tagged_endpoints
            )
            ...
            if search_vulnerability_ids:
                ...
                vulnerability_ids = authorized_vulnerability_ids
                vulnerability_ids = apply_vulnerability_id_filter(
                    vulnerability_ids, operators
                )
                if keywords_query:
                ...
                vulnerability_ids = vulnerability_ids.prefetch_related(
                    "finding__test__engagement__product",
                    "finding__test__engagement__product__tags",
                )
...
def apply_tag_filters(qs, operators, skip_relations=False):
    ...
    for tag_filter in tag_filters:
        if tag_filter in operators:
            ...
            qs = qs.filter(
                **{"%stags__name__contains" % tag_filters[tag_filter]: value}
            )
    ...
    for tag_filter in tag_filters:
        ...
        if tag_filter in operators:
            ...
            qs = qs.exclude(
                **{
                    "%stags__name__contains"
                    % tag_filters[tag_filter.replace("not-", "")]: value
                }
            )
    ...
    if "vulnerability_id" in operators:
        ...
        vulnerability_ids = list(
            itertools.chain.from_iterable(
                [vulnerability_id.split(",") for vulnerability_id in value]
            )
        )
