...
def engagement_calendar(request):
    ...
    if "lead" not in request.GET or "0" in request.GET.getlist("lead"):
        ...
        engagements = get_authorized_engagements(Permissions.Engagement_View).filter(
...
def get_filtered_engagements(request, view):
    ...
    engagements = get_authorized_engagements(Permissions.Engagement_View).order_by(
        "-target_start"
    )
    ...
    if System_Settings.objects.get().enable_jira:
        engagements = engagements.prefetch_related(
            "jira_project__jira_instance", "product__jira_project_set__jira_instance"
        )
...
def engagements(request, view):
    ...
    product_name_words = sorted(
        get_authorized_products(Permissions.Product_View).values_list("name", flat=True)
    )
    engagement_name_words = sorted(
...
def engagements_all(request):
    ...
    products_with_engagements = products_with_engagements.filter(
        ~Q(engagement=None)
    ).distinct()
    ...
    filter_qs = products_with_engagements.prefetch_related(
        Prefetch(
            "engagement_set",
            queryset=Engagement.objects.all().annotate(test_count=Count("test__id")),
        )
    )
    ...
    filtered = EngagementFilter(request.GET, queryset=filter_qs)
    ...
    add_breadcrumb(
        title="All Engagements", top_level=not len(request.GET), request=request
    )

    return render(
        request,
        "dojo/engagements_all.html",
        {
            "products": prods,
...
def edit_engagement(request, eid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if new_status == "Cancelled" or new_status == "Completed":
                ...
                create_notification(
                    ...
                    url=reverse("engagement_all_findings", args=(engagement.id,)),
                ),
            else:
        ...
        form = EngForm(
            initial={"product": engagement.product},
            instance=engagement,
            cicd=is_ci_cd,
            product=engagement.product,
            user=request.user,
        ...
        if get_system_setting("enable_jira"):
            jira_project = jira_helper.get_jira_project(
                engagement, use_inheritance=False
            )
            jira_project_form = JIRAProjectForm(
                instance=jira_project, target="engagement", product=engagement.product
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            create_notification(
                ...
                description='The engagement "%s" was copied by %s'
                % (engagement.name, request.user),
                product=product,
                url=request.build_absolute_uri(
            ...
            return redirect_to_return_url_or_else(
                request, reverse("view_engagements", args=(product.id,))
            )
        else:
            messages.add_message(
                request,
...
def view_engagement(request, eid):
    ...
    prod = eng.product
    risks_accepted = (
        eng.risk_acceptance.all()
        .select_related("owner")
        .annotate(accepted_findings_count=Count("accepted_findings__id"))
    )
    ...
    creds = (
        Cred_Mapping.objects.filter(product=eng.product)
        .select_related("cred_id")
        .order_by("cred_id")
    )
    cred_eng = (
        Cred_Mapping.objects.filter(engagement=eng.id)
        .select_related("cred_id")
        .order_by("cred_id")
    )
...
def prefetch_for_view_tests(tests):
    ...
    if isinstance(
        ...
        prefetched = prefetched.annotate(
            count_findings_test_all=Count("finding__id", distinct=True)
        )
        prefetched = prefetched.annotate(
            count_findings_test_active=Count(
                "finding__id", filter=Q(finding__active=True), distinct=True
            )
        )
        prefetched = prefetched.annotate(
            count_findings_test_active_verified=Count(
                "finding__id",
                filter=Q(finding__active=True) & Q(finding__verified=True),
                distinct=True,
            )
        )
        prefetched = prefetched.annotate(
            count_findings_test_mitigated=Count(
                "finding__id", filter=Q(finding__is_mitigated=True), distinct=True
            )
        )
        prefetched = prefetched.annotate(
            count_findings_test_dups=Count(
                "finding__id", filter=Q(finding__duplicate=True), distinct=True
            )
        )
        prefetched = prefetched.annotate(
            total_reimport_count=Count(
                "test_import__id",
                filter=Q(test_import__type=Test_Import.REIMPORT_TYPE),
                distinct=True,
            )
        )
            ...
            if cred_form.is_valid():
                if cred_form.cleaned_data["cred_user"]:
                    ...
                    cred_user = Cred_Mapping.objects.filter(
                        pk=cred_form.cleaned_data["cred_user"].id, engagement=eid
                    ).first()
...
def import_scan_results(request, eid=None, pid=None):
    ...
    user_has_permission_or_403(
        user, engagement_or_product, Permissions.Import_Scan_Result
    )
    ...
    if request.method == "POST":
        ...
        if jira_helper.get_jira_project(engagement_or_product):
            jform = JIRAImportScanForm(
                request.POST, push_all=push_all_jira_issues, prefix="jiraform"
            )
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            create_finding_groups_for_all_findings = form.cleaned_data[
            ...
            if engagement is None:
                ...
                engagement.name = "AdHoc Import - " + strftime(
                    "%a, %d %b %Y %X", timezone.now().timetuple()
                )
            ...
            added_endpoints = save_endpoints_to_add(
                form.endpoints_to_add_list, engagement.product
            )
            ...
            try:
                ...
                test, finding_count, closed_finding_count, _ = importer.import_scan(
                    ...
                    endpoints_to_add=list(form.cleaned_data["endpoints"])
                    + added_endpoints,
                    scan_date=scan_date,
                    version=version,
                ...
                logger.exception(e)
                add_error_message_to_response(
                    "An exception error occurred during the report import:%s" % str(e)
                )
                error = True
    ...
    ].queryset = Product_API_Scan_Configuration.objects.filter(
        product__id=product_tab.product.id
    )
    return render(
        request,
...
def close_eng(request, eid):
    ...
    create_notification(
        ...
        engagement=eng,
        url=reverse("engagement_all_findings", args=(eng.id,)),
    ),
...
def complete_checklist(request, eid):
    ...
    add_breadcrumb(
        parent=eng, title="Complete checklist", top_level=False, request=request
    )
    if request.method == "POST":
...
def add_risk_acceptance(request, eid, fid=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            risk_acceptance = ra_helper.add_findings_to_risk_acceptance(
                risk_acceptance, findings
            )
        ...
        form = RiskAcceptanceForm(
            initial={"owner": request.user, "name": risk_acceptance_title_suggestion}
        )

    finding_choices = (
        Finding.objects.filter(duplicate=False, test__engagement=eng)
        .filter(NOT_ACCEPTED_FINDINGS_QUERY)
        .order_by("title")
    )
    ...
    return render(
        request,
        "dojo/add_risk_acceptance.html",
        {"eng": eng, "product_tab": product_tab, "form": form},
    )
...
def view_edit_risk_acceptance(request, eid, raid, edit_mode=False):
    ...
    if request.method == "POST":
        ...
        if "decision" in request.POST:
            ...
            risk_acceptance_form = EditRiskAcceptanceForm(
        ...
        if "remove_finding" in request.POST:
            finding = get_object_or_404(Finding, pk=request.POST["remove_finding_id"])
    ...
    unaccepted_findings = (
        Finding.objects.filter(test__in=eng.test_set.all())
        .exclude(id__in=accepted_findings)
        .order_by("title")
    )
    ...
    add_findings_form.fields["accepted_findings"].queryset = add_fpage.object_list

    product_tab = Product_Tab(eng.product, title="Risk Acceptance", tab="engagements")
    product_tab.setEngagement(eng)
    return render(
        request,
        "dojo/view_risk_acceptance.html",
        {
...
def expire_risk_acceptance(request, eid, raid):
    risk_acceptance = get_object_or_404(
        prefetch_for_expiration(Risk_Acceptance.objects.all()), pk=raid
    )
    eng = get_object_or_404(Engagement, pk=eid)
...
def download_risk_acceptance(request, eid, raid):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="%s"' % risk_acceptance.filename()
    )
...
def upload_threatmodel(request, eid):
    eng = Engagement.objects.get(id=eid)
    add_breadcrumb(
        parent=eng, title="Upload a threat model", top_level=False, request=request
    )
    ...
    product_tab = Product_Tab(
        eng.product, title="Upload Threat Model", tab="engagements"
    )
    return render(
        request,
        "dojo/up_threat.html",
    ...
    for engagement in engagements:
        if first_row:
            ...
            for key in dir(engagement):
                if (
                    key not in get_excludes()
                    and not callable(getattr(engagement, key))
        ...
        if row_num > 1:
            ...
            for key in dir(engagement):
                if (
                    ...
                    col_num += 1
            worksheet.cell(
                row=row_num, column=col_num, value=test_counts.get(engagement.id, 0)
            )
        row_num += 1
