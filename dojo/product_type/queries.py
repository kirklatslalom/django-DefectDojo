...
def get_authorized_product_types(permission):
    ...
    product_types = Product_Type.objects.annotate(
        member=Exists(authorized_roles), authorized_group=Exists(authorized_groups)
    ).order_by("name")
    ...
    if user.is_superuser or user_has_permission(user, product_type, permission):
        ...
        return (
            Product_Type_Group.objects.filter(
                product_type=product_type, group__in=authorized_groups
            )
...
def get_authorized_product_type_members_for_user(user, permission):
    ...
    product_types = get_authorized_product_types(permission)
    return Product_Type_Member.objects.filter(
        user=user, product_type__in=product_types
    ).select_related("role", "product_type")
