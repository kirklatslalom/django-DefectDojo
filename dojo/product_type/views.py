...
def product_type(request):
    ...
    return render(
        ...
        {"name": page_name, "pts": pts, "ptl": ptl, "name_words": name_words},
    )
...
def prefetch_for_product_type(prod_types):
    ...
    if isinstance(
        ...
        active_verified_findings_query = Q(
            prod_type__engagement__test__finding__active=True,
        ...
        prefetch_prod_types = prefetch_prod_types.annotate(
            active_findings_count=Count(
                "prod_type__engagement__test__finding__id", filter=active_findings_query
            )
        )
        prefetch_prod_types = prefetch_prod_types.annotate(
            active_verified_findings_count=Count(
                "prod_type__engagement__test__finding__id",
                filter=active_verified_findings_query,
            )
        )
        prefetch_prod_types = prefetch_prod_types.annotate(
...
def edit_product_type(request, ptid):
    ...
    pt = get_object_or_404(Product_Type, pk=ptid)
    members = get_authorized_members_for_product_type(
        pt, Permissions.Product_Type_Manage_Members
    )
    ...
    return render(
        ...
        {"name": page_name, "pt_form": pt_form, "pt": pt, "members": members},
    )
...
def add_product_type_member(request, ptid):
    ...
    return render(
        request,
        "dojo/new_product_type_member.html",
        {
            "pt": pt,
            "form": memberform,
        },
...
@user_is_authorized(
    Product_Type_Member, Permissions.Product_Type_Manage_Members, "memberid"
)
def edit_product_type_member(request, memberid):
            ...
            if not member.role.is_owner:
                owners = (
                    Product_Type_Member.objects.filter(
                        product_type=member.product_type, role__is_owner=True
                    )
                ...
                if owners < 1:
                    messages.add_message(
                        ...
                        % {"product_type_name": member.product_type.name},
                        extra_tags="alert-warning",
                    )
    ...
    return render(
        request,
        "dojo/edit_product_type_member.html",
        {
            "name": page_name,
            "memberid": memberid,
            "form": memberform,
...
@user_is_authorized(
    Product_Type_Member, Permissions.Product_Type_Member_Delete, "memberid"
)
def delete_product_type_member(request, memberid):
    ...
    return render(
        request,
        "dojo/delete_product_type_member.html",
        {
            "name": page_name,
            "memberid": memberid,
            "form": memberform,
...
def add_product_type_group(request, ptid):
    ...
    return render(
        request,
        "dojo/new_product_type_group.html",
        {
            "name": page_name,
            "pt": pt,
            "form": group_form,
...
def edit_product_type_group(request, groupid):
    ...
    return render(
        request,
        "dojo/edit_product_type_group.html",
        {"name": page_name, "groupid": groupid, "form": groupform},
    )


@user_is_authorized(
    Product_Type_Group, Permissions.Product_Type_Group_Delete, "groupid"
)
def delete_product_type_group(request, groupid):
    ...
    return render(
        request,
        "dojo/delete_product_type_group.html",
        {"name": page_name, "groupid": groupid, "form": groupform},
    )
