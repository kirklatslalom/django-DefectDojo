...
def view_test(request, tid):
    ...
    paged_test_imports = get_page_items_and_count(
        request, test_import_filter.qs, 5, prefix="test_imports"
    )
    paged_test_imports.object_list = paged_test_imports.object_list.prefetch_related(
        "test_import_finding_action_set"
    )

    paged_findings = get_page_items_and_count(
        request, prefetch_for_findings(findings.qs), 25, prefix="findings"
    )
...
def edit_test(request, tid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            new_test = form.save()
            messages.add_message(
                request, messages.SUCCESS, _("Test saved."), extra_tags="alert-success"
            )
    ...
    product_tab = Product_Tab(
        test.engagement.product, title=_("Edit Test"), tab="engagements"
    )
    product_tab.setEngagement(test.engagement)
    return render(
        request,
...
def delete_test(request, tid):
        ...
        if "id" in request.POST and str(test.id) == request.POST["id"]:
            ...
            if form.is_valid():
                ...
                if get_setting("ASYNC_OBJECT_DELETE"):
                    ...
                    message = _(
                        "Test and relationships will be removed in the background."
                    )
                else:
                ...
                create_notification(
                    ...
                    product=product,
                    description=_('The test "%(title)s" was deleted by %(user)s')
                    % {"title": test.title, "user": request.user},
    ...
    product_tab = Product_Tab(
        test.engagement.product, title=_("Delete Test"), tab="engagements"
    )
    product_tab.setEngagement(test.engagement)
    return render(
        request,
...
def copy_test(request, tid):
    ...
    product = test.engagement.product
    engagement_list = get_authorized_engagements(Permissions.Engagement_Edit).filter(
        product=product
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            create_notification(
                ...
                description='The test "%s" was copied by %s to %s'
                % (test.title, request.user, engagement.name),
                product=product,
                url=request.build_absolute_uri(
            ...
            return redirect_to_return_url_or_else(
                request, reverse("view_engagement", args=(engagement.id,))
            )
        else:
            messages.add_message(
                request,
...
def test_calendar(request):
    ...
    if "lead" not in request.GET or "0" in request.GET.getlist("lead"):
        ...
        tests = get_authorized_tests(Permissions.Test_View).filter(
...
def test_ics(request, tid):
    ...
    cal = get_cal_event(
        ...
        _(
            "Set aside for test %(test_type_name)s, on product %(product_name)s. Additional detail can be found at %(detail_url)s"
        )
        % {
            "test_type_name": test.test_type.name,
            "product_name": test.engagement.product.name,
    ...
    response["Content-Disposition"] = (
        "attachment; filename=%s.ics" % test.test_type.name
    )
    return response
...
def add_findings(request, tid):
    ...
    if request.method == "POST":
        form = AddFindingForm(
            request.POST, req_resp=None, product=test.engagement.product
        )
    ...
    product_tab = Product_Tab(
        test.engagement.product, title=_("Add Finding"), tab="engagements"
    )
    product_tab.setEngagement(test.engagement)
    return render(
        request,
...
def search(request, tid):
    ...
    add_breadcrumb(
        parent=test, title=_("Add From Template"), top_level=False, request=request
    )
    return render(
        request,
        "dojo/templates.html",
...
def re_import_scan_results(request, tid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and (jform is None or jform.is_valid()):
            ...
            create_finding_groups_for_all_findings = form.cleaned_data.get(
            ...
            if scan and is_scan_file_too_large(scan):
                messages.add_message(
                    request,
                    messages.ERROR,
                    _("Report file is too large. Maximum supported size is %(size)d MB")
                    % {"size": settings.SCAN_FILE_MAX_SIZE},
                    extra_tags="alert-danger",
                )
                return HttpResponseRedirect(
    ...
    product_tab = Product_Tab(
        engagement.product,
        title=_("Re-upload a %(scan_type)s") % {"scan_type": scan_type},
        tab="engagements",
    )
