...
def get_authorized_users_for_product_type(users, product_type, permission):
    ...
    product_type_members = Product_Type_Member.objects.filter(
        product_type=product_type, role__in=roles
    ).select_related("user")
    product_type_groups = Product_Type_Group.objects.filter(
        product_type=product_type, role__in=roles
    )
    group_members = Dojo_Group_Member.objects.filter(
        group__in=[ptg.group for ptg in product_type_groups]
    ).select_related("user")
    return users.filter(
...
def get_authorized_users_for_product_and_product_type(users, product, permission):
    ...
    product_groups = Product_Group.objects.filter(product=product, role__in=roles)
    product_type_groups = Product_Type_Group.objects.filter(
        product_type=product.prod_type, role__in=roles
    )
    group_members = Dojo_Group_Member.objects.filter(
...
def get_authorized_users(permission, user=None):
    ...
    roles = get_roles_for_permission(permission)
    product_members = Product_Member.objects.filter(
        product_id__in=authorized_products, role__in=roles
    ).select_related("user")
    product_type_members = Product_Type_Member.objects.filter(
        product_type_id__in=authorized_product_types, role__in=roles
    ).select_related("user")
