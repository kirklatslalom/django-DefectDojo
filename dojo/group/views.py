...
def group(request):
    ...
    return render(
        request,
        "dojo/groups.html",
        {"groups": paged_groups, "filtered": groups, "name": "All Groups"},
    )
...
def edit_group(request, gid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and global_role_form.is_valid():
            if (
                global_role_form.cleaned_data["role"] != previous_global_role
...
def delete_group(request, gid):
    ...
    return render(
        request,
        "dojo/delete_group.html",
        {"to_delete": group, "form": form, "rels": rels},
    )
...
def add_group(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and global_role_form.is_valid():
            if (
                global_role_form.cleaned_data["role"] is not None
                and not request.user.is_superuser
            ):
                messages.add_message(
                    request,
...
def add_group_member(request, gid):
    ...
    if request.method == "POST":
        ...
        if groupform.is_valid():
            if groupform.cleaned_data["role"].is_owner and not user_has_permission(
                ...
                if (
                    "users" in groupform.cleaned_data
                    and len(groupform.cleaned_data["users"]) > 0
                ):
    ...
    return render(
        request, "dojo/new_group_member.html", {"group": group, "form": groupform}
    )
...
def edit_group_member(request, mid):
    ...
    return render(
        request, "dojo/edit_group_member.html", {"memberid": mid, "form": memberform}
    )
...
def delete_group_member(request, mid):
    ...
    return render(
        request, "dojo/delete_group_member.html", {"memberid": mid, "form": memberform}
    )
...
def add_product_group(request, gid):
    ...
    if request.method == "POST":
        group_form = Add_Product_Group_GroupForm(
            request.POST, initial={"group": group.id}
        )
        if group_form.is_valid():
            if (
                "products" in group_form.cleaned_data
                and len(group_form.cleaned_data["products"]) > 0
            ):
                for product in group_form.cleaned_data["products"]:
                    existing_groups = Product_Group.objects.filter(
    ...
    return render(
        request,
        "dojo/new_product_group_group.html",
...
def add_product_type_group(request, gid):
    ...
    if request.method == "POST":
        group_form = Add_Product_Type_Group_GroupForm(
            request.POST, initial={"group": group.id}
        )
        if group_form.is_valid():
            if (
                "product_types" in group_form.cleaned_data
                and len(group_form.cleaned_data["product_types"]) > 0
            ):
                for product_type in group_form.cleaned_data["product_types"]:
                    existing_groups = Product_Type_Group.objects.filter(
                        product_type=product_type, group=group
                    )
                    if existing_groups.count() == 0:
    ...
    return render(
        request,
        "dojo/new_product_type_group_group.html",
        {
            "group": group,
            "form": group_form,
        },
