...
class DojoGroupViewSet(
    ...
    swagger_schema = prefetch.get_prefetch_schema(
        ["dojo_groups_list", "dojo_groups_read"], serializers.DojoGroupSerializer
    ).to_schema()
...
class GlobalRoleViewSet(
    ...
    swagger_schema = prefetch.get_prefetch_schema(
        ["global_roles_list", "global_roles_read"], serializers.GlobalRoleSerializer
    ).to_schema()
...
class EndPointViewSet(
    ...
    def generate_report(self, request, pk=None):
        ...
        if report_options.is_valid():
            ...
            options["include_executive_summary"] = report_options.validated_data[
                "include_executive_summary"
            ]
            options["include_table_of_contents"] = report_options.validated_data[
                "include_table_of_contents"
            ]
        else:
            return Response(report_options.errors, status=status.HTTP_400_BAD_REQUEST)
...
class EngagementViewSet(
    ...
    @extend_schema(request=OpenApiTypes.NONE, responses={status.HTTP_200_OK: ""})
    @swagger_auto_schema(request_body=no_body, responses={status.HTTP_200_OK: ""})
    ...
    def notes(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            author = request.user
            note = Notes(
                entry=entry, author=author, private=private, note_type=note_type
            )
            note.save()
...
class FindingViewSet(
    ...
    def get_queryset(self):
        findings = get_authorized_findings(Permissions.Finding_View).prefetch_related(
            "endpoints",
            "reviewers",
            "found_by",
            "notes",
        ...
        if request.method == "POST":
            ...
            if finding_close.is_valid():
                ...
                if settings.EDITABLE_MITIGATED_DATA:
                    finding.mitigated = (
                        finding_close.validated_data["mitigated"] or timezone.now()
                    )
                else:
    ...
    @extend_schema(
        methods=["GET"], responses={status.HTTP_200_OK: serializers.TagSerializer}
    )
    @extend_schema(
        methods=["POST"],
    ...
    def tags(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if new_tags.is_valid():
                ...
                return Response(new_tags.errors, status=status.HTTP_400_BAD_REQUEST)
        tags = finding.tags
    ...
    def request_response(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if burps.is_valid():
                ...
                return Response(burps.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def remove_tags(self, request, pk=None):
        ...
        if delete_tags.is_valid():
            ...
            for tag in del_tags:
                if tag not in all_tags:
                    return Response(
                        {"error": "'{}' is not a valid tag in list".format(tag)},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                all_tags.remove(tag)
            new_tags = tagulous.utils.render_tags(all_tags)
            ...
            finding.save()
            return Response(
                {"success": "Tag(s) Removed"}, status=status.HTTP_204_NO_CONTENT
            )
        else:
            return Response(delete_tags.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _add_metadata(self, request, finding):
        ...
        if metadata_data.is_valid():
            ...
            return Response(metadata_data.errors, status=status.HTTP_400_BAD_REQUEST)

    def _remove_metadata(self, request, finding):
        name = request.query_params.get("name", None)
        if name is None:
            return Response(
                "A metadata name must be provided", status=status.HTTP_400_BAD_REQUEST
            )
...
@method_decorator(
    name="list",
    decorator=swagger_auto_schema(filter_inspectors=[DjangoFilterDescriptionInspector]),
)
...
class ProductTypeViewSet(
    ...
    swagger_schema = prefetch.get_prefetch_schema(
        ["product_types_list", "product_types_read"], serializers.ProductTypeSerializer
    ).to_schema()
...
class TestsViewSet(
    ...
    @swagger_auto_schema(
        method="get", responses={status.HTTP_200_OK: serializers.TestToNotesSerializer}
    )
    @swagger_auto_schema(
        methods=["post"],
        request_body=serializers.AddNewNoteOptionSerializer,
    ...
    @swagger_auto_schema(
        method="get", responses={status.HTTP_200_OK: serializers.TestToFilesSerializer}
    )
    @swagger_auto_schema(
        method="post",
        request_body=serializers.AddNewFileOptionSerializer,
...
class TestImportViewSet(
    ...
    swagger_schema = prefetch.get_prefetch_schema(
        ["test_imports_list", "test_imports_read"], serializers.TestImportSerializer
    ).to_schema()
...
class UsersViewSet(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if request.user == instance:
            return Response(
                "Users may not delete themselves", status=status.HTTP_400_BAD_REQUEST
            )
        self.perform_destroy(instance)
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class UserProfileView(GenericAPIView):
    ...
    @swagger_auto_schema(
        method="get", responses={status.HTTP_200_OK: serializers.UserProfileSerializer}
    )
...
class ImportScanView(mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def perform_create(self, serializer):
        (
            ...
            do_not_reactivate,
        ) = serializers.get_import_meta_data_from_dict(serializer.validated_data)
...
# Authorization: authenticated users, DjangoModelPermissions
class EndpointMetaImporterView(mixins.CreateModelMixin, viewsets.GenericViewSet):
    """
...
def report_generate(request, obj, options):
    ...
    if type(obj).__name__ == "Product_Type":
        ...
        tests = Test.objects.filter(
            engagement__product__prod_type=product_type, finding__in=findings.qs
        ).distinct()
        ...
        findings = ReportFindingFilter(
            request.GET, queryset=prefetch_related_findings_for_report(obj).distinct()
        )
