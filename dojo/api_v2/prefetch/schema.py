...
def get_prefetch_schema(methods, serializer):
    ...
    fields_name = [
        name for name, field_type in fields if prefetcher._find_serializer(field_type)
    ]
        ...
        [
            (
                ...
                openapi.Schema(
                    ...
                    read_only=True,
                    additional_properties=LazySchemaRef(fields_to_refname[name], True),
                ),
            )
        ...
        if "get" in paths[path] and "parameters" in paths[path]["get"]:
            for parameter in paths[path]["get"]["parameters"]:
                if parameter["name"] == "prefetch":
                    ...
                    result["components"]["schemas"][component_name]["properties"][
                        "prefetch"
                    ] = dict([("type", "object"), ("properties", properties)])
