    ...
    def to_representation(self, value):
        if not isinstance(value, list):
            ...
            if type(value).__name__ == "TagRelatedManager":
                ...
                raise ValueError(
                    "unable to convert %s into list of tags" % type(value).__name__
                )
        return value
        ...
        if not isinstance(value, RequestResponseDict):
            if not isinstance(value, list):
                ...
                if self.order_by:
                    ...
                    burps = value.all()
                value = [
                    {"request": burp.get_request(), "response": burp.get_response()}
                    for burp in burps
                ]
...
class MetaSerializer(serializers.ModelSerializer):
    product = serializers.PrimaryKeyRelatedField(
        queryset=Product.objects.all(), required=False, default=None, allow_null=True
    )
    endpoint = serializers.PrimaryKeyRelatedField(
        queryset=Endpoint.objects.all(), required=False, default=None, allow_null=True
    )
    finding = serializers.PrimaryKeyRelatedField(
        queryset=Finding.objects.all(), required=False, default=None, allow_null=True
    )
    ...
    def update(self, instance, validated_data):
        ...
        if new_configuration_permissions:
            ...
            non_configuration_permissions = (
                set(instance.user_permissions.all()) - allowed_configuration_permissions
            )
...
class DojoGroupMemberSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get("role").is_owner and not user_has_permission(
            self.context["request"].user, data.get("group"), Permissions.Group_Add_Owner
        ):
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        history = NoteHistory(
            data=instance.entry, time=instance.edit_time, current_editor=instance.editor
        )
        history.save()
...
class EndpointStatusSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            status = Endpoint_Status.objects.create(finding=finding, endpoint=endpoint)
...
class EndpointSerializer(TaggitSerializer, serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            ...
            and (
                (endpoint.count() > 1)
                or (endpoint.count() == 1 and endpoint.first().pk != self.instance.pk)
            )
        ) or (self.context["request"].method in ["POST"] and endpoint.count() > 0):
            raise serializers.ValidationError(
...
class TestCreateSerializer(TaggitSerializer, serializers.ModelSerializer):
    ...
    notes = serializers.PrimaryKeyRelatedField(
        allow_null=True, queryset=Notes.objects.all(), many=True, required=False
    )
    tags = TagListSerializerField(required=False)
...
class ProductSerializer(TaggitSerializer, serializers.ModelSerializer):
    ...
    @swagger_serializer_method(
        serializer_or_field=serializers.ListField(child=serializers.IntegerField())
    )
...
class ImportScanSerializer(serializers.Serializer):
    scan_date = serializers.DateField(
        required=False, help_text="Scan completion date will be used on all findings."
    )
    ...
    def save(self, push_to_jira=False):
        ...
        scan_date_time = (
            timezone.make_aware(datetime.combine(scan_date, datetime.min.time()))
            if scan_date
            else None
        )
        importer = Importer()
    ...
    def validate_scan_date(self, value):
        if value and value > timezone.localdate():
            raise serializers.ValidationError("The scan_date cannot be in the future!")
        return value
...
class ReImportScanSerializer(TaggitSerializer, serializers.Serializer):
    ...
    scan_type = serializers.ChoiceField(choices=get_choices_sorted(), required=True)
    endpoint_to_add = serializers.PrimaryKeyRelatedField(
    ...
    tags = TagListSerializerField(
        required=False, help_text="Modify existing tags that help describe this scan."
    )
    ...
    def save(self):
        ...
        for name in deserialized:
            if name not in ["header", "SUM"]:
                ...
                try:
                    language_type, created = Language_Type.objects.get_or_create(
                        language=name
                    )
...
class NotificationsSerializer(serializers.ModelSerializer):
    ...
    user = serializers.PrimaryKeyRelatedField(
        queryset=Dojo_User.objects.all(), required=False, default=None, allow_null=True
    )
