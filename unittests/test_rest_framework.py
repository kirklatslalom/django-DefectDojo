...
class SchemaChecker:
    ...
    def check(self, schema, obj):
        def _check(schema, obj):
            ...
            if (
                "type" in schema
                and schema["type"] is TYPE_ARRAY
                and isinstance(obj, set)
            ):
                obj = list(obj)
                    ...
                    if child_name not in ["prefetch"]:
                        if not properties or child_name not in properties.keys():
                            ...
                            self._register_error(
                                f'unexpected property "{child_name}" found'
                            )
...
class BaseClass:
    class RESTEndpointTest(DojoAPITestCase):
        ...
        def check_schema_response(self, method, status_code, response, detail=False):
            ...
            endpoints_schema = self.schema["paths"][
                format_url(f"/{self.endpoint_path}/{detail_path}")
            ]
            schema = endpoints_schema[method]["responses"][status_code]["content"][
                "application/json"
            ]["schema"]
            obj = response.data
        ...
        def test_list(self):
            ...
            if (
                hasattr(self.endpoint_model, "tags")
                and self.payload
                and self.payload.get("tags", None)
            ):
            ...
            if check_for_tags:
                ...
                for result in response.data["results"]:
                    if result["id"] == check_for_id:
                        ...
                        self.assertEqual(
                            len(check_for_tags), len(result.get("tags", None))
                        )
        ...
        def test_create(self):
            ...
            if (
                ...
                self.assertEqual(
                    len(self.payload.get("tags")), len(response.data.get("tags", None))
                )
        ...
        def test_update(self):
            ...
            if (
                hasattr(self.endpoint_model, "tags")
                and self.update_fields
                and self.update_fields.get("tags", None)
            ):
                self.assertEqual(
                    len(self.update_fields.get("tags")),
                    len(response.data.get("tags", None)),
                )
                for tag in self.update_fields.get("tags"):
                    logger.debug(
                        "looking for tag %s in tag list %s", tag, response.data["tags"]
                    )
                    self.assertTrue(tag in response.data["tags"])
        ...
        def test_delete_preview(self):
            current_objects = self.client.get(self.url, format="json").data
            relative_url = (
                self.url + "%s/delete_preview/" % current_objects["results"][0]["id"]
            )
            response = self.client.get(relative_url)
            ...
            self.assertTrue(
                ...
                "Length: {}".format(len(response.data["results"])),
            )
        ...
        def test_detail_prefetch(self):
            # print("=======================================================")
            prefetchable_fields = [
                x[0] for x in _get_prefetchable_fields(self.viewset.serializer_class)
            ]
        ...
        def test_create_object_not_authorized(self, mock):
            ...
            mock.assert_called_with(
                User.objects.get(username="admin"), ANY, self.permission_create
            )
        ...
        def test_delete_object_not_authorized(self, mock):
            ...
            if self.endpoint_model == Endpoint_Status:
                permission_object = Endpoint.objects.get(
                    id=current_objects["results"][0]["endpoint"]
                )
            elif self.endpoint_model == JIRA_Issue:
                permission_object = Finding.objects.get(
                    id=current_objects["results"][0]["finding"]
                )
            else:
                permission_object = self.permission_check_class.objects.get(
                    id=current_objects["results"][0]["id"]
                )
    ...
    class MemberEndpointTest(RESTEndpointTest):
        ...
        def test_update_object_not_authorized(self, mock):
            ...
            mock.assert_called_with(
                User.objects.get(username="admin"),
                self.permission_check_class.objects.get(
                    id=current_objects["results"][0]["id"]
                ),
                self.permission_update,
            )
...
class FindingRequestResponseTest(DojoAPITestCase):
    ...
    def test_request_response_post(self):
        ...
        payload = {"req_resp": [{"request": "POST", "response": "200"}]}
        response = self.client.post(
            "/api/v2/findings/7/request_response/",
            dumps(payload),
    ...
    def test_request_response_get(self):
        response = self.client.get(
            "/api/v2/findings/7/request_response/", format="json"
        )
        # print('response.data:')
        # print(response.data)
        self.assertEqual(200, response.status_code, response.content[:1000])
...
class FindingFilesTest(DojoAPITestCase):
    ...
    def test_request_response_post(self):
        url_levels = ["findings/7", "tests/3", "engagements/1"]
...
class FindingsTest(BaseClass.RESTEndpointTest):
    ...
    def test_duplicate(self):
        ...
        self.assertEqual(
            result.status_code, status.HTTP_204_NO_CONTENT, "Could not move duplicate"
        )
        result = self.client.get(self.url + "2/")
        self.assertEqual(
            result.status_code, status.HTTP_200_OK, "Could not check new duplicate"
        )
        ...
        self.assertEqual(
            result.status_code, status.HTTP_204_NO_CONTENT, "Could not reset duplicate"
        )
        new_result = self.client.get(self.url + "2/")
        self.assertEqual(
            result.status_code,
            status.HTTP_204_NO_CONTENT,
            "Could not check reset duplicate status",
        )
    ...
    def test_filter_steps_to_reproduce(self):
        ...
        self.assertEqual(
            result.status_code,
            status.HTTP_200_OK,
            "Could not filter on steps_to_reproduce",
        )
        result_json = result.json()
        ...
        result = self.client.patch(
            self.url + "2/", data={"steps_to_reproduce": "Lorem ipsum dolor sit amet"}
        )
        self.assertEqual(
            result.status_code,
            status.HTTP_200_OK,
            "Could not patch finding with steps to reproduce",
        )
        assert result.json()["steps_to_reproduce"] == "Lorem ipsum dolor sit amet"
        result = self.client.patch(
            self.url + "3/", data={"steps_to_reproduce": "Ut enim ad minim veniam"}
        )
        self.assertEqual(
            result.status_code,
            status.HTTP_200_OK,
        ...
        assert (
            result_json["results"][0]["steps_to_reproduce"]
...
class FindingMetadataTest(BaseClass.RESTEndpointTest):
    ...
    def test_create(self):
        response = self.client.post(
            self.base_url, data={"name": "test_meta2", "value": "40"}
        )
    ...
    def test_create_duplicate(self):
        result = self.client.post(
            self.base_url, data={"name": "test_meta", "value": "40"}
        )
        assert (
            result.status_code == status.HTTP_400_BAD_REQUEST
        ), "Metadata creation did not failed on duplicate"
    ...
    def test_update(self):
        self.client.put(
            self.base_url + "?name=test_meta", data={"name": "test_meta", "value": "40"}
        )
        result = self.client.get(self.base_url).data[0]
        assert (
            result["name"] == "test_meta" and result["value"] == "40"
        ), "Metadata not edited correctly"
...
class UsersTest(BaseClass.RESTEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.update_fields = {
            ...
            "configuration_permissions": [219, 220],
        }
        self.test_type = TestType.CONFIGURATION_PERMISSIONS
        self.deleted_objects = 18
        BaseClass.RESTEndpointTest.__init__(self, *args, **kwargs)
    ...
    def test_create_user_with_non_configuration_permissions(self):
        ...
        payload["configuration_permissions"] = [
            25,
            26,
        ]  # these permissions exist but user can not assign them becaause they are not "configuration_permissions"
    ...
    def test_update_user_other_permissions_will_not_leak_and_stay_untouched(self):
        ...
        self.assertEqual(
            response.data["configuration_permissions"],
            payload["configuration_permissions"],
        )
        user_permissions = (
            User.objects.get(username="user5")
            .user_permissions.all()
            .values_list("id", flat=True)
        )
        self.assertEqual(
            set(user_permissions), set(payload["configuration_permissions"] + [26, 28])
        )
...
class ImportScanTest(BaseClass.RESTEndpointTest):
    ...
    def test_create_not_authorized_product_name_engagement_name(
        ...
        mock.assert_called_with(
            ...
            Engagement.objects.get(
                id=2
            ),  # engagement id found via product name and engagement name
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_not_authorized_product_name_engagement_name_auto_create_engagement(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = False
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_not_authorized_product_name_engagement_name_auto_create_product(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = False
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
    ...
    def test_create_not_authorized_product_name_engagement_name_auto_create_product_type(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = False
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
        ...
        self.assertEqual(403, response.status_code, response.content[:1000])
        mock.assert_called_with(
            User.objects.get(username="admin"), Permissions.Product_Type_Add
        )
        importer_mock.assert_not_called()
        reimporter_mock.assert_not_called()
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_authorized_product_name_engagement_name_auto_create_engagement(
        self, mock, importer_mock, reimporter_mock
    ):
        """
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_authorized_product_name_engagement_name_auto_create_product(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = True
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_global_permission")
    def test_create_authorized_product_name_engagement_name_auto_create_product_type(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = True
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
...
class ReimportScanTest(DojoAPITestCase):
    ...
    def test_create_not_authorized_product_name_engagement_name(
        ...
        mock.assert_called_with(
            ...
            Test.objects.get(
                id=4
            ),  # test id found via product name and engagement name and scan_type
            Permissions.Import_Scan_Result,
        )
        importer_mock.assert_not_called()
        reimporter_mock.assert_not_called()
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_authorized_product_name_engagement_name_scan_type_title_auto_create(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = True
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_not_authorized_product_name_engagement_name_scan_type(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = False
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
    ...
    @patch("dojo.importers.importer.importer.DojoDefaultImporter.import_scan")
    @patch("dojo.api_v2.permissions.user_has_permission")
    def test_create_not_authorized_product_name_engagement_name_scan_type_title(
        self, mock, importer_mock, reimporter_mock
    ):
        mock.return_value = False
        importer_mock.return_value = IMPORTER_MOCK_RETURN_VALUE
        ...
        mock.assert_called_with(
            ...
            Test.objects.get(
                id=4
            ),  # test id found via product name and engagement name and scan_type and test_title
            Permissions.Import_Scan_Result,
        )
        importer_mock.assert_not_called()
        reimporter_mock.assert_not_called()
...
class DojoGroupsTest(BaseClass.RESTEndpointTest):
    ...
    def test_update_group_other_permissions_will_not_leak_and_stay_untouched(self):
        Dojo_Group.objects.get(name="Group 1 Testdata").auth_group.permissions.set(
            [218, 220, 26, 28]
        )  # I was trying to set this in 'dojo_testdata.json' but it hasn't sucessful
        payload = {}
        payload["configuration_permissions"] = [217, 218, 219]
        ...
        permissions = (
            Dojo_Group.objects.get(name="Group 1 Testdata")
            .auth_group.permissions.all()
            .values_list("id", flat=True)
        )
        self.assertEqual(
            set(permissions), set(payload["configuration_permissions"] + [26, 28])
        )
...
class DojoGroupsUsersTest(BaseClass.MemberEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.payload = {"group": 1, "user": 3, "role": 4}
...
class ProductTypeMemberTest(BaseClass.MemberEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.payload = {"product_type": 1, "user": 3, "role": 2}
        self.update_fields = {"role": 3}
        self.test_type = TestType.OBJECT_PERMISSIONS
...
class ProductMemberTest(BaseClass.MemberEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.payload = {"product": 3, "user": 2, "role": 2}
        self.update_fields = {"role": 3}
        self.test_type = TestType.OBJECT_PERMISSIONS
...
class ProductTypeGroupTest(BaseClass.MemberEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.payload = {"product_type": 1, "group": 2, "role": 2}
        self.update_fields = {"role": 3}
        self.test_type = TestType.OBJECT_PERMISSIONS
...
class ProductGroupTest(BaseClass.MemberEndpointTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.payload = {"product": 1, "group": 2, "role": 2}
        self.update_fields = {"role": 3}
        self.test_type = TestType.OBJECT_PERMISSIONS
