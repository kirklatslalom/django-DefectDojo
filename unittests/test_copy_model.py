...
class TestCopyFindingModel(DojoTestCase):
    def test_duplicate_finding_same_test(self):
        ...
        test = self.create_test(
            engagement=engagement, scan_type="NPM Audit Scan", title="test"
        )
        ...
        self.assertEqual(
            current_finding_count + 1, Finding.objects.filter(test=test).count()
        )
    ...
    def test_duplicate_finding_different_test(self):
        ...
        product = self.create_product("test_deuplicate_finding", prod_type=product_type)
        engagement = self.create_engagement("eng", product)
        test1 = self.create_test(
            engagement=engagement, scan_type="NPM Audit Scan", title="test1"
        )
        test2 = self.create_test(
            engagement=engagement, scan_type="NPM Audit Scan", title="test2"
        )
        finding = Finding.objects.create(test=test1, reporter=user)
        # Do the counting
        engagement_finding_count = Finding.objects.filter(
        ...
        # Make sure the copy was made without error
        self.assertEqual(
            Finding.objects.filter(test=test1).count(),
            Finding.objects.filter(test=test2).count(),
        )
        # Are the findings the same
        self.assertEqual(finding.hash_code, finding_copy.hash_code)
        # Does the engagement have more findings
        self.assertEqual(
            engagement_finding_count + 1,
            Finding.objects.filter(test__engagement=engagement).count(),
        )
    ...
    def test_duplicate_finding_with_endpoints(self):
        ...
        current_endpoint_status_count = Endpoint_Status.objects.filter(
            endpoint=endpoint
        ).count()
        # Do the copy
        finding_copy = finding.copy(test=test)
        ...
        self.assertEqual(
            current_endpoint_finding_count + 1, endpoint.findings_count
        )  # TODO
        # Make sure the number of endpoint status objects grew
        self.assertEqual(
            current_endpoint_status_count + 1,
            Endpoint_Status.objects.filter(endpoint=endpoint).count(),
        )
...
class TestCopyTestModel(DojoTestCase):
    def test_duplicate_test_same_enagagement(self):
        ...
        current_engagement_finding_count = Finding.objects.filter(
            test__engagement=engagement
        ).count()
        # Do the copy
        ...
        # Make sure the copy was made without error
        self.assertEqual(
            current_test_count + 1, Test.objects.filter(engagement=engagement).count()
        )
        # Do the tests have the same number of findings
        self.assertEqual(
            current_test_finding_count, Finding.objects.filter(test=test_copy).count()
        )
        # Make sure the engagement has more findings
        self.assertEqual(
            current_engagement_finding_count + 1,
            Finding.objects.filter(test__engagement=engagement).count(),
        )
    ...
    def test_duplicate_tests_different_engagements(self):
        ...
        test = self.create_test(
            engagement=engagement1, scan_type="NPM Audit Scan", title="test"
        )
        _ = Finding.objects.create(test=test, reporter=user)
        # Do the counting
        product_finding_count = Finding.objects.filter(
        ...
        # Make sure the copy was made without error
        self.assertEqual(
            Test.objects.filter(engagement=engagement1).count(),
            Test.objects.filter(engagement=engagement2).count(),
        )
        # Do the enagements have the same number of findings
        self.assertEqual(
            Finding.objects.filter(test__engagement=engagement1).count(),
            Finding.objects.filter(test__engagement=engagement2).count(),
        )
        # Are the tests equal
        ...
        self.assertEqual(
            ...
            Finding.objects.filter(test__engagement__product=product).count(),
        )
...
class TestCopyEngagementModel(DojoTestCase):
    def test_duplicate_engagement(self):
        ...
        current_engagement_product_finding_count = Finding.objects.filter(
            test__engagement__product=product
        ).count()
        # Do the copy
        ...
        self.assertEqual(
            ...
            Engagement.objects.filter(product=product).count(),
        )
        # Do the tests have the same number of findings
        self.assertEqual(
            current_engagement_finding_count,
            Finding.objects.filter(test__engagement=engagement_copy).count(),
        )
        # Make sure the product has more findings
        self.assertEqual(
            current_engagement_product_finding_count + 1,
            Finding.objects.filter(test__engagement__product=product).count(),
        )
