...
class TestBurpGraphQLParser(DojoTestCase):
    def test_burp_one_finding(self):
        with open(
            path.join(path.dirname(__file__), "../scans/burp_graphql/one_finding.json")
        ) as test_file:
            ...
            self.assertEqual(
                "request data 1/request data 2/request data 3/",
                findings[0].unsaved_req_resp[0]["req"],
            )
    ...
    def test_burp_two_findings(self):
        with open(
            path.join(path.dirname(__file__), "../scans/burp_graphql/two_findings.json")
        ) as test_file:
            parser = BurpGraphQLParser()
            findings = parser.get_findings(test_file, Test())
            for finding in findings:
    ...
    def test_burp_no_findings(self):
        with open(
            path.join(path.dirname(__file__), "../scans/burp_graphql/no_findings.json")
        ) as test_file:
    ...
    def test_burp_null_title(self):
        with open(
            path.join(path.dirname(__file__), "../scans/burp_graphql/null_title.json")
        ) as test_file:
    ...
    def test_burp_null_request_segments(self):
        with open(
            path.join(
                ...
                "../scans/burp_graphql/null_request_segments.json",
            )
        ) as test_file:
            parser = BurpGraphQLParser()
            findings = parser.get_findings(test_file, Test())
    ...
    def test_burp_null_data(self):
        with open(
            path.join(path.dirname(__file__), "../scans/burp_graphql/null_data.json")
        ) as test_file:
            parser = BurpGraphQLParser()
            findings = parser.get_findings(test_file, Test())
            for finding in findings:
