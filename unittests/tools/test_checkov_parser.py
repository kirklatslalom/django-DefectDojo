...
class TestCheckovParser(DojoTestCase):
    ...
    def test_parse_file_with_multiple_check_type_has_multiple_check_type(self):
        testfile = open(
            "unittests/scans/checkov/checkov-report-multiple-check_type.json"
        )
        parser = CheckovParser()
        findings = parser.get_findings(testfile, Test())
        ...
        self.assertEqual(
            "aws_db_instance.default", first_terraform_finding.component_name
        )
        ...
        self.assertEqual(
            "/aws/resources/Dockerfile", first_dockerfile_finding.file_path
        )
        self.assertEqual(0, first_dockerfile_finding.line)
        self.assertEqual(
            "/aws/resources/Dockerfile.", first_dockerfile_finding.component_name
        )
