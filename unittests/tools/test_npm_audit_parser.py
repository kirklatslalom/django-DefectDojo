...
class TestNpmAuditParser(DojoTestCase):
    def test_npm_audit_parser_with_no_vuln_has_no_findings(self):
        testfile = open(
            path.join(path.dirname(__file__), "../scans/npm_audit_sample/no_vuln.json")
        )
    ...
    def test_npm_audit_parser_with_one_criticle_vuln_has_one_findings(self):
        testfile = open(
            path.join(path.dirname(__file__), "../scans/npm_audit_sample/one_vuln.json")
        )
        parser = NpmAuditParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_npm_audit_parser_with_many_vuln_has_many_findings(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/npm_audit_sample/many_vuln.json"
            )
        )
        parser = NpmAuditParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_npm_audit_parser_multiple_cwes_per_finding(self):
        ...
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/npm_audit_sample/multiple_cwes.json"
            )
        )
        parser = NpmAuditParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_npm_audit_parser_multiple_cwes_per_finding_list(self):
        ...
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/npm_audit_sample/multiple_cwes2.json"
            )
        )
        parser = NpmAuditParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_npm_audit_parser_with_one_criticle_vuln_has_null_as_cwe(self):
        testfile = open(
            path.join(path.dirname(__file__), "../scans/npm_audit_sample/cwe_null.json")
        )
        parser = NpmAuditParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
        ...
        with self.assertRaises(ValueError) as context:
            testfile = open(
                path.join(
                    ...
                    "../scans/npm_audit_sample/empty_with_error.json",
                )
            )
            parser = NpmAuditParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_npm_audit_parser_many_vuln_npm7(self):
        with self.assertRaises(ValueError) as context:
            testfile = open(
                path.join(
                    path.dirname(__file__),
                    "../scans/npm_audit_sample/many_vuln_npm7.json",
                )
            )
            parser = NpmAuditParser()
            findings = parser.get_findings(testfile, Test())
    ...
    def test_npm_audit_censored_hash(self):
        ...
        censored_path = censor_path_hashes(path)
        self.assertEqual(
            censored_path, "censored_by_npm_audit>censored_by_npm_audit>lodash"
        )
