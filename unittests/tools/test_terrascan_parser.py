...
class TestTerrascanParser(DojoTestCase):
    ...
    def test_parse_many_findings(self):
        ...
        with self.subTest(i=0):
            finding = findings[0]
            self.assertEqual(
                "Infrastructure Security: gkeControlPlaneNotPublic", finding.title
            )
            ...
            self.assertEqual(
                "google_container_cluster/k8s_cluster", finding.component_name
            )
        ...
        with self.subTest(i=1):
            finding = findings[1]
            self.assertEqual(
                "Security Best Practices: autoNodeRepairEnabled", finding.title
            )
            ...
            self.assertEqual(
                "google_container_node_pool/k8s_cluster_node_pool",
        ...
        with self.subTest(i=2):
            finding = findings[2]
            self.assertEqual(
                "Infrastructure Security: checkRequireSSLEnabled", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            ...
            self.assertEqual(
                "google_sql_database_instance/master", finding.component_name
            )
        ...
        with self.subTest(i=3):
            finding = findings[3]
            self.assertEqual(
                "Logging and Monitoring: stackDriverLoggingEnabled", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual("cluster.tf", finding.file_path)
        ...
        with self.subTest(i=4):
            finding = findings[4]
            self.assertEqual(
                "Logging and Monitoring: stackDriverMonitoringEnabled", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual("cluster.tf", finding.file_path)
        ...
        with self.subTest(i=5):
            finding = findings[5]
            self.assertEqual(
                "Security Best Practices: checkRotation365Days", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
        ...
        with self.subTest(i=6):
            finding = findings[6]
            self.assertEqual(
                "Infrastructure Security: networkPolicyEnabled", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual("cluster.tf", finding.file_path)
        ...
        with self.subTest(i=7):
            finding = findings[7]
            self.assertEqual(
                "Security Best Practices: checkRotation90Days", finding.title
            )
            self.assertEqual("Medium", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual("vault.tf", finding.file_path)
        ...
        with self.subTest(i=8):
            finding = findings[8]
            self.assertEqual(
                "Security Best Practices: autoNodeUpgradeEnabled", finding.title
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual("cluster.tf", finding.file_path)
