...
class TestNeuVectorComplianceParser(DojoTestCase):
    def test_parse_file_with_no_vuln(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/neuvector_compliance/no_vuln.json"
            )
        )
    ...
    def test_parse_file_with_one_vuln(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/neuvector_compliance/one_vuln.json"
            )
        )
        parser = NeuVectorComplianceParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_with_many_vulns(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/neuvector_compliance/many_vulns.json"
            )
        )
        parser = NeuVectorComplianceParser()
        findings = parser.get_findings(testfile, Test())
