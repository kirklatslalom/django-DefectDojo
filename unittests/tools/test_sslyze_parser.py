...
class TestSslyzeJSONParser(DojoTestCase):
    def test_parse_json_file_with_one_target_has_zero_vuln_old(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_zero_vuln_old.json"
            )
        )
        parser = SslyzeParser()
    ...
    def test_parse_json_file_with_one_target_has_one_vuln_old(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_one_vuln_old.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
        ...
        finding = findings[0]
        self.assertEqual(
            "Problems in certificate deployments (www.example.com:443)", finding.title
        )
 ...
 - certificate has expired for trust store Windows, version 2020-05-04"""
    ...
    def test_parse_json_file_with_one_target_has_four_vuln_old(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_many_vuln_old.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_json_file_with_two_target_has_many_vuln_old(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/two_targets_two_vuln_old.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_json_file_with_one_target_has_zero_vuln_new(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_zero_vuln_new.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_json_file_with_one_target_has_one_vuln_new(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_one_vuln_new.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
        ...
        finding = findings[0]
        self.assertEqual(
            "Unrecommended cipher suites for TLS 1.2 (example.com:443)", finding.title
        )
 ...
 - TLS_DHE_RSA_WITH_AES_128_CBC_SHA"""
    ...
    def test_parse_json_file_with_one_target_has_three_vuln_new(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/one_target_many_vuln_new.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_json_file_with_two_target_has_many_vuln_new(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/two_targets_many_vuln_new.json"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
 ...
 - TLS_DHE_RSA_WITH_AES_128_CBC_SHA"""
        ...
        self.assertEqual(
            "example2.com:443 accepts TLS 1.0 connections", finding.description
        )
        self.assertEqual("Medium", finding.severity)
        self.assertEqual(
            "TLS recommendations of German BSI: https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/Publications/TechGuidelines/TG02102/BSI-TR-02102-2.pdf?__blob=publicationFile&v=10",
...
class TestSSLyzeXMLParser(DojoTestCase):
    def test_parse_file_with_one_target_has_three_vuln(self):
        testfile = open(
            path.join(
                ...
                "../scans/sslyze/report_one_target_three_vuln.xml",
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_xml_file_with_one_target_has_one_vuln(self):
        testfile = open(
            path.join(
                path.dirname(__file__), "../scans/sslyze/report_one_target_one_vuln.xml"
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_xml_file_with_two_target_has_many_vuln(self):
        testfile = open(
            path.join(
                path.dirname(__file__),
                "../scans/sslyze/report_two_target_many_vuln.xml",
            )
        )
        parser = SslyzeParser()
        findings = parser.get_findings(testfile, Test())
