...
class TestGitleaksParser(DojoTestCase):
    ...
    def test_parse_file_legacy_with_multiple_redacted_finding(self):
        testfile = open(
            get_unit_tests_path() + "/scans/gitleaks/redacted_data_many.json"
        )
        parser = GitleaksParser()
        findings = parser.get_findings(testfile, Test())
    ...
    def test_parse_file_from_version_8(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertIn(
                "74d53286c550630f80847d37f68aa3065554ac813544072ccd1278da71fafe31",
                finding.description,
            )
            self.assertIn(
                "9619c91b3fd2998be5d9ce198833d7ac9489d9bc378ad7cd28963d5a967f8699",
                finding.description,
            )
            self.assertIn(
        ...
        with self.subTest(i=1):
            finding = findings[1]
            self.assertEqual(
                "Hard coded RSA private key found in conf/rsa.pk", finding.title
            )
...
**Rule Id:** RSA-PK"""
        ...
        with self.subTest(i=2):
            finding = findings[2]
            self.assertEqual(
                "Hard coded Generic API Key found in tests/api.py", finding.title
            )
