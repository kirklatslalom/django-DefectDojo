...
class TestCargoAuditParser(DojoTestCase):
    ...
    def test_parse_many_findings(self):
        ...
        with self.subTest(i=0):
            finding = findings[0]
            self.assertEqual(
                "[arc-swap 0.4.7] Dangling reference in `access::Map` with Constant",
                finding.title,
            )
        ...
        with self.subTest(i=1):
            finding = findings[1]
            self.assertEqual(
                "[hyper 0.13.9] Multiple Transfer-Encoding headers misinterprets request payload",
                finding.title,
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
        ...
        with self.subTest(i=2):
            finding = findings[2]
            self.assertEqual(
                "[smallvec 0.6.13] Buffer overflow in SmallVec::insert_many",
                finding.title,
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
            self.assertEqual(
                ["buffer-overflow", "heap-overflow", "unsound"], finding.tags
            )
        ...
        with self.subTest(i=3):
            finding = findings[3]
            self.assertEqual(
                "[smallvec 1.5.0] Buffer overflow in SmallVec::insert_many",
                finding.title,
            )
            self.assertEqual("High", finding.severity)
            self.assertIsNotNone(finding.description)
