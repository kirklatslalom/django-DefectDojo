...
class TestBanditParser(DojoTestCase):
    ...
    def test_bandit_parser_has_one_finding(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual(
                datetime.datetime(2020, 12, 30, 10, 3, 39, tzinfo=tzlocal()), item.date
            )
    ...
    def test_bandit_parser_has_many_findings(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual(
                datetime.datetime(2020, 12, 30, 9, 35, 48, tzinfo=tzlocal()), item.date
            )
            self.assertEqual("Low", item.severity)
    ...
    def test_bandit_parser_has_many_findings_recent(self):
        ...
        with self.subTest(i=0):
            item = findings[0]
            self.assertEqual(
                "Use of insecure MD2, MD4, MD5, or SHA1 hash function.", item.title
            )
            self.assertEqual(
                datetime.datetime(2021, 3, 30, 18, 23, 12, tzinfo=tzlocal()), item.date
            )
    ...
    def test_bandit_parser_has_many_findings_recent2(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual("Try, Except, Pass detected.", item.title)
            self.assertEqual(
                datetime.datetime(2021, 10, 3, 12, 53, 18, tzinfo=tzlocal()), item.date
            )
            self.assertEqual("Low", item.severity)
            self.assertEqual("dojo/benchmark/views.py", item.file_path)
            self.assertEqual("try_except_pass:B110", item.vuln_id_from_tool)
        ...
        with self.subTest(i=164):
            ...
            self.assertEqual(
                "hardcoded_bind_all_interfaces:B104", item.vuln_id_from_tool
            )
