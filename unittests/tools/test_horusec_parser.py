...
class TestHorusecParser(DojoTestCase):
    def test_get_findings(self):
        ...
        with open(
            path.join(path.dirname(__file__), "../scans/horusec/version_2.6.3.json")
        ) as testfile:
    ...
    def test_get_tests_ok(self):
        """Version 2.6.3 with big project in Python"""
        with open(
            path.join(path.dirname(__file__), "../scans/horusec/horres3.json")
        ) as testfile:
            parser = HorusecParser()
            tests = parser.get_tests("Horusec Scan", testfile)
            self.assertEqual(1, len(tests))
    ...
    def test_get_tests_issue_6258(self):
        """"""
        with open(
            path.join(path.dirname(__file__), "../scans/horusec/issue_6258.json")
        ) as testfile:
            parser = HorusecParser()
            tests = parser.get_tests("Horusec Scan", testfile)
            self.assertEqual(1, len(tests))
            ...
            with self.subTest(i=0):
                ...
                self.assertEqual("Critical", finding.severity)
                self.assertEqual(
                    "GetGestaoVisaoWeb/src/main/webapp/js/jquery/jquery-ui-1.9.1.custom.min.js",
                    finding.file_path,
                )
            ...
            with self.subTest(i=10):
                ...
                self.assertEqual("Critical", finding.severity)
                self.assertEqual(
                    "resources-external/GetGestao/security/env.xml", finding.file_path
                )
            ...
            with self.subTest(i=13):
                ...
                self.assertEqual("Medium", finding.severity)
                self.assertEqual(
                    "GetGestaoVisaoWeb/src/main/java/br/gov/dataprev/get/web/util/ErrorHandlingUtils.java",
                    finding.file_path,
                )
    ...
    def test_get_tests_pr_6563(self):
        """"""
        with open(
            path.join(path.dirname(__file__), "../scans/horusec/pr_6563.json")
        ) as testfile:
            parser = HorusecParser()
            tests = parser.get_tests("Horusec Scan", testfile)
            self.assertEqual(1, len(tests))
