    ...
    def assert_file_has_n_items(self, filename, item_count):
        with open(filename) as file:
            ...
            if item_count == 1:
                ...
                self.assertEqual(
                    "CVE-2019-9315", findings[0].unsaved_vulnerability_ids[0]
                )

    def test_parser_no_items(self):
        self.assert_file_has_n_items(
            get_unit_tests_path() + "/scans/outpost24/none.xml", 0
        )

    def test_parser_one_item(self):
        self.assert_file_has_n_items(
            get_unit_tests_path() + "/scans/outpost24/one.xml", 1
        )

    def test_parser_sample_items(self):
        self.assert_file_has_n_items(
            get_unit_tests_path() + "/scans/outpost24/sample.xml", 24
        )
