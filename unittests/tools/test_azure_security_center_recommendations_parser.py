...
from dojo.tools.azure_security_center_recommendations.parser import (
    AzureSecurityCenterRecommendationsParser,
)
...
class TestAzureSecurityCenterRecommendationsParser(DojoTestCase):
    def test_parse_file_with_no_findings(self):
        testfile = open(
            "unittests/scans/azure_security_center_recommendations/zero_vulns.csv"
        )
    ...
    def test_parse_file_with_multiple_findings(self):
        testfile = open(
            "unittests/scans/azure_security_center_recommendations/many_vulns.csv"
        )
        parser = AzureSecurityCenterRecommendationsParser()
        findings = parser.get_findings(testfile, Test())
        ...
        finding = findings[0]
        self.assertEqual(
            "my_virtual_network - Virtual networks should be protected by Azure Firewall",
            finding.title,
        )
...
**Subscription Id:** 9cfbad7a-7369-42e4-bcce-7677c5b3a44b"""
        ...
        self.assertEqual(
            "f67fb4ed-d481-44d7-91e5-efadf504f74a", finding.vuln_id_from_tool
        )

        finding = findings[1]
        self.assertEqual(
            "My first subscription - Azure Defender for Resource Manager should be enabled",
            finding.title,
        )
        self.assertEqual(date.fromisoformat("2021-09-28"), finding.date)
        self.assertEqual(1032, finding.cwe)
...
**Native Cloud Account Id:** my_native_cloud_id"""
        ...
        self.assertEqual(unique_id_from_tool, finding.unique_id_from_tool)
        self.assertEqual(
            "f0fb2a7e-16d5-849f-be57-86db712e9bd0", finding.vuln_id_from_tool
        )

        finding = findings[2]
        self.assertEqual(
            "swe10032201245e263h - Storage account should use a private link connection",
            finding.title,
        )
        self.assertEqual(date.fromisoformat("2021-09-28"), finding.date)
        self.assertEqual(1032, finding.cwe)
...
**Subscription Id:** 9cfbad7a-7369-42e4-bcce-7677c5b3a44b"""
        ...
        self.assertEqual(unique_id_from_tool, finding.unique_id_from_tool)
        self.assertEqual(
            "cdc78c07-02b0-4af0-1cb2-cb7c672a8b0a", finding.vuln_id_from_tool
        )
