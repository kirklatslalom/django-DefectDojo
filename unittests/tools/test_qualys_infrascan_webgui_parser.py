...
class TestQualysInfrascanWebguiParser(DojoTestCase):
    def test_parse_file_with_no_vuln_has_no_findings(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/qualys_infrascan_webgui/qualys_infrascan_webgui_0.xml"
        )
    ...
    def test_parse_file_with_one_vuln_has_one_findings(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/qualys_infrascan_webgui/qualys_infrascan_webgui_1.xml"
        )
        parser = QualysInfrascanWebguiParser()
        findings = parser.get_findings(testfile, Test())
        ...
        finding = findings[0]
        self.assertEqual(
            "Oracle Java SE Critical Patch Update - January 2015", finding.title
        )
        self.assertEqual(
            "Critical", finding.severity
        )  # Negligible is translated to Informational
        self.assertEqual(
            datetime(2019, 4, 2, 10, 14, 53, tzinfo=pytz.utc), finding.date
        )
    ...
    def test_parse_file_with_multiple_vuln_has_multiple_findings(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/qualys_infrascan_webgui/qualys_infrascan_webgui_multiple.xml"
        )
        parser = QualysInfrascanWebguiParser()
        findings = parser.get_findings(testfile, Test())
        for finding in findings:
    ...
    def test_parse_file_with_finding_no_dns(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/qualys_infrascan_webgui/qualys_infrascan_webgui_3.xml"
        )
        parser = QualysInfrascanWebguiParser()
        findings = parser.get_findings(testfile, Test())
        for finding in findings:
