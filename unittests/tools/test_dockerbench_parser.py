...
class TestDockerBenchParser(DojoTestCase):
    def test_parse_file_with_no_vuln_has_no_findings(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/dockerbench/docker-bench-report-zero-vulns.json"
        )
    ...
    def test_parse_file_with_one_vuln_has_one_finding(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/dockerbench/docker-bench-report-single-vuln.json"
        )
        parser = DockerBenchParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
        ...
        self.assertIn(
            "Ensure base device size is not changed until needed", finding.description
        )

    def test_parse_file_with_multiple_vuln_has_multiple_findings(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/dockerbench/docker-bench-report-many-vulns.json"
        )
        parser = DockerBenchParser()
        findings = parser.get_findings(testfile, Test())
        self.assertTrue(len(findings) == 50)
        self.assertTrue(
            sum(1 for f in findings if f.severity.upper() == "CRITICAL") == 0
        )
        ...
        self.assertIn(
            "Install auditd. Add -a exit,always -F path=/run/containerd -F perm=war -k docker to the /etc/audit/rules.d/audit.rules file. Then restart the audit daemon using command service auditd restart.",
            finding.mitigation,
        )
        self.assertIn(
            "Audit can generate large log files. So you need to make sure that they are rotated and archived periodically. Create a separate partition for audit logs to avoid filling up other critical partitions.",
            finding.mitigation,
        )
        ...
        self.assertIn(
            "Ensure that experimental features are not implemented in production (Scored)",
            finding.description,
        )
        self.assertIn(
        ...
        self.assertIn(
            "Ensure Content trust for Docker is Enabled (Automated)",
            finding.description,
        )
        self.assertIn(
            "Add DOCKER_CONTENT_TRUST variable to the /etc/environment file using command echo DOCKER_CONTENT_TRUST=1 | sudo tee -a /etc/environment.",
            finding.mitigation,
        )
        self.assertIn(
