...
class TestSslLabsParser(DojoTestCase):
    ...
    def test_parse_ok(self):
        ...
        self.assertEqual(
            findings[0].unsaved_endpoints[0].host, "defectdojo.mevitae.com"
        )
    ...
    def test_parse_multiple(self):
        parser = SslLabsParser()
        with open(
            "unittests/scans/ssl_labs/ssl_labs_multiple_v1.5.0.json"
        ) as test_file:
            findings = parser.get_findings(test_file, Test())
        for finding in findings:
            for endpoint in finding.unsaved_endpoints:
        ...
        for finding in findings:
            ...
            if finding.severity == "Info":
                self.assertEqual(
                    finding.title, "TLS Grade 'A+' for defectdojo.mevitae.com"
                )
                self.assertEqual(len(finding.unsaved_endpoints), 4)
                self.assertEqual(
                    finding.unsaved_endpoints[0].host, "defectdojo.mevitae.com"
                )
                foundInfo = True
                ...
                self.assertEqual(len(finding.unsaved_endpoints), 3)
                self.assertEqual(
                    finding.unsaved_endpoints[0].host, "revoked.badssl.com"
                )
                foundCritical = True
            else:
