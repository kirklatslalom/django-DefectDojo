...
class TestBundlerAuditParser(DojoTestCase):
    def test_get_findings(self):
        with open(
            path.join(
                ...
                "../scans/bundler_audit/bundler-audit_v0.6.1.txt",
            )
        ) as testfile:
            ...
            with self.subTest(i=0):
                finding = findings[0]
                self.assertEqual(
                    "Gem rack: Possible XSS vulnerability in Rack [CVE-2018-16471]",
                    finding.title,
                )
            ...
            with self.subTest(i=1):
                finding = findings[1]
                self.assertEqual(
                    "Gem sprockets: Path Traversal in Sprockets [CVE-2018-3760]",
                    finding.title,
                )
                self.assertEqual("Medium", finding.severity)
                self.assertEqual(1, len(finding.unsaved_vulnerability_ids))
    ...
    def test_get_findings_version9(self):
        with open(
            path.join(path.dirname(__file__), "../scans/bundler_audit/version_9.0.txt")
        ) as testfile:
            parser = BundlerAuditParser()
            findings = parser.get_findings(testfile, Test())
            ...
            with self.subTest(i=0):
                finding = findings[0]
                self.assertEqual(
                    "Gem rack: Directory traversal in Rack::Directory app bundled with Rack [CVE-2020-8161]",
                    finding.title,
                )
                self.assertEqual("Medium", finding.severity)
                self.assertEqual(1, len(finding.unsaved_vulnerability_ids))
            ...
            with self.subTest(i=1):
                finding = findings[1]
                self.assertEqual(
                    "Gem rack: Percent-encoded cookies can be used to overwrite existing prefixed cookie names [CVE-2020-8184]",
                    finding.title,
                )
                self.assertEqual("Medium", finding.severity)
                self.assertEqual(1, len(finding.unsaved_vulnerability_ids))
