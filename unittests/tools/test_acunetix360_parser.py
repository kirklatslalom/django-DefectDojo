...
class TestAcunetix360Parser(DojoTestCase):
    def test_parse_file_with_one_finding(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertTrue(
                "https://online.acunetix360.com/issues/detail/735f4503-e9eb-4b4c-4306-ad49020a4c4b"
                in finding.references
            )

    def test_parse_file_with_one_finding_false_positive(self):
        testfile = open(
            "unittests/scans/acunetix360/acunetix360_one_finding_false_positive.json"
        )
        parser = Acunetix360Parser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
    ...
    def test_parse_file_with_one_finding_risk_accepted(self):
        testfile = open(
            "unittests/scans/acunetix360/acunetix360_one_finding_accepted_risk.json"
        )
        parser = Acunetix360Parser()
        findings = parser.get_findings(testfile, Test())
        self.assertEqual(1, len(findings))
