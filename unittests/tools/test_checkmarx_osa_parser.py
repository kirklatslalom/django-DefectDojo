...
class TestCheckmarxOsaParser(DojoTestCase):
    ...
    def test_checkmarx_osa_parse_file_with_single_vulnerability_has_single_finding(
        ...
        self.assertEqual(
            "2A3E02E74053088617923D6FE19F14E8188B5271", item.unique_id_from_tool
        )
        self.assertEqual(str, type(item.title))
        self.assertEqual(
            "com.fasterxml.jackson.core:jackson-databind 2.10.2 | CVE-2020-25649",
            item.title,
        )
        ...
        self.assertEqual(
            datetime.strptime("2020-12-03T17:15:00", "%Y-%m-%dT%H:%M:%S"),
            item.publish_date,
        )
        ...
        self.assertEqual(
            "https://nvd.nist.gov/vuln/detail/CVE-2020-25649", item.references
        )
    ...
    def test_checkmarx_osa_parse_file_with_false_positive_is_false_positive(
        ...
        my_file_handle, product, engagement, test = self.init(
            get_unit_tests_path()
            + "/scans/checkmarx_osa/single_finding_false_positive.json"
        )
        parser = CheckmarxOsaParser()
        findings = parser.get_findings(my_file_handle, test)
        self.teardown(my_file_handle)
    ...
    def test_checkmarx_osa_parse_file_with_no_libraryId_raises_ValueError(
        ...
        with self.assertRaises(ValueError) as context:
            my_file_handle, product, engagement, test = self.init(
                get_unit_tests_path()
                + "/scans/checkmarx_osa/single_finding_no_libraryId.json"
            )
            parser = CheckmarxOsaParser()
            parser.get_findings(my_file_handle, test)
            self.teardown(my_file_handle)
            self.assertTrue(
                "Invalid format: missing mandatory field libraryId:"
