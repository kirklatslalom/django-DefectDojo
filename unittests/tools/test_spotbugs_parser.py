...
class TestSpotbugsParser(DojoTestCase):
    def test_no_findings(self):
        ...
        findings = parser.get_findings(
            get_unit_tests_path() + "/scans/spotbugs/no_finding.xml", Test()
        )
    ...
    def test_parse_many_finding(self):
        parser = SpotbugsParser()
        findings = parser.get_findings(
            get_unit_tests_path() + "/scans/spotbugs/many_findings.xml", Test()
        )
    ...
    def test_find_sast_source_path(self):
        ...
        test_finding = findings[0]
        self.assertEqual(
            "securitytest/command/IdentityFunctionCommandInjection.kt",
            test_finding.sast_source_file_path,
        )
    ...
    def test_file(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual(
                "securitytest.command.IdentityFunctionCommandInjection",
        ...
        with self.subTest(i=40):
            ...
            self.assertEqual(
                "securitytest/injection/KotlinLogging.kt", finding.file_path
            )
            self.assertEqual(23, finding.line)
            self.assertEqual(
                "securitytest.injection.KotlinLogging", finding.sast_source_object
            )
            self.assertEqual("Medium", finding.severity)
        ...
        with self.subTest(i=80):
            ...
            self.assertEqual(
                "securitytest/pathtraversal/PathTraversalKotlin.kt", finding.file_path
            )
            self.assertEqual(36, finding.line)
            self.assertEqual(
                "securitytest.pathtraversal.PathTraversalKotlin",
                finding.sast_source_object,
            )
            self.assertEqual("Medium", finding.severity)
    ...
    def test_version_4_4(self):
        ...
        with self.subTest(i=8):
            ...
            self.assertIsNone(
                finding.file_path
            )  # manage special case where file = 'N/A'
