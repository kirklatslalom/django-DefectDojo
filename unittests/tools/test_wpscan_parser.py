...
class TestWpscanParser(DojoTestCase):
    ...
    def test_parse_file_with_one_vuln_has_one_findings(self):
        ...
        self.assertNotEqual(
            "Info", finding.severity
        )  # it is a vulnerability so not 'Info'
        self.assertEqual(
            "YouTube Embed <= 11.8.1 - Cross-Site Request Forgery (CSRF)", finding.title
        )
    ...
    def test_parse_file_with_multiple_vuln(self):
        ...
        with self.subTest(i=0):
            finding = findings[0]
            self.assertEqual(
                "7391118e-eef5-4ff8-a8ea-f6b65f442c63", finding.unique_id_from_tool
            )
            self.assertNotEqual(
                "Info", finding.severity
            )  # it is a vulnerability so not 'Info'
            self.assertEqual(
                "Contact Form 7 < 5.3.2 - Unrestricted File Upload", finding.title
            )
    ...
    def test_parse_file_with_multiple_vuln_in_version(self):
        ...
        with self.subTest(i=2):
            finding = findings[2]
            self.assertEqual(
                "d40374cf-ee95-40b7-9dd5-dbb160b877b1", finding.unique_id_from_tool
            )
            self.assertNotEqual(
                "Info", finding.severity
            )  # it is a vulnerability so not 'Info'
    ...
    def test_parse_file_issue5774(self):
        ...
        with self.subTest(i=0):
            finding = findings[0]
            self.assertEqual(
                "16353d45-75d1-4820-b93f-daad90c322a8", finding.unique_id_from_tool
            )
            ...
            self.assertEqual(
                "Tentative", finding.get_scanner_confidence_text()
            )  # data are at 30%
        ...
        with self.subTest(i=50):
            finding = findings[50]
            self.assertEqual(
                "6a3ec618-c79e-4b9c-9020-86b157458ac5", finding.unique_id_from_tool
            )
            self.assertNotEqual("Info", finding.severity)
            self.assertEqual(
