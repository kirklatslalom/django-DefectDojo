...
class TestAwsProwlerParser(DojoTestCase):
    ...
    def test_aws_prowler_parser_with_many_vuln_has_many_findings(self):
        ...
        self.assertEqual(
            "User example_user has never used access key 1 since creation and not rotated it in the past 90 days",
            findings[1].title,
        )
        self.assertEqual("Medium", findings[1].severity)
        self.assertEqual(
            "Password Policy has weak reuse requirement (lower than 24)",
            findings[2].title,
        )
        self.assertEqual("Medium", findings[2].severity)
        self.assertEqual(
            "eu-west-2: sg-01234567890qwerty is not being used!", findings[3].title
        )
    ...
    def test_aws_prowler_parser_with_many_vuln_has_many_findings2(self):
        ...
        self.assertEqual(
            "User example has never used access key 1 since creation and not rotated it in the past 90 days",
            findings[4].title,
        )
    ...
    def test_aws_prowler_parser_issue4450(self):
        ...
        with self.subTest(i=1):
            ...
            self.assertEqual(
                "User ansible-test-user has Password enabled but MFA disabled",
                finding.title,
            )
    ...
    def test_aws_prowler_parser_with_critical_vuln_has_one_findings_json(self):
        ...
        self.assertEqual(1, len(findings))
        self.assertEqual(
            "eu-central-1: Only Virtual MFA is enabled for root", findings[0].title
        )
        ...
        self.assertIn(
            "The root account is the most privileged user in an AWS account. MFA adds an extra layer",
            findings[0].impact,
        )
        self.assertEqual(
    ...
    def test_aws_prowler_parser_with_many_vuln_has_many_findings_json(self):
        ...
        with self.subTest(i=1):
            self.assertEqual(
                "eu-central-1: Cluster control plane access is not restricted for EKS cluster prod",
                findings[1].title,
            )
            self.assertEqual("High", findings[1].severity)
        with self.subTest(i=2):
            self.assertEqual(
                "eu-central-1: Control plane logging is not enabled for EKS cluster prod",
                findings[2].title,
            )
            self.assertEqual("Medium", findings[2].severity)
        with self.subTest(i=3):
            self.assertEqual(
                "eu-central-1: prod.config_read.iam has inline policy directly attached",
                findings[3].title,
            )
            self.assertEqual("Low", findings[3].severity)
