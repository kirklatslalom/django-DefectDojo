...
class TestNmapParser(DojoTestCase):
    ...
    def test_parse_issue4406(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual(
                "MSF:ILITIES/UBUNTU-CVE-2019-6111/", finding.vuln_id_from_tool
            )
        ...
        with self.subTest(i=27):
            ...
            self.assertEqual("Medium", finding.severity)
            self.assertEqual(
                "EXPLOITPACK:EBDBC5685E3276D648B4D14B75563283", finding.title
            )
            self.assertEqual(datetime.datetime(2021, 4, 29, 9, 26, 36), finding.date)
            self.assertEqual(
        ...
        with self.subTest(i=48):
            ...
            endpoint = finding.unsaved_endpoints[0]
            self.assertEqual(
                "ip-10-250-195-71.eu-west-1.compute.internal", endpoint.host
            )
