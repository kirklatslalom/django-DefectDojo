...
class TestScoutSuiteParser(DojoTestCase):
    ...
    def test_scout_suite_parser_with_two_findings(self):
        ...
        with self.subTest(i=0):
            ...
            self.assertEqual(
                "gcp:cloudstorage-bucket-no-logging", finding.vuln_id_from_tool
            )
        with self.subTest(i=2):
            ...
            self.assertEqual(
                "gcp:cloudstorage-bucket-no-versioning", finding.vuln_id_from_tool
            )
    ...
    def test_get_findings(self):
        ...
        with self.subTest(i=29):
            ...
            self.assertEqual(1032, finding.cwe)
            self.assertEqual(
                "aws:config-recorder-not-configured", finding.vuln_id_from_tool
            )
    ...
    def test_get_tests(self):
        ...
        self.assertIn(
            "Amazon Web Services", test.description
        )  # check that the Cloud provider is in the description
        self.assertIn(
            "430150006394", test.description
        )  # check that the account is in the description (very usefull)
