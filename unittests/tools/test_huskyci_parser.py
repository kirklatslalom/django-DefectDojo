...
class TestHuskyCIParser(DojoTestCase):
    ...
    def test_parse_file_has_one_finding_one_tool(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/huskyci/huskyci_report_one_finding_one_tool.json"
        )
        parser = HuskyCIParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_parse_file_has_many_finding_one_tool(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/huskyci/huskyci_report_many_finding_one_tool.json"
        )
        parser = HuskyCIParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_parse_file_has_many_finding_two_tools(self):
        testfile = open(
            get_unit_tests_path()
            + "/scans/huskyci/huskyci_report_many_finding_two_tools.json"
        )
        parser = HuskyCIParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
