...
class EndpointTest(APITestCase):
    ...
    def test_endpoint_missing_host_product(self):
        r = self.client.post(
            reverse("endpoint-list"), {"host": "FOO.BAR"}, format="json"
        )
    ...
    def test_endpoint_add_existing(self):
        r = self.client.post(
            reverse("endpoint-list"), {"product": 1, "host": "FOO.BAR"}, format="json"
        )
    ...
    def test_endpoint_change_product(self):
        r = self.client.post(
            reverse("endpoint-list"), {"product": 1, "host": "product1"}, format="json"
        )
        eid = r.json()["id"]
        self.assertEqual(r.status_code, 201, r.content[:1000])

        r = self.client.patch(
            reverse("endpoint-detail", args=(eid,)), {"product": 2}, format="json"
        )
        self.assertEqual(r.status_code, 400, r.content[:1000])
    ...
    def test_endpoint_remove_host(self):
        ...
        self.assertEqual(r.status_code, 201, r.content[:1000])
        r = self.client.patch(
            reverse("endpoint-detail", args=(eid,)), {"host": None}, format="json"
        )
        self.assertEqual(r.status_code, 400, r.content[:1000])
        self.assertIn("Host must not be empty", r.content.decode("utf-8"))
