...
def create(
    when: datetime, product_id: int, titles_and_severities: List[Tuple[str, str]]
):
    with patch("django.db.models.fields.timezone.now") as mock_now:
        mock_now.return_value = when
        engagement = Engagement.objects.create(
            product_id=product_id, target_start=when.date(), target_end=when.date()
        )
        test = Test.objects.create(
            engagement=engagement, test_type_id=120, target_start=when, target_end=when
        )
...
def create_with_duplicates(
    ...
    with patch("django.db.models.fields.timezone.now") as mock_now:
        ...
        originals = Finding.objects.filter(
        ...
        Finding.objects.bulk_create(
            (
                Finding(
                    ...
                    severity=severity,
                    verified=False,
                    duplicate=(title in originals_map),
                    duplicate_finding=originals_map.get(title),
                )
                for title, severity in titles_and_severities
            )
        )
...
def mitigate(when: datetime, product_id: int, title: str):
    with patch("django.db.models.fields.timezone.now") as mock_now:
        ...
        Finding.objects.filter(
            test__engagement__product_id=product_id, title=title
        ).update(is_mitigated=True, mitigated=when)
...
class TestDashboard(DojoTestCase):
    ...
    def setUpTestData(cls) -> None:
        System_Settings.objects.update(
            enable_deduplication=False
        )  # The default deduplication does not work.
    ...
    def test_charts_as_staff(self):
        ...
        expected = [
            {
                "y": f"{self.month_ago.year}-{self.month_ago.month:02}",
                "a": 1,
                "b": 1,
                "c": 1,
                "d": 1,
            ...
            {
                "y": f"{self.now.year}-{self.now.month:02}",
                "a": 1,
                "b": 0,
                "c": 0,
                "d": 0,
