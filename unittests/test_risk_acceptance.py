...
class RiskAcceptanceTestUI(DojoTestCase):
    ...
    def add_risk_acceptance(self, eid, data_risk_accceptance, fid=None):
        ...
        response = self.client.post(
            reverse("add_risk_acceptance", args=args), data_risk_accceptance
        )
    ...
    def test_add_findings_to_risk_acceptance_findings_accepted(self):
        ...
        response = self.client.post(
            reverse("view_risk_acceptance", args=(1, ra.id)),
            urlencode(MultiValueDict(data_add_findings_to_ra), doseq=True),
            content_type="application/x-www-form-urlencoded",
        )

        self.assertEqual(302, response.status_code, response.content[:1000])
        self.assert_all_inactive_risk_accepted(
            Finding.objects.filter(id__in=[2, 3, 4, 5])
        )
    ...
    def test_remove_findings_from_risk_acceptance_findings_active(self):
        ...
        ra = Risk_Acceptance.objects.last()
        response = self.client.post(
            reverse("view_risk_acceptance", args=(1, ra.id)), data
        )
        self.assertEqual(302, response.status_code, response.content[:1000])
    ...
    def test_remove_risk_acceptance_findings_active(self):
        ...
        self.assert_all_active_not_risk_accepted(
            Finding.objects.filter(test__engagement=1)
        )
    ...
    def test_expire_risk_acceptance_findings_active(self):
        ...
        self.assertTrue(
            all(finding in ra.accepted_findings.all() for finding in findings)
        )
    ...
    def test_expire_risk_acceptance_findings_not_active(self):
        ...
        self.assert_all_active_not_risk_accepted(
            Finding.objects.filter(test__engagement=1).filter(~Q(id=2))
        )
        # findings remain in (expired) risk acceptance
        self.assertTrue(
            all(finding in ra.accepted_findings.all() for finding in findings)
    ...
    def test_expire_risk_acceptance_sla_not_reset(self):
        ...
        self.assertTrue(
            all(finding.sla_start_date != timezone.now().date() for finding in findings)
        )
    ...
    def test_reinstate_risk_acceptance_findings_accepted(self):
        ...
        risk_acceptance_expiration_date = timezone.now() + relativedelta(
            days=expiration_delta_days
        )

        self.assertEqual(
            ra.expiration_date.date(), risk_acceptance_expiration_date.date()
        )
    ...
    def test_expiration_handler(self):
        ...
        ra1.expiration_date = timezone.now().date() + relativedelta(
            days=heads_up_days - 1
        )
        ra2.expiration_date = timezone.now().date() + relativedelta(
            days=heads_up_days + 1
        )
        ...
        to_warn = ra_helper.get_almost_expired_risk_acceptances_to_handle(
