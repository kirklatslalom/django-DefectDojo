...
class TestAuthorizationDecorators(DojoTestCase):
    def setUp(self):
        ...
        self.decorated_func = user_is_authorized(
            Product_Type, Permissions.Product_Type_View, "id", "pk", Mock()
        )

    @patch(
        "dojo.authorization.authorization_decorators.get_object_or_404",
        side_effect=Http404(),
    )
    ...
    @patch(
        "dojo.authorization.authorization_decorators.user_has_permission_or_403",
        side_effect=PermissionDenied(),
    )
    def test_authorization_permission_denied(
        self, mock_user_has_permission, mock_shortcuts_get
    ):
        ...
        mock_user_has_permission.assert_called_with(
    ...
    def test_authorization_user_has_permission(
        self, mock_user_has_permission, mock_shortcuts_get
    ):
        mock_shortcuts_get.return_value = self.product_type
...
class TestConfigurationAuthorizationDecorators(DojoTestCase):
    ...
    @patch(
        "dojo.authorization.authorization_decorators.user_has_configuration_permission"
    )
