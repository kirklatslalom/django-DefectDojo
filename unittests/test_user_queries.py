...
class TestUserQueries(DojoTestCase):
    def setUp(self):
        ...
        Global_Role(
            user=self.global_permission_user, role=Role.objects.get(name="Reader")
        ).save()
        ...
        Product_Member(
            user=self.product_type_user,
            product=self.product_2,
            role=Role.objects.get(name="Maintainer"),
        ).save()
    ...
    def test_user_none(self, mock_current_user):
        ...
        self.assertQuerysetEqual(
            Dojo_User.objects.none(), get_authorized_users(Permissions.Product_View)
        )
    ...
    def test_user_regular(self, mock_current_user_1, mock_current_user_2):
        ...
        users = Dojo_User.objects.exclude(username="invisible_user").order_by(
