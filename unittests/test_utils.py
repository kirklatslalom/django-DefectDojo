...
TEST_IMPORT_FINDING_ACTION_CREATED = TEST_IMPORT_FINDING_ACTION_ALL.filter(
    action=IMPORT_CREATED_FINDING
)
TEST_IMPORT_FINDING_ACTION_CLOSED = TEST_IMPORT_FINDING_ACTION_ALL.filter(
    action=IMPORT_CLOSED_FINDING
)
TEST_IMPORT_FINDING_ACTION_REACTIVATED = TEST_IMPORT_FINDING_ACTION_ALL.filter(
    action=IMPORT_REACTIVATED_FINDING
)
TEST_IMPORT_FINDING_ACTION_UNTOUCHED = TEST_IMPORT_FINDING_ACTION_ALL.filter(
...
class TestUtils(DojoTestCase):
    ...
    def test_user_post_save_without_template(
    ...
    @patch("dojo.utils.Dojo_Group_Member")
    @patch("dojo.utils.Notifications")
    def test_user_post_save_with_template(
        self, mock_notifications, mock_member, mock_settings
    ):
        user = Dojo_User()
        user.id = 1
    ...
    @patch("dojo.utils.Dojo_Group_Member")
    @patch("dojo.utils.Notifications")
    def test_user_post_save_email_pattern_matches(
        self, mock_notifications, mock_member, mock_settings
    ):
        user = Dojo_User()
        user.id = 1
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        self.test_case.assertEqual(
            ...
            % (
                ...
                self.num,
                self.queryset.query,
                self.queryset.all().order_by("-id")[:100],
            ),
        )


@contextmanager
def assertTestImportModelsCreated(
    test_case,
    imports=0,
    reimports=0,
    affected_findings=0,
    ...
    with assertNumOfModelsCreated(
        test_case, TEST_IMPORTS, num=imports
    ) as ti_import_count, assertNumOfModelsCreated(
        test_case, TEST_REIMPORTS, num=reimports
    ) as ti_reimport_count, assertNumOfModelsCreated(
        test_case, TEST_IMPORT_FINDING_ACTION_AFFECTED, num=affected_findings
    ) as tifa_count, assertNumOfModelsCreated(
        test_case, TEST_IMPORT_FINDING_ACTION_CREATED, num=created
    ) as tifa_created_count, assertNumOfModelsCreated(
        test_case, TEST_IMPORT_FINDING_ACTION_CLOSED, num=closed
    ) as tifa_closed_count, assertNumOfModelsCreated(
        test_case, TEST_IMPORT_FINDING_ACTION_REACTIVATED, num=reactivated
    ) as tifa_reactivated_count, assertNumOfModelsCreated(
        test_case, TEST_IMPORT_FINDING_ACTION_UNTOUCHED, num=untouched
    ) as tifa_untouched_count:
...
def assertImportModelsCreated(
    test_case, tests=0, engagements=0, products=0, product_types=0, endpoints=0
):

    with assertNumOfModelsCreated(
        test_case, TESTS, num=tests
    ) as test_count, assertNumOfModelsCreated(
        test_case, ENGAGEMENTS, num=engagements
    ) as engagement_count, assertNumOfModelsCreated(
        test_case, PRODUCTS, num=products
    ) as product_count, assertNumOfModelsCreated(
        test_case, PRODUCT_TYPES, num=product_types
    ) as product_type_count, assertNumOfModelsCreated(
        test_case, ENDPOINTS, num=endpoints
    ) as endpoint_count:
