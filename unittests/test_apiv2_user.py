...
class UserTest(APITestCase):
    ...
    def test_user_add(self):
        ...
        r = self.client.post(
            reverse("user-list"), {"username": "api-user-1"}, format="json"
        )
        ...
        r = self.client.post(
            reverse("user-list"),
            {"username": "api-user-2", "password": password},
            format="json",
        )
        self.assertEqual(r.status_code, 201, r.content[:1000])
        ...
        r = self.client.post(
            reverse("user-list"),
            {"username": "api-user-3", "password": "weakPassword"},
            format="json",
        )
        self.assertEqual(r.status_code, 400, r.content[:1000])
        self.assertIn(
            "Password must contain at least 1 digit, 0-9.", r.content.decode("utf-8")
        )

    def test_user_change_password(self):
        # some user
        r = self.client.post(
            reverse("user-list"), {"username": "api-user-4"}, format="json"
        )
        self.assertEqual(r.status_code, 201, r.content[:1000])
        user_id = r.json()["id"]

        r = self.client.put(
            "{}{}/".format(reverse("user-list"), user_id),
            {"username": "api-user-4", "first_name": "first"},
            format="json",
        )
        self.assertEqual(r.status_code, 200, r.content[:1000])
        ...
        r = self.client.put(
            "{}{}/".format(reverse("user-list"), user_id),
            {"username": "api-user-4", "password": "testTEST1234!@#$"},
            format="json",
        )
        self.assertEqual(r.status_code, 400, r.content[:1000])
        self.assertIn(
            "Update of password though API is not allowed", r.content.decode("utf-8")
        )
