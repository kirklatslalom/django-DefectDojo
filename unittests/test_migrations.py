...
class TestOptiEndpointStatus(MigratorTestCase):
    ...
    def prepare(self):
        ...
        self.endpoint = Endpoint.objects.create(
            host="foo.bar", product_id=self.product.pk
        ).pk
        self.endpoint_status = Endpoint_Status.objects.create(
            finding_id=self.finding, endpoint_id=self.endpoint
        ).pk
    ...
    def presudotest_before_migration(self):
        ...
        with self.subTest("Old: List EPS from endpoint"):
            ...
            self.assertEqual(
                str(exc.exception),
                "'Endpoint' object has no attribute 'status_endpoint'",
            )
    ...
    def test_after_migration(self):
        ...
        with self.subTest("New: Add existing EPS to endpoint"):
            ...
            self.assertEqual(
                str(exc.exception),
                "'Endpoint' object has no attribute 'endpoint_status'",
            )
        ...
        with self.subTest("New: Add existing EPS to finding"):
            ...
            self.assertEqual(
                str(exc.exception),
                "'Finding' object has no attribute 'endpoint_status'",
            )
