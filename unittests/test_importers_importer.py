...
class TestDojoDefaultImporter(DojoTestCase):
    ...
    def test_import_scan(self):
        ...
        scan_type = SarifParser().get_scan_types()[
            0
        ]  # SARIF format implement the new method
        ...
        test, len_new_findings, len_closed_findings, _ = importer.import_scan(
            scan,
            scan_type,
            engagement,
            lead=None,
    ...
    def test_import_scan_without_test_scan_type(self):
        ...
        scan = open(
            get_unit_tests_path() + "/scans/gitlab_sast/gl-sast-report-1-vuln.json"
        )
...
class FlexibleImportTestAPI(DojoAPITestCase):
    ...
    def create_default_data(self):
        ...
        self.engagement = self.create_engagement(
            ENGAGEMENT_NAME_DEFAULT, product=self.product
        )
        # engagement name is not unique by itself and not unique inside a product
        self.engagement_last = self.create_engagement(
            ENGAGEMENT_NAME_DEFAULT, product=self.product
        )

    @patch("dojo.jira_link.helper.get_jira_project")
    def test_import_by_engagement_id(self, mock):
        with assertImportModelsCreated(
            self, tests=1, engagements=0, products=0, product_types=0, endpoints=0
        ):
            import0 = self.import_scan_with_params(
                NPM_AUDIT_NO_VULN_FILENAME,
                scan_type=NPM_AUDIT_SCAN_TYPE,
                engagement=self.engagement.id,
                test_title=TEST_TITLE_DEFAULT,
            )
            test_id = import0["test"]
            self.assertEqual(
                get_object_or_none(Test, id=test_id).title, TEST_TITLE_DEFAULT
            )
    ...
    def test_import_by_product_name_exists_engagement_name_exists(self, mock):
        with assertImportModelsCreated(
            ...
            test_id = import0["test"]
            self.assertEqual(
                Test.objects.get(id=test_id).engagement, self.engagement_last
            )
    ...
    def test_import_by_product_name_exists_engagement_name_not_exists(self):
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, product_types=0, endpoints=0
        ):
            import0 = self.import_scan_with_params(
                NPM_AUDIT_NO_VULN_FILENAME,
                scan_type=NPM_AUDIT_SCAN_TYPE,
    ...
    @patch("dojo.jira_link.helper.get_jira_project")
    def test_import_by_product_name_exists_engagement_name_not_exists_auto_create(
        self, mock
    ):
        mock.return_value = None
        with assertImportModelsCreated(
            self, tests=1, engagements=1, products=0, product_types=0, endpoints=0
        ):
            import0 = self.import_scan_with_params(
                NPM_AUDIT_NO_VULN_FILENAME,
                scan_type=NPM_AUDIT_SCAN_TYPE,
            ...
            self.assertEqual(
                get_object_or_none(Engagement, id=import0["engagement_id"]).name,
                ENGAGEMENT_NAME_NEW,
            )
            self.assertEqual(import0["product_id"], self.engagement.product.id)
    ...
    def test_import_by_product_name_not_exists_engagement_name_auto_create(self, mock):
        with assertImportModelsCreated(
            self, tests=1, engagements=1, products=1, product_types=0, endpoints=0
        ):
            import0 = self.import_scan_with_params(
                NPM_AUDIT_NO_VULN_FILENAME,
                scan_type=NPM_AUDIT_SCAN_TYPE,
            ...
            self.assertEqual(
                get_object_or_none(Product, id=import0["product_id"]).prod_type.name,
                PRODUCT_TYPE_NAME_DEFAULT,
            )

        mock.assert_not_called()

    @patch("dojo.jira_link.helper.get_jira_project")
    def test_import_by_product_type_name_not_exists_product_name_not_exists_engagement_name_auto_create(
        self, mock
    ):
        with assertImportModelsCreated(
            self, tests=1, engagements=1, products=1, product_types=1, endpoints=0
        ):
            import0 = self.import_scan_with_params(
                NPM_AUDIT_NO_VULN_FILENAME,
                scan_type=NPM_AUDIT_SCAN_TYPE,
            ...
            self.assertEqual(
                get_object_or_none(Product_Type, id=import0["product_type_id"]).name,
                PRODUCT_TYPE_NAME_NEW,
            )
    ...
    def test_endpoint_meta_import_by_product_name_exists(self):
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, endpoints=0
        ):
            import0 = self.endpoint_meta_import_scan_with_params(
                ENDPOINT_META_IMPORTER_FILENAME,
                product=None,
                product_name=PRODUCT_NAME_DEFAULT,
    ...
    def test_import_with_invalid_parameters(self):
        ...
        with self.subTest("no product data"):
            import0 = self.import_scan_with_params(
                ...
                scan_type=NPM_AUDIT_SCAN_TYPE,
                engagement=None,
                engagement_name="what the bleep",
                expected_http_status_code=400,
            )
            self.assertEqual(import0, ["product_name parameter missing"])
        ...
        with self.subTest("invalid product type"):
            import0 = self.import_scan_with_params(
                ...
                scan_type=NPM_AUDIT_SCAN_TYPE,
                engagement=None,
                product_type_name="valentijn",
                product_name="67283",
                engagement_name="valentijn",
                expected_http_status_code=400,
            )
        ...
        with self.subTest("invalid product"):
            ...
            import0 = self.import_scan_with_params(
                ...
                scan_type=NPM_AUDIT_SCAN_TYPE,
                engagement=None,
                product_type_name=another_product_type_name,
                product_name=PRODUCT_NAME_DEFAULT,
                engagement_name="valentijn",
                expected_http_status_code=400,
            )
            self.assertEqual(
                ...
                [
                    ...
                    % (PRODUCT_NAME_DEFAULT, another_product_type_name)
                ],
            )
        ...
        with self.subTest("invalid engagement, but exists in another product"):
            ...
            self.assertEqual(
                import0,
                [
                    "Engagement 'Engagement 1' doesn't exist in Product '%s'"
...
class FlexibleReimportTestAPI(DojoAPITestCase):
    ...
    def create_default_data(self):
        ...
        self.test_last_by_title = self.create_test(
            engagement=self.engagement,
            scan_type=NPM_AUDIT_SCAN_TYPE,
            title=TEST_TITLE_DEFAULT,
        )
        self.test_with_title = self.create_test(
            engagement=self.engagement,
            scan_type=NPM_AUDIT_SCAN_TYPE,
            title=TEST_TITLE_ALTERNATE,
        )
        self.test_last_by_scan_type = self.create_test(
            engagement=self.engagement, scan_type=NPM_AUDIT_SCAN_TYPE
        )
    ...
    def test_reimport_by_test_id(self):
        with assertImportModelsCreated(
            ...
            import0 = self.reimport_scan_with_params(
                self.test.id, NPM_AUDIT_NO_VULN_FILENAME, scan_type=NPM_AUDIT_SCAN_TYPE
            )
            test_id = import0["test"]
            self.assertEqual(
                get_object_or_none(Test, id=test_id).title, TEST_TITLE_DEFAULT
    ...
    def test_reimport_by_product_name_exists_engagement_name_exists_no_title(self):
        with assertImportModelsCreated(
            ...
            self.assertEqual(
                import0["engagement_id"], self.test_last_by_scan_type.engagement.id
            )
            self.assertEqual(
                import0["product_id"], self.test_last_by_scan_type.engagement.product.id
            )

    def test_reimport_by_product_name_exists_engagement_name_exists_scan_type_not_exsists_test_title_exists(
        self,
    ):
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, product_types=0, endpoints=0
    ...
    @patch("dojo.jira_link.helper.get_jira_project")
    def test_reimport_by_product_name_exists_engagement_name_exists_scan_type_not_exsists_test_title_exists_auto_create(
        self, mock
    ):
        with assertImportModelsCreated(
            self, tests=1, engagements=0, products=0, product_types=0, endpoints=1
        ):
            import0 = self.reimport_scan_with_params(
                None,
    ...
    def test_reimport_by_product_name_exists_engagement_name_exists_test_title_exists(
        self,
    ):
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, product_types=0, endpoints=0
    ...
    def test_reimport_by_product_name_exists_engagement_name_not_exists_auto_create(
        ...
        with assertImportModelsCreated(
            ...
            self.assertEqual(import0["product_id"], self.engagement.product.id)
            self.assertEqual(
                import0["product_type_id"], self.engagement.product.prod_type.id
            )
        # the new engagement should inherit the jira settings from the product
        # the jira settings are retrieved before an engagement is auto created
        mock.assert_called_with(self.product)
    ...
    @patch("dojo.jira_link.helper.get_jira_project")
    def test_reimport_by_product_name_not_exists_engagement_name_auto_create(
        self, mock
    ):
        with assertImportModelsCreated(
            self, tests=1, engagements=1, products=1, product_types=0, endpoints=0
...
class TestImporterUtils(DojoAPITestCase):
    ...
    def test_handle_vulnerability_ids_references_and_cve(self, mock):
        ...
        self.assertEqual(
            vulnerability_ids,
            mock.mock_calls[0].kwargs["finding"].unsaved_vulnerability_ids,
        )
        ...
        self.assertEqual(
            vulnerability_ids,
            mock.mock_calls[2].kwargs["finding"].unsaved_vulnerability_ids,
        )
