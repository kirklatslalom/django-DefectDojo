...
class EndpointMetaImportMixin(object):
    def __init__(self, *args, **kwargs):
        ...
        self.meta_import_no_hostname = (
            "endpoint_meta_import/no_hostname_endpoint_meta_import.csv"
        )
        self.meta_import_updated_added = (
            "endpoint_meta_import/updated_added_endpoint_meta_import.csv"
        )
        self.meta_import_updated_removed = (
            "endpoint_meta_import/updated_removed_endpoint_meta_import.csv"
        )
        self.meta_import_updated_changed = (
            "endpoint_meta_import/updated_changed_endpoint_meta_import.csv"
        )
    ...
    def test_endpoint_meta_import_endpoint_create_tag_create_meta_create(self):
        ...
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, endpoints=3
        ):
            import0 = self.endpoint_meta_import_scan_with_params(
                self.meta_import_full,
    ...
    def test_endpoint_meta_import_tag_changed_column(self):
        ...
        endpoint = self.get_product_endpoints_api(1, host=self.updated_tag_host)[
            "results"
        ][0]
        human_resource_tag = endpoint["tags"][
            endpoint["tags"].index("team:human resources")
        ]
        # Import again with one column missing
        with assertImportModelsCreated(
            self, tests=0, engagements=0, products=0, endpoints=0
    ...
    def test_endpoint_meta_import_meta_changed_column(self):
        ...
        meta_count_before = self.db_dojo_meta_count()
        # Grab the endpoint that is known to change
        endpoint_id = self.get_product_endpoints_api(1, host=self.updated_tag_host)[
            "results"
        ][0]["id"]
        meta_value = self.get_endpoints_meta_api(endpoint_id, "team")["results"][0][
            "value"
        ]
        # Import again with one column missing
        with assertImportModelsCreated(
        ...
        meta_value_updated = self.get_endpoints_meta_api(endpoint_id, "team")[
            "results"
        ][0]["value"]
        # Make sure the tags are not the same
...
class EndpointMetaImportTestUI(DojoAPITestCase, EndpointMetaImportMixin):
    ...
    def endpoint_meta_import_ui(self, product, payload):
        ...
        response = self.client_ui.post(
            reverse("import_endpoint_meta", args=(product,)), payload
        )
        self.assertEqual(302, response.status_code, response.content[:1000])

    def endpoint_meta_import_scan_with_params_ui(
        self,
        filename,
        product=1,
        create_endpoints=True,
