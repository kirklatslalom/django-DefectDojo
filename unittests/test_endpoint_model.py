...
class TestEndpointModel(DojoTestCase):
    ...
    def test_url_full(self):
        endpoint = Endpoint.from_uri(
            "http://alice@foo.bar:8080/path1/path2?key1=value&no_value_key#fragment1"
        )
    ...
    def test_truncates_large_attributes(self):
        ...
        fragment = "baz" * 1000
        endpoint = Endpoint.from_uri(
            "http://alice@foo.bar:8080/{}?{}#{}".format(path, query, fragment)
        )
    ...
    def test_invalid(self):
        self.assertRaises(
            ValidationError, Endpoint.from_uri, "http://127.0.0.1:portNo/"
        )
    ...
    def test_get_or_create(self):
        ...
        endpoint7, created7 = endpoint_get_or_create(
            protocol="https", host="bar.foo", port=8443
        )
        self.assertTrue(created7)


@skip(
    "Outdated - this class was testing clean-up broken entries in old version of model; new version of model doesn't to store broken entries"
)
class TestEndpointStatusBrokenModel(DojoTestCase):
    def test_endpoint_status_broken(self):
        ...
        self.another_finding = Finding.objects.create(
            test=self.test, reporter_id=user
        ).pk
        self.another_endpoint = Endpoint.objects.create(
            protocol="http", host="bar.foo.eps"
        ).pk
        ...
        with self.subTest("Stadnard eps for finding"):
            ...
            self.assertEqual(
                f.endpoint_status.first().pk, self.endpoint_status["standard"]
            )
        ...
        with self.subTest("Stadnard eps for endpoint"):
            ...
            self.assertEqual(
                e.endpoint_status.first().pk, self.endpoint_status["standard"]
            )
...
class TestEndpointStatusModel(DojoTestCase):
    ...
    def test_status_evaluation(self):
        ...
        with self.subTest(
            "Endpoint with vulnerabilities but all of them are mitigated because of different reasons"
        ):
        ...
        with self.subTest("Host without vulnerabilities"):
            ...
            self.assertEqual(
                ep1.host_active_findings_count, 0, ep1.host_active_findings
            )
            self.assertEqual(
                ep2.host_active_findings_count, 0, ep2.host_active_findings
            )
            self.assertEqual(
                ep1.host_mitigated_endpoints_count, 1, ep1.host_mitigated_endpoints
            )
            self.assertEqual(
                ep2.host_mitigated_endpoints_count, 1, ep2.host_mitigated_endpoints
            )
        ...
        with self.subTest("Host with vulnerabilities"):
            ...
            self.assertEqual(
                ep3.host_active_findings_count, 1, ep3.host_active_findings
            )
            self.assertEqual(
                ep4.host_active_findings_count, 1, ep4.host_active_findings
            )
            self.assertEqual(
                ep5.host_active_findings_count, 1, ep5.host_active_findings
            )
            self.assertEqual(
                ep3.host_mitigated_endpoints_count, 2, ep3.host_mitigated_endpoints
            )
            self.assertEqual(
                ep4.host_mitigated_endpoints_count, 2, ep4.host_mitigated_endpoints
            )
            self.assertEqual(
                ep5.host_mitigated_endpoints_count, 2, ep5.host_mitigated_endpoints
            )
